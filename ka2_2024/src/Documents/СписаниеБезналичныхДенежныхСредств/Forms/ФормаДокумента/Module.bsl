...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаполнитьВариантКурсаДоговораРасшифровки();
...
Процедура ДоговорПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		Если ЗначениеЗаполнено(УчетнаяИнформацияПоДоговору) Тогда
			...
			ИсключаяСвойства = СтрокаИсключенныхСвойствПоДоговору(ТекущаяСтрока, УчетнаяИнформацияПоДоговору);
			ЗаполнитьЗначенияСвойств(ТекущаяСтрока, УчетнаяИнформацияПоДоговору, , ИсключаяСвойства);
...
Процедура ЗаполнитьКурсИКратностьВСтрокеРасшифровки(СтрокаТЧ)
	...
	КурсИКратность = ДенежныеСредстваСервер.КурсЧислительИКурсЗнаменательВзаиморасчетов(Объект.Валюта, СтрокаТЧ.ВалютаВзаиморасчетов,
		ВалютаРеглУчета, ?(ЗначениеЗаполнено(Объект.ДатаПроведенияБанком), Объект.ДатаПроведенияБанком, Объект.Дата), СтрокаТЧ.ОбъектРасчетов, Объект.Договор);
...
Процедура ПодборПоОстаткамЗавершениеСервер()
	...
	Для Каждого СтрокаРасшифровки Из Объект.РасшифровкаПлатежа Цикл
		Если СтрокаРасшифровки.ОбъектРасчетовСовпадаетСДокументом Тогда
			Если ЗначениеЗаполнено(Объект.Договор) Тогда
				СтрокаРасшифровки.СтавкаНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "СтавкаНДС");
			КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьВариантКурсаДоговораРасшифровки()
	СоответствиеКурсаВалютыДоговора = ВзаиморасчетыСервер.СоответствиеОбъектовРасчетовИКурсаВалютыДоговора(
