...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Основание", ЭтотОбъект.Заявка);
	Параметры.Свойство("БанковскийСчет", ЭтотОбъект.БанковскийСчет);
	
	ЭтотОбъект.Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭтотОбъект.Заявка, "Организация");
...
Процедура ЛицевыеСчетаФизическихЛицНаОплатуБанковскийСчетОрганизацииНачалоВыбора(Элемент,
	...
	Если ЭтотОбъект.РежимФормированияСписания = 1 Тогда
		ПараметрыОтбора.Вставить("Банк", СтрокаТаблицы.Банк);
	КонецЕсли;
	ПараметрыОтбора.Вставить("Владелец", ЭтотОбъект.Организация);
...
Функция ПолучитьЛицевыеСчетаПоЗаявкам()
	...
	Если ЗначениеМассивЗаявок <> Неопределено Тогда
		...
		ЗначениеМассивЗаявок.Значение = ЭтотОбъект.Заявка;
...
Функция СозданиеДокументовНаСервере()
	...
	МассивДанныхПлатежныхДокументов = ЗаполнениеДанныхПлатежныхДокументов(ЭтотОбъект.РежимФормированияСписания,
...
Функция ДанныеДляСозданияДокументов(Режим, ИсходныеДанные)
	...
	Если Режим = 1 Тогда
		...
		ЛицевыеСчета = ИсходныеДанные.Скопировать(, "ФизическоеЛицо, ЛицевойСчет, Сумма");
...
Процедура НастроитьОтображениеЭлементовФормы()
	...
	Элементы.ЛицевыеСчетаФизическихЛицНаОплату.ЧередованиеЦветовСтрок = НЕ (ЭтотОбъект.РежимФормированияСписания = 1);
