...
Процедура ЗаписатьРасчетАмортизации(ПараметрыРасчета)
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ВнеоборотныеАктивыСлужебный.УничтожитьВременныеТаблицыПослеЗаписиРасчетаАмортизации(ПараметрыРасчета.МенеджерВременныхТаблиц, ПараметрыРасчета);
...
Процедура ТекстЗапросаПараметрыРасчетаАмортизации(СписокЗапросов, ПараметрыРасчета)
	...
	#Область ОрганизацииСНезависимымРеглУчетом
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	&НачалоМесяца КАК Период,
	|	ПакетыАмортизации.Организация КАК Организация
	|ПОМЕСТИТЬ ОрганизацииСНезависимымРеглУчетом
	|ИЗ
	|	ПакетыАмортизации КАК ПакетыАмортизации
	|
	...
	|ГДЕ
	|	&ЭтоМеждународнаяВерсия
	|	И ЕСТЬNULL(УчетнаяПолитикаФинансовогоУчета.ПорядокУчетаВНАРегл, ЗНАЧЕНИЕ(Перечисление.ПорядокУчетаВНАРегл.СовпадаетСУпрУчетом)) = ЗНАЧЕНИЕ(Перечисление.ПорядокУчетаВНАРегл.Независимый)
	|	И ЕСТЬNULL(УчетнаяПолитикаФинансовогоУчета.ПорядокНачисленияАмортизацииНМАРегл, ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.СДатыПринятияКУчету)) = ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.СДатыПринятияКУчету)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|
	|	И ПараметрыАмортизацииНМАБУ.Организация В
	|			(ВЫБРАТЬ
	|				ОрганизацииСНезависимымРеглУчетом.Организация
	|			ИЗ
	|				ОрганизацииСНезависимымРеглУчетом КАК ОрганизацииСНезависимымРеглУчетом
	|			ГДЕ
	|				ОрганизацииСНезависимымРеглУчетом.Период = &НачалоМесяца)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|				ОрганизацииСНезависимымРеглУчетом КАК ОрганизацииСНезависимымРеглУчетом
	|			ГДЕ
	|				ОрганизацииСНезависимымРеглУчетом.Период = &НачалоПредыдущегоМесяца)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|				ПакетыАмортизации КАК СписокНМА)
	|
	|	И ПорядокУчетаНМАБУ.Организация В
	|			(ВЫБРАТЬ
	|				ОрганизацииСНезависимымРеглУчетом.Организация
	|			ИЗ
	|				ОрганизацииСНезависимымРеглУчетом КАК ОрганизацииСНезависимымРеглУчетом
	...
	|					ПакетыАмортизации КАК СписокНМА)
	|
	|		И СписаниеНМА2_4.Организация В
	|				(ВЫБРАТЬ
	|					ОрганизацииСНезависимымРеглУчетом.Организация
	|				ИЗ
	|					ОрганизацииСНезависимымРеглУчетом КАК ОрганизацииСНезависимымРеглУчетом
	...
	|
	|ГДЕ
	|	(ТаблицаИзменений.Организация, ТаблицаИзменений.Период) В 
	|			(ВЫБРАТЬ
	|				ОрганизацииСНезависимымРеглУчетом.Организация,
	|				ОрганизацииСНезависимымРеглУчетом.Период
	|			ИЗ
	|				ОрганизацииСНезависимымРеглУчетом КАК ОрганизацииСНезависимымРеглУчетом)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|							И НАЧАЛОПЕРИОДА(ПервоначальныеСведенияНМА.ДатаПринятияКУчетуУУ, МЕСЯЦ) = &НачалоПредыдущегоМесяца
	|						ТОГДА 0
	|					ИНАЧЕ ВЫРАЗИТЬ(ЕСТЬNULL(ВыработкаНМА.ОбъемВыработкиНаНачалоМесяца, 0) КАК ЧИСЛО(20,8))
	|				КОНЕЦ
	|
	|		КОГДА НАЧАЛОПЕРИОДА(ДатыПоследнегоИзменения.Дата, МЕСЯЦ) = СписокНМА.НачалоМесяца
...
Функция ТекстЗапросаСуммаАмортизацииУУ(ПараметрыТекстаЗапроса)
	...
	ТекстЗапроса =
	"ВЫБОР 
	|	КОГДА НЕ СписокНМА.НачислятьАмортизацию ИЛИ (&ОстаточнаяСтоимость - &СуммаДоначисления - &ОстатокОбесценения) <= 0
	|
	|		ТОГДА 0
	|
	|	КОГДА СписокНМА.АмортизацияНачисляетсяПоСтандартамМУ
