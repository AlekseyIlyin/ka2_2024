...
Процедура ПроверитьЗаполнениеНастроекОтражения(НепроверяемыеРеквизиты, Отказ)
	...
	НастройкиОтражения = Документы.ОтражениеЗарплатыВФинансовомУчете2_5.НастройкиОтраженияНачислений();
	
	// Начисления
	Для Каждого СтрокаТаблицы Из Начисления Цикл
		
		ПараметрыОтражения = НастройкиОтражения.ДоступныеВарианты[СтрокаТаблицы.ВариантОтражения];
		Если ПараметрыОтражения = Неопределено ИЛИ НЕ ПараметрыОтражения.ПроверятьЗаполнение Тогда
			Продолжить;
		КонецЕсли;
		
		ФлагПроверки = Ложь;
		Для Каждого Реквизит Из ПараметрыОтражения.Реквизиты Цикл
			СвойстваРеквизита = Реквизит.Значение; // Структура -
			Если СвойстваРеквизита.Обязательный И НЕ ЗначениеЗаполнено(СтрокаТаблицы[Реквизит.Ключ]) Тогда
				ФлагПроверки = Истина;
			КонецЕсли;
		КонецЦикла;
		...
		Если ФлагПроверки Тогда
			ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Начисления", СтрокаТаблицы.НомерСтроки, "НастройкиОтражения");
	...
	Для Каждого СтрокаТаблицы Из НалогиВзносыОрганизации Цикл
		...
		Если ФлагПроверки Тогда
			ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"НалогиВзносыОрганизации", СтрокаТаблицы.НомерСтроки, "НастройкиОтражения");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Для Каждого ВариантОтражения Из НастройкиОтражения.ДоступныеВарианты Цикл
		ПараметрыОтражения = ВариантОтражения.Значение;
		Если НЕ ПараметрыОтражения.ПроверятьЗаполнение Тогда
			Продолжить;
		КонецЕсли;
		Для Каждого Реквизит Из ПараметрыОтражения.Реквизиты Цикл
	...
	// Удержания
	НастройкиОтражения = Документы.ОтражениеЗарплатыВФинансовомУчете2_5.НастройкиОтраженияУдержаний();
	...
	Для Каждого СтрокаТаблицы Из Удержания Цикл
		...
		Если ФлагПроверки Тогда
			ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Удержания", СтрокаТаблицы.НомерСтроки, "НастройкиОтражения");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
