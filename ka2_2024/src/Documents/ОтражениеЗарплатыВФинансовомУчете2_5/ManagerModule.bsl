...
// Возвращает параметры настройки счетов учета для отдельной формы настройки.
// 
// Параметры:
//  ИмяТабличнойЧасти - Строка - имя редактируемой табличной части документа
// 
// Возвращаемое значение:
// 	Массив - Массив параметров настройки счетов учета (См. НастройкаСчетовУчетаСервер.ПараметрыНастройкиСчетаУчетаОперации).
//
Функция ПараметрыНастройкиСчетовУчетаВФормеНастройкиОтражения(ИмяТабличнойЧасти) Экспорт
	
	МассивПараметров = Новый Массив();
	
	Если ИмяТабличнойЧасти = "Начисления" ИЛИ ИмяТабличнойЧасти = "НалогиВзносыОрганизации" Тогда
		...
		ПараметрыНастройки.ИсточникиПодбораСубконто.Добавить(
			Тип("СправочникСсылка.Организации"),          "Объект.Организация");
		ПараметрыНастройки.ИсточникиПодбораСубконто.Добавить(
			Тип("СправочникСсылка.СтруктураПредприятия"), "Объект." + ИмяТабличнойЧасти + ".Подразделение");
		ПараметрыНастройки.ИсточникиПодбораСубконто.Добавить(
			Тип("СправочникСсылка.ФизическиеЛица"),   "Объект." + ИмяТабличнойЧасти + ".ФизическоеЛицо");
		
		ПараметрыНастройки.ЭлементыФормы.Добавить("НастройкаСчетовУчетаНачислений");
		МассивПараметров.Добавить(ПараметрыНастройки);
		
	ИначеЕсли ИмяТабличнойЧасти = "Удержания" Тогда
...
Функция ПараметрыВыбораСтатейИАналитик() Экспорт 
	...
	ПараметрыВыбора.ЭлементыФормы.Статья.Добавить("НачисленияНастройкиОтражения");
	ПараметрыВыбора.ЭлементыФормы.АналитикаРасходов.Добавить("НачисленияНастройкиОтражения");
	ПараметрыВыбора.ЭлементыФормы.АналитикаАктивовПассивов.Добавить("НачисленияНастройкиОтражения");
	...
	#Область Удержания_НаПрочиеДоходыПассивы
	ПараметрыВыбора = ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики();
...
// Возвращает параметры выбора статей и аналитик для отдельной формы настройки.
//
// Параметры:
//   ВариантОтражения - ПеречислениеСсылка.ВариантыОтраженияНачисленийУдержаний - вариант отражения.
// 
// Возвращаемое значение:
//   Структура - Параметры выбора статей и аналитик (См. ДоходыИРасходыСервер.ПараметрыВыбораСтатьиИАналитики).
//
Функция ПараметрыВыбораСтатейИАналитикВФормеНастройкиОтражения(ВариантОтражения) Экспорт
...
Функция НастройкиОтраженияНачислений() Экспорт
	
	Настройки = Новый Структура;
	
	Настройки.Вставить("ДоступныеВарианты", Новый Соответствие);
	
	// Отражение затрат по статье расходов
	
	Вариант = СтруктураПараметровОтражения(
		...
		"ПостатейноеОтражениеНачислений");
	
	Вариант.ПроверятьЗаполнение = Ложь;
	
	СвойстваРеквизита = Вариант.Реквизиты.СтатьяРасходов; // см. СвойстваРеквизитаОтражения
	СвойстваРеквизита.Обязательный = Истина;
	Вариант.Реквизиты.НаправлениеДеятельности.Используется =
		ПолучитьФункциональнуюОпцию("ИспользоватьУчетЗатратПоНаправлениямДеятельности");
	
	Настройки.ДоступныеВарианты.Вставить(Вариант.Ссылка, Вариант);
	...
	Вариант.Используется = ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрочихАктивовПассивов");
	Вариант.ПроверятьЗаполнение = Ложь;
	...
	Вариант = СтруктураПараметровОтражения(
		...
		"РаспределениеСдельныхНачислений");
	
	ДоступныеЗначения = Новый СписокЗначений();
	ДоступныеЗначения.Добавить(Перечисления.ТипыБазыРаспределенияРасходов.НормативыОплатыТруда,
		НСтр("ru = 'по нормативной стоимости работ'"));
	ДоступныеЗначения.Добавить(Перечисления.ТипыБазыРаспределенияРасходов.ДлительностьВыполненияРабот,
		НСтр("ru = 'по длительности работ'"));
	
	СвойстваРеквизита = Вариант.Реквизиты.ТипБазыРаспределения; // см. СвойстваРеквизитаОтражения
	СвойстваРеквизита.ДоступныеЗначения = ДоступныеЗначения;
	СвойстваРеквизита.Обязательный = Истина;
	...
	РеквизитыСтроки = Новый Структура("ВариантОтражения,
		|СтатьяРасходов, АналитикаРасходов, АналитикаАктивовПассивов, НастройкаСчетовУчета,
		|ТипБазыРаспределения, СтатьяКалькуляции, НаправлениеДеятельности");
	Настройки.Вставить("РеквизитыСтроки", РеквизитыСтроки);
	
	// 
	
	Настройки.Вставить("ПодбираемаяАналитика", ТипыАналитикиНачисленийДоступныеКЗаполнению());
	
	Возврат Настройки;
	
КонецФункции

Функция НастройкиОтраженияУдержаний() Экспорт
	...
	Вариант = СтруктураПараметровОтражения(
		...
		"ПостатейноеОтражениеУдержаний");
	
	Вариант.ПроверятьЗаполнение = Ложь;
	
	СвойстваРеквизита = Вариант.Реквизиты.СтатьяДоходов; // см. СвойстваРеквизитаОтражения
	СвойстваРеквизита.Обязательный = Истина;
	...
	Вариант = СтруктураПараметровОтражения(
		...
		"ПогашениеВзаиморасчетов");
	
	СвойстваРеквизита = Вариант.Реквизиты.Контрагент; // см. СвойстваРеквизитаОтражения
	СвойстваРеквизита.Обязательный = Истина;
	
	СвойстваРеквизита = Вариант.Реквизиты.ОбъектРасчетов; // см. СвойстваРеквизитаОтражения
	СвойстваРеквизита.Обязательный = Истина;
	...
	Вариант.Реквизиты.ЦельВыдачи.Используется =
	...
	РеквизитыСтроки = Новый Структура("ВариантОтражения,
		|СтатьяДоходов, АналитикаДоходов, АналитикаАктивовПассивов, НастройкаСчетовУчета,
		|Контрагент, ОбъектРасчетов, ЦельВыдачи, НаправлениеДеятельности");
	Настройки.Вставить("РеквизитыСтроки", РеквизитыСтроки);
	
	// 
	
	Настройки.Вставить("ПодбираемаяАналитика", ТипыАналитикиУдержанийДоступныеКЗаполнению());
...
Процедура ПодготовитьДанныеЗаполнения(Параметры, АдресРезультата) Экспорт
	
	Источники = Параметры.ИсточникиЗаполнения;
	Если Источники.Свойство("ВыработкаСотрудников") Тогда
		Источники.ВыработкаСотрудников = ПолучитьДанныеВыработки(Параметры);
	КонецЕсли;
	
	ПоместитьВоВременноеХранилище(Источники, АдресРезультата);
...
Функция СтруктураПараметровОтражения(ВариантОтражения, Подсказка, Реквизиты, Группа = "")
	
	СтруктураПараметров = Новый Структура;
	
	СтруктураПараметров.Вставить("Ссылка",              ВариантОтражения);
	СтруктураПараметров.Вставить("Подсказка",           Подсказка);
	СтруктураПараметров.Вставить("Используется",        Истина);
	СтруктураПараметров.Вставить("ПроверятьЗаполнение", Истина);
	СтруктураПараметров.Вставить("Группа",              Группа);
	
	СтруктураПараметров.Вставить("СписокРеквизитов", Реквизиты);
	СтруктураПараметров.Вставить("Реквизиты", Новый Структура);
	...
	Для Каждого Элемент Из МассивРеквизитов Цикл
		...
		СтруктураПараметров.Реквизиты.Вставить(ИмяРеквизита, СвойстваРеквизитаОтражения());
...
//    * Обязательный - Булево - признак обязательности заполнения,
//    * ДоступныеЗначения - Произвольный - доступные значения.
//
Функция СвойстваРеквизитаОтражения()
	
	СвойстваРеквизита = Новый Структура;
	СвойстваРеквизита.Вставить("Используется", Истина);
	СвойстваРеквизита.Вставить("Обязательный", Ложь);
	СвойстваРеквизита.Вставить("ДоступныеЗначения");
...
Функция ПолучитьДанныеВыработки(Параметры)
	...
	Запрос.УстановитьПараметр("ФильтрПоПодразделению", ЗначениеЗаполнено(Подразделение));
	...
	|	ТЧРаботники.Работник КАК ФизическоеЛицо,
...
Процедура ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(ТекстыЗапроса, Регистры)
	...
	|
	|	ВтНалогиВзносыОрганизации.ИдентификаторФинЗаписи КАК ИдентификаторФинЗаписи,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтражениеНалоговВзносовСЗаработнойПлаты) НастройкаХозяйственнойОперации
	|ИЗ
	|	ВтНалогиВзносыОрганизации КАК ВтНалогиВзносыОрганизации
	|ГДЕ
	|	ВтНалогиВзносыОрганизации.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияНачисленийУдержаний.ОтнестиНаПрочиеАктивыПассивы)
	...
	|	ВЫБОР
	|		КОГДА ВтУдержания.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияНачисленийУдержаний.ПогаситьРасчетыСПодотчетником)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.УдержаниеНеизрасходованныхПодотчетныхСумм)
	|		КОГДА ВтУдержания.ВариантОтражения = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияНачисленийУдержаний.ПогаситьВзаиморасчеты)
...
Процедура ТекстЗапросаТаблицаТрудозатратыНезавершенногоПроизводства(ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ОтражениеСдельнойЗаработнойПлаты) КАК НастройкаХозяйственнойОперации
	|ИЗ
...
Функция ТекстЗапросаТаблицаОплатаОтКлиента()
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.УдержаниеИзЗарплатыВСчетРеализацииСотруднику) КАК НастройкаХозяйственнойОперации
	|ИЗ
