...
Процедура ОбработатьВыбранныхСотрудников(ВыбранныеФизическиеЛица)
	...
	Если ЗначениеЗаполнено(СотрудникиЗаписи) Тогда
		КадровыеДаннные = ДанныеСотрудников(СотрудникиЗаписи);
		Для Каждого СтрокаСотрудника Из ДобавленныеСтроки Цикл
			КадровыеДаннныеСотрудника = КадровыеДаннные.Получить(СтрокаСотрудника.СотрудникЗаписи);
...
Функция ДанныеСотрудников(Сотрудники)
	...
	КадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудники, ИменаКадровыхДанных);
...
Процедура ЗаполнитьСотрудникаЗаписи()
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли Не ЗначениеЗаполнено(ТекущиеДанные.СотрудникЗаписи) Тогда
			...
			КадровыеДаннные = ДанныеСотрудников(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.СотрудникЗаписи));
			КадровыеДаннныеСотрудника = КадровыеДаннные.Получить(ТекущиеДанные.СотрудникЗаписи);
			Если КадровыеДаннныеСотрудника <> Неопределено Тогда
