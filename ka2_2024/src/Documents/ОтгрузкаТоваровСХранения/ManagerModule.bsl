...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет значение пустого реквизита ""Заказ клиента"" табличной части ""Товары"" значением ""Неопределено"".
	|Пересчитывает сумму документа по данным реквизита ""Сумма"" табличной части ""Товары"".
	|Заполняет реквизит ""Сумма"" табличной части ""Виды запасов"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			Документ.ОтгрузкаТоваровСХранения.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|		ГДЕ
	|			ТаблицаВидыЗапасов.Ссылка = ОтгрузкаТоваровСХранения.Ссылка
	|			И ТаблицаВидыЗапасов.Количество * ТаблицаВидыЗапасов.Цена <> ТаблицаВидыЗапасов.Сумма)
	|	ИЛИ ОтгрузкаТоваровСХранения.Проведен
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			И ТаблицаВидыЗапасов.Количество * ТаблицаВидыЗапасов.Цена <> ТаблицаВидыЗапасов.Сумма) КАК ПересчитатьСуммуВТЧВидыЗапасов,
	|	ОбновляемыеДанные.Ссылка.Проведен
	|	И НЕ ОбновляемыеДанные.Ссылка.СуммаДокумента В
	|										(ВЫБРАТЬ
	|											ЕСТЬNULL(СУММА(ТаблицаТовары.Сумма), 0) КАК СуммаДокумента
	|										ИЗ
	|											Документ.ОтгрузкаТоваровСХранения.Товары КАК ТаблицаТовары
	|										ГДЕ
	|											ТаблицаТовары.Ссылка = ОбновляемыеДанные.Ссылка) КАК ПересчитатьСуммуДокумента
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные";
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			Если ВыборкаДокументов.ПересчитатьСуммуВТЧВидыЗапасов Тогда
				
				ОбъектИзменен = Истина;
				
				ТабличнаяЧастьВидыЗапасов = ДанныеОбъекта.ВидыЗапасов;
				Для каждого СтрокаТЧВидыЗапасов Из ТабличнаяЧастьВидыЗапасов Цикл
					СтрокаТЧВидыЗапасов.Сумма = СтрокаТЧВидыЗапасов.Количество * СтрокаТЧВидыЗапасов.Цена;
				КонецЦикла;
				
			КонецЕсли;
			
			Если ВыборкаДокументов.ПересчитатьСуммуДокумента Тогда
				ОбъектИзменен = Истина;
				ДанныеОбъекта.СуммаДокумента = ДанныеОбъекта.Товары.Итог("Сумма");
			КонецЕсли;
