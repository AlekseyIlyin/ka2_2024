...
Функция ВыбратьЗаблокированныеДляЧтенияИИзмененияСсылки(СписокИменТаблиц, СписокСсылок = Неопределено) Экспорт
	...
	Для каждого ИмяТаблицы Из СписокИменТаблиц Цикл
		...
		Иначе
			...
			ТекстИсключения = НСтр("ru = 'В функции ""%1"" не реализовано получение заблокированных распоряжений для таблицы ""%2"".'");
			ВызватьИсключение СтрШаблон(ТекстИсключения, "ВыбратьЗаблокированныеДляЧтенияИИзмененияСсылки()", ИмяТаблицы);
	...
	ИтоговыйТекстЗапроса = СтрЗаменить(ИтоговыйТекстЗапроса, "ТекстВложенногоЗапроса", "(" + ТекстЗапроса + ")");
...
Процедура ОбработкаПолученияПолейПредставления(Поля, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоПередачаВПереработку2_4") Тогда
...
Функция ПараметрыВыполненияКомандУстановкиДействий(ВсеЗаказы, КодДействия) Экспорт
	...
	Результат = Новый Структура("Заказы,ПараметрыВстраивания",
		...
		Документы.ЗаказПереработчику.ДоступныеОстаткиПараметрыВстраивания());
			...
			Если Заказы.Количество() > 0 Тогда
				...
				Иначе
					...
					Текст =
						...
						|			И ЗаписиЗаказа.ЗаказНаОтгрузку = &ИмяПоляЗаказ)";
...
Функция ТекстЗапросаДокументыДляЗаменыКОбеспечениюНаТребуется() Экспорт
	...
	ТекстЗапроса =
		...
		|ГДЕ
		|	Движения.Активность
		|		И Движения.Регистратор ССЫЛКА Документ.ЗаказПереработчику
		...
		|				ИСТИНА
		|			ИЗ
		|				Документ.ЗаказПереработчику.РаспоряжениеСпецификация КАК Товары
		|			ГДЕ
		|				Товары.Ссылка = Движения.Регистратор
		|					И Товары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.РезервироватьПоМереПоступления)
		|					И Товары.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))";
