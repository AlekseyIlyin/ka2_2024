...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("d04dd3ce-23c5-407a-b3fc-55526995d774");
	...
	Обработчик.Комментарий = НСтр("ru = 'Переносит значение поля ""Цена"" таблицы ""Товары"" в поле ""Цена (забаланс)"" для товаров, принятых на ответственное хранение.
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.ПересортицаТоваровУХранителя.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|			ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ОбновляемыеДанные.Ссылка КАК Документ.ПересортицаТоваровУХранителя) КАК Ссылка
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(Выборка.Ссылка, Выборка.ВерсияДанных, Параметры.Очередь); // ДокументОбъект.ПересортицаТоваровУХранителя
...
Процедура ПеренестиЦены(Объект)
	...
	Для Каждого СтрокаВидыЗапасов Из Объект.ВидыЗапасов Цикл
		...
		Если Не ТипыЗапасов.Получить(СтрокаВидыЗапасов.ВидЗапасов) = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи Тогда
			Продолжить;
		КонецЕсли;
