...
//
// Параметры:
//	ОтчетПереработчика      - ДокументСсылка.ОтчетПереработчика2_5 - Документ, состояние которого необходимо вычислить
...
Процедура РассчитатьСостояние(Знач ОтчетПереработчика, Знач Договор, СостояниеРасчетов) Экспорт
	...
	Если ЗначениеЗаполнено(ОтчетПереработчика) И ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСПоставщиками) Тогда
		...
		ДанныеРасчетаПараметров.Вставить("ПоЗаказам", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтчетПереработчика, "ПоЗаказам"));
		ДанныеРасчетаПараметров.Вставить("Ссылка",    ОтчетПереработчика);
		
		ДанныеПоСостоянию = ВзаиморасчетыСервер.СостояниеВзаиморасчетов(ОтчетПереработчика, ПараметрыВзаиморасчеты(ДанныеРасчетаПараметров));
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	ТаблицаТовары.ПартияПроизводства                                КАК КорПартия,
	...
	|	ТаблицаТовары.АналитикаУчетаПартий                              КАК КорАналитикаУчетаПартий,
	|	ВЫБОР
	|		КОГДА &ЗакупкаПодДеятельность = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|			ТОГДА 0
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("ТоварыКОформлениюДокументовИмпорта", Регистры) Тогда
		...
		УчетНДСУП.ОтразитьВвозТоваровИзТаможенногоСоюзаНоваяМеханика(Запрос, ТекстыЗапроса, Регистры, ТекстТовары);
...
Функция ТекстЗапросаПобочныеИзделияКОформлению()
	...
	|			ТОГДА ТоварыКОформлению.КОформлению
	|					* Нормативы.Коэффициент
...
Функция ТекстЗапросаМатериалыИРаботыКОформлению()
	...
	|	ТабличнаяЧасть.СтатьяКалькуляции   КАК СтатьяКалькуляции,
	|	СУММА(ТабличнаяЧасть.Количество
	|			* Нормативы.Коэффициент)   КАК Количество
	|ИЗ
	|	МатериалыИРаботы КАК ТабличнаяЧасть
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Нормативы КАК Нормативы
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.11.19";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("43766438-f148-4795-b431-06091aa2a40f");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение настроек счетов учета в видах запасов изделий отчетов переработчика.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5.ВыходныеИзделия КАК Изделия
	|		ПО Изделия.Ссылка = ВидыЗапасов.Ссылка
	|		И Изделия.ИдентификаторСтроки = ВидыЗапасов.ИдентификаторСтроки
	|	ГДЕ
	|		ВидыЗапасов.СписатьНаРасходы
	...
	|		И ВидыЗапасов.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	|		И НЕ ВидыЗапасов.ИдентификаторСтроки = """"
	|		И НЕ Изделия.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ВидыЗапасов
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5.ПобочныеИзделия КАК Изделия
	|		ПО Изделия.Ссылка = ВидыЗапасов.Ссылка
	|		И Изделия.ИдентификаторСтроки = ВидыЗапасов.ИдентификаторСтроки
	|	ГДЕ
	|		ВидыЗапасов.СписатьНаРасходы
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ОбновляемыеДанные.Ссылка КАК Документ.ОтчетПереработчика2_5) КАК Ссылка
	...
	|	ОбновляемыеДанные.ИдентификаторСтроки  КАК ИдентификаторСтроки,
	|	ОбновляемыеДанные.НастройкаСчетовУчета КАК НастройкаСчетовУчета
	|ПОМЕСТИТЬ НастройкиСчетовУчета
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОбновляемыеДанные.Ссылка     КАК Ссылка,
	|		Изделия.ИдентификаторСтроки  КАК ИдентификаторСтроки,
	|		Изделия.НастройкаСчетовУчета КАК НастройкаСчетовУчета
	|	ИЗ
	|		ОбновляемыеДанные КАК ОбновляемыеДанные
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ВидыЗапасов
	|		ПО ВидыЗапасов.Ссылка = ОбновляемыеДанные.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика2_5.ВыходныеИзделия КАК Изделия
	|		ПО Изделия.Ссылка = ВидыЗапасов.Ссылка
	|		И Изделия.ИдентификаторСтроки = ВидыЗапасов.ИдентификаторСтроки
	...
	|	ОбновляемыеДанные.ИдентификаторСтроки,
	|	ОбновляемыеДанные.НастройкаСчетовУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ЗаполнитьНастройкиСчетовУчета
	|ИЗ
	|	НастройкиСчетовУчета КАК ОбновляемыеДанные
	|
	|СГРУППИРОВАТЬ ПО
	|	ОбновляемыеДанные.Ссылка,
	...
	|	ОбновляемыеДанные.Ссылка                                КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ЗаполнитьНастройкиСчетовУчета.ВерсияДанных ЕСТЬ НЕ NULL
	|			ТОГДА ЗаполнитьНастройкиСчетовУчета.ВерсияДанных
	|		ИНАЧЕ ОбновляемыеДанные.Ссылка.ВерсияДанных
	|	КОНЕЦ                                                   КАК ВерсияДанных,
	|	ЗаполнитьНастройкиСчетовУчета.ВерсияДанных ЕСТЬ НЕ NULL КАК ЗаполнитьНастройкиСчетовУчета
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьНастройкиСчетовУчета КАК ЗаполнитьНастройкиСчетовУчета
	|	ПО ЗаполнитьНастройкиСчетовУчета.Ссылка = ОбновляемыеДанные.Ссылка
	|";
	Запрос.УстановитьПараметр("ОбновляемыеДанные", ОбновляемыеДанные);
	
	Результат = Запрос.ВыполнитьПакет();
	ТаблицаТовары = Результат[Результат.ВГраница() - 1].Выгрузить();
	ТаблицаТовары.Индексы.Добавить("Ссылка, ИдентификаторСтроки");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(Выборка.Ссылка, Выборка.ВерсияДанных, Параметры.Очередь); // ДокументОбъект.ОтчетПереработчика2_5
			Если ДокументОбъект = Неопределено Тогда
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			Если Выборка.ЗаполнитьНастройкиСчетовУчета Тогда
				ЗапонитьНастройкиСчетовУчета(ДокументОбъект, ТаблицаТовары);
			КонецЕсли;
...
Процедура ЗапонитьНастройкиСчетовУчета(Объект, ТаблицаТовары)
	
	Отбор = Новый Структура("Ссылка, ИдентификаторСтроки");
	Для Каждого ТекущиеДанные Из Объект.ВидыЗапасовИзделия Цикл
		
		Если ТекущиеДанные.СписатьНаРасходы
			И ЗначениеЗаполнено(ТекущиеДанные.СтатьяРасходов)
			И ТипЗнч(ТекущиеДанные.СтатьяРасходов) = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов")
			И Не ЗначениеЗаполнено(ТекущиеДанные.НастройкаСчетовУчета) Тогда
			
			Отбор.Ссылка = Объект.Ссылка;
			Отбор.ИдентификаторСтроки = ТекущиеДанные.ИдентификаторСтроки;
			
			НайденныеСтроки = ТаблицаТовары.НайтиСтроки(Отбор);
			Если Не НайденныеСтроки.Количество() = 0 Тогда
				ТекущиеДанные.НастройкаСчетовУчета = НайденныеСтроки[0].НастройкаСчетовУчета;
			КонецЕсли;
