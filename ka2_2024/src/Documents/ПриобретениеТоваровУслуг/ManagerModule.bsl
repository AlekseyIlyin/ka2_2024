...
Процедура РассчитатьСостояние(Знач ПриобретениеТоваровУслуг, Знач ЕстьКорректировки, СостояниеРасчетов) Экспорт
	...
	Если ЗначениеЗаполнено(ПриобретениеТоваровУслуг) И ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСПоставщиками) Тогда
		...
		МассивПараметров =  ПараметрыВзаиморасчеты(ДанныеРасчетаПараметров);
		Для Каждого СтруктураПараметров Из МассивПараметров Цикл
			...
			ДанныеПоСостоянию = ВзаиморасчетыСервер.СостояниеВзаиморасчетов(ПриобретениеТоваровУслуг, СтруктураПараметров);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.ДатаКурсаВалютыДокумента КАК ДатаКурсаВалютыДокумента,
	|	ВЫБОР
	...
	Запрос.УстановитьПараметр("ДатаКурсаВалютыДокумента", Реквизиты.ДатаКурсаВалютыДокумента);
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	ТаблицаТовары.ИдентификаторФинЗаписи
	|
	// Неотфактурованные поставки (отрицательное количество)
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	&Период КАК Период,
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено) Экспорт
	...
	Иначе
		...
		|	ВЫБОР КОГДА ТаблицаТовары.Ссылка.ДатаКурсаВалютыДокумента <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА ТаблицаТовары.Ссылка.ДатаКурсаВалютыДокумента
	...
	|						ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)))
		...
		|	ВЫБОР КОГДА ВидыЗапасов.Ссылка.ДатаКурсаВалютыДокумента <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА ВидыЗапасов.Ссылка.ДатаКурсаВалютыДокумента
		|		ИНАЧЕ ВидыЗапасов.Ссылка.Дата 
		|	КОНЕЦ КАК ПериодБазыНДС,
		|	ВЫБОР КОГДА ВидыЗапасов.Ссылка.ДатаКурсаВалютыДокумента <> ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА ВидыЗапасов.Ссылка.ДатаКурсаВалютыДокумента
		|		ИНАЧЕ ВидыЗапасов.Ссылка.Дата 
...
Функция ТекстЗапросаТаблицаВтСуммыДокументовВВалютахУчета(Запрос, ТекстыЗапроса)
	...
	|	(ТаблицаДокумента.СуммаСНДС - ТаблицаДокумента.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаБезНДСРегл,
	|	ТаблицаДокумента.СуммаНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаНДСРегл,
	|	ТаблицаДокумента.СуммаСНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаСНДСРегл,
	|	НЕОПРЕДЕЛЕНО КАК ТипРасчетов,
	|	(ТаблицаДокумента.СуммаСНДС - ТаблицаДокумента.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаБезНДСУпр,
	|	ТаблицаДокумента.СуммаНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаНДСУпр,
	|	ТаблицаДокумента.СуммаСНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаСНДСУпр,
	|	
	|	ТаблицаДокумента.СуммаВзаиморасчетов - ТаблицаДокумента.СуммаНДСВзаиморасчетов КАК СуммаПриведенная,
	|	(ТаблицаДокумента.СуммаСНДС - ТаблицаДокумента.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаПриведеннаяРегл,
	|	(ТаблицаДокумента.СуммаСНДС - ТаблицаДокумента.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаПриведеннаяУпр,
	|	
	|	ЛОЖЬ КАК ОтражаетсяВРасчетах,
	|	ТаблицаДокумента.СуммаСНДС КАК СуммаСНДС,
	|	&Период КАК ПериодБазыНДС,
	|	НЕОПРЕДЕЛЕНО КАК ОбъектРасчетов,
	|	(ТаблицаДокумента.СуммаСНДС - ТаблицаДокумента.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК БазаНДСРегл,
...
Функция ТекстЗапросаВТКурсыВалютСтрокОтчета(Запрос, ТекстыЗапроса)
	...
	|	СУММА(ДенежныеСредства.СуммаУпрОстаток) / СУММА(ДенежныеСредства.СуммаОстаток)КАК КоэффициентПересчетаУпр,
	|	СУММА(ДенежныеСредства.СуммаРеглОстаток) / СУММА(ДенежныеСредства.СуммаОстаток)КАК КоэффициентПересчетаРегл,
	...
	|	Товары.Ссылка.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	...
	|	СУММА(РазделеннаяСтрокаРасхода.СуммаУпр) / СУММА(РазделеннаяСтрокаРасхода.Сумма) КАК КоэффициентПересчетаУпр,
	|	СУММА(РазделеннаяСтрокаРасхода.СуммаРегл) / СУММА(РазделеннаяСтрокаРасхода.Сумма) КАК КоэффициентПересчетаРегл
	|ИЗ
	|	(ВЫБРАТЬ
	|		СтрокиОтчета.ИдентификаторСтроки КАК ИдентификаторСтроки,
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо))
	|	И (НЕ &ИспользуетсяДокументПоступлениеТоваров
	|		ИЛИ Товары.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры В (
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга),
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)))
	|	И (ВЫБОР
...
Процедура ДобавитьТекстыОтраженияВзаиморасчетов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстОплата = "
		...
		|	ВЫБОР КОГДА НЕ (Таблица.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
		|					И ЕСТЬNULL(Таблица.Договор.ЗаданГрафикИсполнения, ЛОЖЬ))
		...
		|ГДЕ
		|	Таблица.Ссылка В (&Ссылка)
		|	И Таблица.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет)
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	#Область ВнешнееПоступление_НефактурованныеПоставки_РаботУслуг
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.ПриобретениеТоваровУслуг.ПереходНаНовуюМеханикуСозданияЗаявленийОВвозе_ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.11.18";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("e9b7bc66-49fc-4636-9654-5dece6580658");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ПриобретениеТоваровУслуг.ПереходНаНовуюМеханикуСозданияЗаявленийОВвозе_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает флаг ""Новая механика создания Заявлений о ввозе товаров"" у документов ""Приобретение товаров и услуг"", 
										|по которым есть остатки в РН ""Товары к оформлению Заявлений о ввозе"", 										
										|формирует по этим поступлениям приходные движения в РН ""Товары к оформлению документов импорта""'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Документы.ПриобретениеТоваровУслуг.ПолноеИмя());
	Читаемые.Добавить(Метаданные.РегистрыНакопления.ТоварыКОформлениюЗаявленийОВвозе.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Документы.ПриобретениеТоваровУслуг.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыНакопления.ТоварыКОформлениюЗаявленийОВвозе.ПолноеИмя());
...
//
Процедура ПереходНаНовуюМеханикуСозданияЗаявленийОВвозе_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	ПараметрыВыборки = Параметры.ПараметрыВыборки;
	ПараметрыВыборки.ПолныеИменаОбъектов = Метаданные.Документы.ПриобретениеТоваровУслуг.ПолноеИмя();
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТоварыКОформлениюЗаявленийОВвозеОстатки.ДокументПоступления КАК Ссылка
	|ИЗ
	|	РегистрНакопления.ТоварыКОформлениюЗаявленийОВвозе.Остатки КАК ТоварыКОформлениюЗаявленийОВвозеОстатки
	|ГДЕ
	|	(ТоварыКОформлениюЗаявленийОВвозеОстатки.КОформлениюОстаток > 0
	|			ИЛИ ТоварыКОформлениюЗаявленийОВвозеОстатки.ФактурнаяСтоимостьОстаток > 0)";
	
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
	
КонецПроцедуры

Процедура ПереходНаНовуюМеханикуСозданияЗаявленийОВвозе_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	ПолноеИмяОбъекта = Метаданные.Документы.ПриобретениеТоваровУслуг.ПолноеИмя();
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка   
			...
			ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.ТоварыКОформлениюДокументовИмпорта");
			ЭлементБлокировки.УстановитьЗначение("ДокументПоступления", Выборка.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			
			ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.ТоварыКОформлениюЗаявленийОВвозе");
			ЭлементБлокировки.УстановитьЗначение("ДокументПоступления", Выборка.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ДокументОбъект.НоваяМеханикаСозданияЗаявленийОВвозе = Истина;
			
				ОбновлениеИнформационнойБазы.ЗаписатьДанные(ДокументОбъект);
		
				// Получение первичных движений.
				ТаблицыДляДвижений = ПроведениеДокументов.ДанныеДокументаДляПроведения(Выборка.Ссылка, "ТоварыКОформлениюДокументовИмпорта, ТоварыКОформлениюЗаявленийОВвозе");
		
				НаборЗаписейТоварыКОформлениюДокументовИмпорта = РегистрыНакопления.ТоварыКОформлениюДокументовИмпорта.СоздатьНаборЗаписей();
				НаборЗаписейТоварыКОформлениюДокументовИмпорта.Отбор.Регистратор.Установить(Выборка.Ссылка);
				
				Если ТаблицыДляДвижений.Свойство("ТаблицаТоварыКОформлениюДокументовИмпорта") Тогда 
					ТаблицаДанныхТоварыКОформлениюДокументовИмпорта = ТаблицыДляДвижений["ТаблицаТоварыКОформлениюДокументовИмпорта"];
					НаборЗаписейТоварыКОформлениюДокументовИмпорта.Загрузить(ТаблицаДанныхТоварыКОформлениюДокументовИмпорта);
				КонецЕсли;  
			
				ОбновлениеИнформационнойБазы.ЗаписатьДанные(НаборЗаписейТоварыКОформлениюДокументовИмпорта);
				
				НаборЗаписейТоварыКОформлениюЗаявленийОВвозе = РегистрыНакопления.ТоварыКОформлениюЗаявленийОВвозе.СоздатьНаборЗаписей();
				НаборЗаписейТоварыКОформлениюЗаявленийОВвозе.Отбор.Регистратор.Установить(Выборка.Ссылка);
			
				Если ТаблицыДляДвижений.Свойство("ТаблицаТоварыКОформлениюЗаявленийОВвозе") Тогда 
					ТаблицаДанныхТоварыКОформлениюЗаявленийОВвозе = ТаблицыДляДвижений["ТаблицаТоварыКОформлениюЗаявленийОВвозе"];
					НаборЗаписейТоварыКОформлениюЗаявленийОВвозе.Загрузить(ТаблицаДанныхТоварыКОформлениюЗаявленийОВвозе);
				КонецЕсли;
			
				ОбновлениеИнформационнойБазы.ЗаписатьДанные(НаборЗаписейТоварыКОформлениюЗаявленийОВвозе);
