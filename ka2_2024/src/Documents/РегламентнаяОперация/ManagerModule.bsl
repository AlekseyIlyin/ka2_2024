...
Процедура Использование_РасчетКурсовыхРазниц(ПараметрыОбработчика) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам.СрезПоследних(&КонецПериода, ) КАК КурсыВалютРасчетовПоДоговорамСрезПоследних
	...
	ВыборкаОрганизация = РезультатЗапроса[11].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Процедура РасчетКурсовыхРазниц(ПараметрыРасчета, Отказ) Экспорт
	...
	Иначе
		НастройкиУП = НастройкиНалоговУчетныхПолитикПовтИсп.ДействующиеПараметрыНалоговУчетныхПолитик("НастройкиУчетаНалогаНаПрибыль", ПараметрыРасчета.Организация, Период);
