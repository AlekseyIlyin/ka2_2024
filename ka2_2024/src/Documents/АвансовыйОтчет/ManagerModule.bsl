...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	Если ИмяРегистра = "РеестрДокументов" Тогда
		...
		//++ НЕ УТ
		ТекстыЗапросаВременныхТаблиц.Вставить(
			"ПоступлениеДенежныхДокументовРасходовано",
			АвансовыйОтчетЛокализация.ТекстЗапросаВременнаяТаблицаИзрасходованоПоступлениеДенежныхДокументов(Запрос, ТекстыЗапроса));
		//-- НЕ УТ
...
Функция ТекстЗапросаВТКурсыВалютСтрокОтчета(Запрос, ТекстыЗапроса)
	...
	|	ДенежныеСредства.СуммаУпрОстаток / ДенежныеСредства.СуммаОстаток КАК КоэффициентПересчетаУпр,
	|	ДенежныеСредства.СуммаРеглОстаток / ДенежныеСредства.СуммаОстаток КАК КоэффициентПересчетаРегл,
	...
	|ВЫБРАТЬ
	|	СтрокиОтчета.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЕСТЬNULL(ТаблицаКурсыВалютУпр.КоэффициентПересчета, 0) КАК КоэффициентПересчетаУпр,
	|	ЕСТЬNULL(ТаблицаКурсыВалютРегл.КоэффициентПересчета, 0) КАК КоэффициентПересчетаРегл
	|ИЗ
	|	СтрокиОтчета КАК СтрокиОтчета
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютАвансов КАК КурсыВалютАвансов
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|
	|	ТаблицаРасходов.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаСНДС,
	|
	|	(ТаблицаРасходов.СуммаСНДС - ТаблицаРасходов.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаБезНДС,
	|	(ТаблицаРасходов.СуммаСНДС - ТаблицаРасходов.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаБезНДСУпр,
	|
	|	ВЫБОР КОГДА НЕ ТаблицаРасходов.ПредъявленСФ И НЕ ТаблицаРасходов.БланкСтрогойОтчетности
	|		И НастройкиСистемыНалогообложения.СистемаНалогообложения <> ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная) ТОГДА
	|		(ТаблицаРасходов.СуммаСНДС - ТаблицаРасходов.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл
	|	ИНАЧЕ
	|		ТаблицаРасходов.СуммаСНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл
	|	КОНЕЦ КАК СуммаСНДСРегл,
	|	
	|	(ТаблицаРасходов.СуммаСНДС - ТаблицаРасходов.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаБезНДСРегл,
...
Функция ТекстЗапросаТаблицаВтИсходныеДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса)
	...
	|
	|	ПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаРегл,
	|	(ПрочиеРасходы.Сумма - ПрочиеРасходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл КАК СуммаБезНДСРегл,
	|	ПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаУпр,
	|	(ПрочиеРасходы.Сумма - ПрочиеРасходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаБезНДСУпр,
	|	ПрочиеРасходы.СуммаСНДС * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаСНДС,
	|	(ПрочиеРасходы.СуммаСНДС - ПрочиеРасходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр КАК СуммаБезНДС,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|
	|	ПрочиеРасходы.ИдентификаторСтроки КАК ИдентификаторСтроки,
...
Функция ТекстЗапросаТаблицаВтИсходныеПартииПрочихРасходов(Запрос, ТекстыЗапроса)
	...
	|	НЕОПРЕДЕЛЕНО                                КАК ВидДеятельностиНДС,
	|
	|	ПрочиеРасходы.Сумма * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр Стоимость,
	|	(ПрочиеРасходы.СуммаСНДС - ПрочиеРасходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаУпр СтоимостьБезНДС,
	|	0 КАК НДСУпр,
	|	(ПрочиеРасходы.СуммаСНДС - ПрочиеРасходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл СтоимостьРегл,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|	0 КАК НДСРегл,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПрочиеРасходы) КАК ХозяйственнаяОперация,
...
Функция ТекстЗапросаТаблицаПартииПрочихРасходов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	0 КАК НДСУпр,
	|
	|	(Расходы.СуммаСНДС - Расходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл СтоимостьРегл,
	|
	|	ВЫБОР КОГДА НЕ СтатьиСтроительства.ПринятиеКналоговомуУчету ТОГДА
	|		(Расходы.СуммаСНДС - Расходы.СуммаНДС) * ТаблицаКурсыВалютСтрокОтчета.КоэффициентПересчетаРегл
	|	ИНАЧЕ
	|		0
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ПоступлениеДенежныхДокументовРасходовано", ТекстыЗапроса) Тогда
		АвансовыйОтчетЛокализация.ТекстЗапросаВременнаяТаблицаИзрасходованоПоступлениеДенежныхДокументов(Запрос, ТекстыЗапроса);
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	+ ЕСТЬNULL(ПоступлениеДенежныхДокументовИзрасходовано.Сумма, 0)
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоступлениеДенежныхДокументовРасходовано КАК ПоступлениеДенежныхДокументовИзрасходовано
	|		ПО ДанныеДокумента.Ссылка = ПоступлениеДенежныхДокументовИзрасходовано.Ссылка
	//-- НЕ УТ
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИЛИ НастройкиСистемыНалогообложения.СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.Упрощенная) ТОГДА
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.11.38";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("05b7c68b-1d6b-4d35-b0c8-0d0433fefe29");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение в документе ""Авансовый отчет"" нового реквизита ""Дата утверждения"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазыУТ.ЗаменитьЗначениеРеквизитаЗарегистрироватьДанныеКОбработке(
		Параметры, ПараметрыВыборки.ПолныеИменаОбъектов, "ДатаУтверждения", Дата("00010101"));
	...
	Для каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				Если Не ЗначениеЗаполнено(ДокументОбъект.ДатаУтверждения) И ЗначениеЗаполнено(ДокументОбъект.Дата) Тогда
					ДокументОбъект.ДатаУтверждения = ДокументОбъект.Дата;
