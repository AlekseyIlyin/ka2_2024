...
Процедура Использование_ПереоценкаАренды(ПараметрыОбработчика) Экспорт
	...
	РегистрыДляРасчета = РегистрыРасчета(Перечисления.ХозяйственныеОперации.ПереоценкаФинансовыхИнструментов);
...
Процедура Выполнить_ПереоценкаАренды(ПараметрыОбработчика) Экспорт
	...
	Пока НачалоРасчета <= ПараметрыРасчета.КонецПериода ИЛИ ТребуетсяПереоценка Цикл
		...
		Попытка
			РегистрыДляРасчета = РегистрыРасчета(Перечисления.ХозяйственныеОперации.ПереоценкаФинансовыхИнструментов);
			ОрганизацииКПереоценке = ТребуетсяПереоценка(ПараметрыРасчета.МассивОрганизаций, ОкончаниеПериода,
		...
		СледующийМесяц = СледующийМесяцКРасчетуАренды(НачалоРасчета, ПараметрыРасчета.МассивОрганизаций);
...
Функция ТекстЗапросаКурсовыеРазницыРасчетыПоФинансовымИнструментам()
	...
	//++ НЕ УТ
	|				И НЕ ОстаткиНаДень.УчитыватьПредметыАрендыВРеглУчете
	//-- НЕ УТ 
	|			ТОГДА 0
	|		ИНАЧЕ
	...
	|		КОГДА ОстаткиНаДень.ХарактерДоговора = &ХарактерДоговораАренды
	//++ НЕ УТ 
	|				И НЕ ОстаткиНаДень.УчитыватьПредметыАрендыВУпрУчете
	//-- НЕ УТ 
	|			ТОГДА 0
	|		ИНАЧЕ
	...
	//++ НЕ УТ
	|		Остатки.Договор.УчитыватьПредметыАрендыВРеглУчете КАК УчитыватьПредметыАрендыВРеглУчете,
	|		Остатки.Договор.УчитыватьПредметыАрендыВУпрУчете КАК УчитыватьПредметыАрендыВУпрУчете,
	//-- НЕ УТ
...
Процедура ПровестиДокументПереоценки(Организация, Период, Операция, ПараметрыПереоценки = Неопределено)
	...
	Попытка
		...
		//++ Локализация
		Если Операция = Перечисления.ХозяйственныеОперации.ПереоценкаФинансовыхИнструментов
...
Функция ЗаписатьКурсовыеРазницыПоФинИнструментамНУ(ДокументСсылка, МенеджерТаблиц, День, ДополнительныеПараметры)
	...
	Если НаборЗаписейНУ.Количество() > 0 Тогда
		Если НаборЗаписейНУ.Количество() > 1 Тогда
			Таблица = НаборЗаписейНУ.Выгрузить();
			Таблица.Свернуть("ВидДвижения,Период,АналитикаУчетаПоПартнерам,Договор,ТипСуммы,РасчетныйДокумент,Валюта,ДокументРегистратор","СуммаНУ,ОтложенныйДоход,ОтложенныйРасход");
			НаборЗаписейНУ.Загрузить(Таблица);
		КонецЕсли;
...
Функция ТребуетсяПереоценка(Организация, Период, РегистрыДляРасчета = Неопределено, АналитикаРасчета = Неопределено, ВернутьОрганизацииКПереоценке = Ложь, ДляОтладки = Неопределено) Экспорт
	...
	Если РегистрыДляРасчета.Свойство("РасчетыСПоставщикамиПоСрокам") Тогда
		...
		|	Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл КАК ОстатокПоКурсуРегл,
		//++ НЕ УТ
		...
		|
		|ГДЕ
		|	Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.ДолгУпрОстаток >= 0.01
		|	ИЛИ Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.ДолгУпрОстаток <= -0.01
		|	ИЛИ Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - Остатки.ДолгРеглОстаток >= 0.01
		|	ИЛИ Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - Остатки.ДолгРеглОстаток <= -0.01
		|	ИЛИ (Остатки.ПредоплатаОстаток = 0
		|		И (Остатки.ПредоплатаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - Остатки.ПредоплатаРеглОстаток >= 0.01
		|			ИЛИ Остатки.ПредоплатаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - Остатки.ПредоплатаРеглОстаток <= -0.01
		|			ИЛИ Остатки.ПредоплатаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.ПредоплатаУпрОстаток >= 0.01
		|			ИЛИ Остатки.ПредоплатаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.ПредоплатаУпрОстаток <= -0.01
		|			)
		|		)
		//++ НЕ УТ
		...
		|			И (Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - ОстаткиНУ.СуммаНУОстаток >= 0.01
		|				ИЛИ Остатки.ДолгОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - ОстаткиНУ.СуммаНУОстаток <= -0.01)
		|		)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	0 КАК КурсРазнУпр,
		|	0 КАК КурсРазнРегл,
		|	Остатки.ДолгОстаток КАК ОстатокПоКурсуРегл,
		|	ОстаткиНУ.СуммаНУОстаток КАК ОстатокНУ,
	...
	Если РегистрыДляРасчета.Свойство("ДенежныеСредстваУПодотчетныхЛиц") Тогда
		...
		|	И (Остатки.СуммаОстаток * КурсРегл.КурсЧислитель / КурсРегл.КурсЗнаменатель * КурсУпр.КурсЗнаменатель / КурсУпр.КурсЧислитель - Остатки.СуммаУпрОстаток >= 0.01
		|	ИЛИ Остатки.СуммаОстаток * КурсРегл.КурсЧислитель / КурсРегл.КурсЗнаменатель * КурсУпр.КурсЗнаменатель / КурсУпр.КурсЧислитель - Остатки.СуммаУпрОстаток <= -0.01
		|	ИЛИ Остатки.СуммаОстаток * КурсРегл.КурсЧислитель / КурсРегл.КурсЗнаменатель - Остатки.СуммаРеглОстаток >= 0.01
		|	ИЛИ Остатки.СуммаОстаток * КурсРегл.КурсЧислитель / КурсРегл.КурсЗнаменатель - Остатки.СуммаРеглОстаток <= -0.01)";
	КонецЕсли;
	...
	Если РегистрыДляРасчета.Свойство("РасчетыПоФинансовымИнструментам") Тогда
		...
		|			И (Остатки.СуммаОстаток * КурсРегл.КурсЧислитель / КурсРегл.КурсЗнаменатель - ОстаткиНУ.СуммаНУОстаток >= 0.01
		|				ИЛИ Остатки.СуммаОстаток * КурсРегл.КурсЧислитель / КурсРегл.КурсЗнаменатель - ОстаткиНУ.СуммаНУОстаток <= -0.01)
		|		)
		//-- Локализация
		...
		|	0 КАК КурсРазнУпр,
		|	0 КАК КурсРазнРегл,
		|	0 КАК ОстатокПоКурсуРегл,
		|	ОстаткиНУ.СуммаНУОстаток КАК ОстатокНУ,
		|	-ОстаткиНУ.СуммаНУОстаток КАК КурсРазнНУ,
		|	ОстаткиНУ.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
		|	Аналитика.Организация КАК Организация,
		|	ОстаткиНУ.Договор КАК ДенежныеСредства,
		|	ОстаткиНУ.ТипСуммы КАК РасчетныйДокумент,
		|	НЕОПРЕДЕЛЕНО КАК ДатаПлановогоПогашения,
		|	НЕОПРЕДЕЛЕНО КАК ДатаВозникновения
		|ИЗ
		|	 РегистрНакопления.РасчетыПоФинансовымИнструментамНУ.Остатки(
	...
	Если РегистрыДляРасчета.Свойство("РезервыПоСомнительнымДолгам") Тогда
		...
		|	Остатки.СуммаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл КАК ОстатокПоКурсуРегл,
		|	0                                 КАК ОстатокНУ,
		|	0                                 КАК КурсРазнНУ,
		|	Остатки.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
		...
		|
		|ГДЕ
		|	Остатки.СуммаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.СуммаУпрОстаток >= 0.01
		|	ИЛИ Остатки.СуммаОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.СуммаУпрОстаток <= -0.01
		|";
	...
	Если РегистрыДляРасчета.Свойство("ПроцентныеРасходыДисконтирования") Тогда
		...
		|	Остатки.СуммаДисконтированияОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл КАК ОстатокПоКурсуРегл,
		//++ НЕ УТ
		...
		|
		|ГДЕ
		|	Остатки.СуммаДисконтированияОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.СуммаДисконтированияУпрОстаток >= 0.01
		|	ИЛИ Остатки.СуммаДисконтированияОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл * Остатки.КурсЗнаменательУпр / Остатки.КурсЧислительУпр - Остатки.СуммаДисконтированияУпрОстаток <= -0.01
		|	ИЛИ Остатки.СуммаДисконтированияОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - Остатки.СуммаДисконтированияРеглОстаток >= 0.01
		|	ИЛИ Остатки.СуммаДисконтированияОстаток * Остатки.КурсЧислительРегл / Остатки.КурсЗнаменательРегл - Остатки.СуммаДисконтированияРеглОстаток <= -0.01
		|";
...
//++ Локализация
Процедура ЗаполнитьДвиженияНУФинансовыхИнструментов(Организация, Период, ХозяйственнаяОперация, ДатаНачалаПересчета = Неопределено) Экспорт
	...
	|	РегистраторыКурсовыхРазниц.ПометкаУдаления КАК ПометкаУдаления,
	...
	Пока Месяцы.Следующий() Цикл
		...
		Пока Регистраторы.Следующий() Цикл
			...
			КонецЕсли;
			Если Регистраторы.ПометкаУдаления Тогда
