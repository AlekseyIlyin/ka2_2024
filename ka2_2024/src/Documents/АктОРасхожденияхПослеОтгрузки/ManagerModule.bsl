...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает ""Неопределено"" в качестве пустого значения реквизита ""Документ поступления"" табличной части ""Товары"".
	|Очищает данные по НДС в табличной части ""Товары"" для операции ""Отгрузка товаров с хранения"", ""Передача давальцу"", ""Возврат давальцу"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			Документ.АктОРасхожденияхПослеОтгрузки.Товары КАК ТаблицаТовары
	|		ГДЕ
	|			ТаблицаТовары.Ссылка = АктОРасхожденияхПослеОтгрузки.Ссылка
	|			И АктОРасхожденияхПослеОтгрузки.ТипОснованияАктаОРасхождении В (
	|				ЗНАЧЕНИЕ(Перечисление.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровСХранения))
	|			И (ТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|				ИЛИ ТаблицаТовары.СуммаНДС > 0
	|				ИЛИ ТаблицаТовары.СуммаСНДС > 0
	|				ИЛИ ТаблицаТовары.СуммаНДСПоДокументу > 0
	|				ИЛИ ТаблицаТовары.СуммаСНДСПоДокументу > 0))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			ТаблицаТовары.Ссылка = АктОРасхожденияхПослеОтгрузки.Ссылка
	|			И ТаблицаТовары.ДокументПоступления = ЗНАЧЕНИЕ(Документ.ПриобретениеТоваровУслуг.ПустаяСсылка))
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			Документ.АктОРасхожденияхПослеОтгрузки.Товары КАК ТаблицаТовары
	|		ГДЕ
	|			ТаблицаТовары.Ссылка = ОбновляемыеДанные.Ссылка
	|			И ОбновляемыеДанные.Ссылка.ТипОснованияАктаОРасхождении В (
	|											ЗНАЧЕНИЕ(Перечисление.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровСХранения))
	|			И (ТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|				ИЛИ ТаблицаТовары.СуммаНДС > 0
	|				ИЛИ ТаблицаТовары.СуммаСНДС > 0
	|				ИЛИ ТаблицаТовары.СуммаНДСПоДокументу > 0
	|				ИЛИ ТаблицаТовары.СуммаСНДСПоДокументу > 0)) КАК ОчиститьНДС,
	|	 ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|			ГДЕ
	|				ТаблицаТовары.Ссылка = ОбновляемыеДанные.Ссылка
	|				И ТаблицаТовары.ДокументПоступления = ЗНАЧЕНИЕ(Документ.ПриобретениеТоваровУслуг.ПустаяСсылка)) КАК ОчиститьДокументПоступления
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|";
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			ДанныеОбъекта = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
			...
			Если ВыборкаДокументов.ОчиститьНДС
				Или ВыборкаДокументов.ОчиститьДокументПоступления Тогда
				
				ОбъектИзменен = Истина;
				
				ТабличнаяЧастьТовары = ДанныеОбъекта.Товары;
				Для Каждого СтрокаТЧТовары Из ТабличнаяЧастьТовары Цикл
					
					Если ВыборкаДокументов.ОчиститьНДС Тогда
						
						Если ЗначениеЗаполнено(СтрокаТЧТовары.СтавкаНДС)
						 Или ЗначениеЗаполнено(СтрокаТЧТовары.СуммаНДС)
						 Или ЗначениеЗаполнено(СтрокаТЧТовары.СуммаСНДС)
						 Или ЗначениеЗаполнено(СтрокаТЧТовары.СуммаНДСПоДокументу)
						 Или ЗначениеЗаполнено(СтрокаТЧТовары.СуммаСНДСПоДокументу) Тогда
							
							СтрокаТЧТовары.СтавкаНДС            = Справочники.СтавкиНДС.ПустаяСсылка();
							СтрокаТЧТовары.СуммаНДС             = 0;
							СтрокаТЧТовары.СуммаСНДС            = 0;
							СтрокаТЧТовары.СуммаНДСПоДокументу  = 0;
							СтрокаТЧТовары.СуммаСНДСПоДокументу = 0;
							
						КонецЕсли;
						
					КонецЕсли;
					
					Если ВыборкаДокументов.ОчиститьДокументПоступления
						И Не ЗначениеЗаполнено(СтрокаТЧТовары.ДокументПоступления) Тогда
						СтрокаТЧТовары.ДокументПоступления = Неопределено;
					КонецЕсли;
