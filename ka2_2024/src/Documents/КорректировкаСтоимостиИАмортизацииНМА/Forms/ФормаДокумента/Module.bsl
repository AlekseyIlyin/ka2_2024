...
Процедура НастроитьЗависимыеЭлементыФормыНаКлиентеНаСервере(Форма, ИзмененныеРеквизиты)
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ВидОбъектовУчета")
		...
		Элементы.ПодсказкаАмортизация.Заголовок = 
			?(Объект.ВидОбъектовУчета = ПредопределенноеЗначение("Перечисление.ВидыОбъектовУчетаНМА.РасходыНаНИОКР"),
				НСтр("ru = 'Стоимость указывается без учета списания расходов при расчете амортизации в месяце корректировки'"),
				НСтр("ru = 'Сумма амортизации указывается без учета амортизации, начисленной в месяце корректировки'"));
	КонецЕсли;
...
Процедура ВидОбъектовУчетаПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	НастроитьЗависимыеЭлементыФормы("ВидОбъектовУчета");
