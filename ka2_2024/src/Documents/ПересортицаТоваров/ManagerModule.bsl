...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|Переносит значение поля ""Цена"" таблицы ""Товары"" в поле ""Цена (забаланс)"" для товаров, принятых на ответственное хранение.
	|Переносит значение поля ""Сумма"" таблицы ""Виды запасов"" в поле ""Сумма (забаланс)"" для товаров, принятых на ответственное хранение.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		
	|	ГДЕ
	|		НЕ Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|		И ИСТИНА В(
	|			ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|				И ТаблицаВидыЗапасов.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|				И ТаблицаВидыЗапасов.ВидЗапасовОприходование.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|				И ТаблицаВидыЗапасов.СуммаЗабаланс = 0
	|			)
	|	) КАК ВложенныйЗапрос
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ОбновляемыеДанные.Ссылка КАК Документ.ПересортицаТоваров) КАК Ссылка
	...
	|ПОМЕСТИТЬ ПеренестиЦены
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|ГДЕ
	|	НЕ ОбновляемыеДанные.Ссылка.ПриходоватьТоварыПоСебестоимостиСписания
	|	И ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ПересортицаТоваров.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|		ГДЕ
	|			ТаблицаВидыЗапасов.Ссылка = ОбновляемыеДанные.Ссылка
	|			И ТаблицаВидыЗапасов.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|			И ТаблицаВидыЗапасов.ВидЗапасовОприходование.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|			И ТаблицаВидыЗапасов.СуммаЗабаланс = 0
	|		)
	...
	|	ОбновляемыеДанные.Ссылка                КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ПеренестиЦены.ВерсияДанных ЕСТЬ НЕ NULL
	|			ТОГДА ПеренестиЦены.ВерсияДанных
	|		ИНАЧЕ ОбновляемыеДанные.Ссылка.ВерсияДанных
	|	КОНЕЦ                                   КАК ВерсияДанных,
	|	ПеренестиЦены.ВерсияДанных ЕСТЬ НЕ NULL КАК ПеренестиЦены
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПеренестиЦены КАК ПеренестиЦены
	|	ПО ПеренестиЦены.Ссылка = ОбновляемыеДанные.Ссылка
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(Выборка.Ссылка, Выборка.ВерсияДанных, Параметры.Очередь); // ДокументОбъект.ПересортицаТоваров
			Если ДокументОбъект = Неопределено Тогда
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			Если Не ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПересортицаТоваров Тогда
				ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПересортицаТоваров;
			КонецЕсли;
			
			Если Выборка.ПеренестиЦены Тогда
				ПеренестиЦены(ДокументОбъект);
			КонецЕсли;
...
Процедура ПеренестиЦены(Объект)
	
	ТипыЗапасов = Объект.ВидыЗапасов.ВыгрузитьКолонку("ВидЗапасов");
	ТипыЗапасов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ТипыЗапасов, "ТипЗапасов");
	
	ТипыЗапасовОприходования = Объект.ВидыЗапасов.ВыгрузитьКолонку("ВидЗапасовОприходование");
	...
	ТаблицаТовары.Индексы.Добавить("АналитикаУчетаНоменклатуры");
	
	Отбор = Новый Структура("АналитикаУчетаНоменклатуры");
	Для Каждого СтрокаВидыЗапасов Из Объект.ВидыЗапасов Цикл
		
		Если Не ТипыЗапасов.Получить(СтрокаВидыЗапасов.ВидЗапасов) = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи
			Или Не ТипыЗапасовОприходования.Получить(СтрокаВидыЗапасов.ВидЗапасовОприходование) = Перечисления.ТипыЗапасов.ТоварНаХраненииСПравомПродажи Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаВидыЗапасов);
		Для Каждого СтрокаТовары Из ТаблицаТовары.НайтиСтроки(Отбор) Цикл
			СтрокаТовары.ЦенаЗабаланс = СтрокаТовары.Цена;
			СтрокаТовары.Цена         = 0;
		КонецЦикла;
		
		СтрокаВидыЗапасов.СуммаЗабаланс = СтрокаВидыЗапасов.Сумма;
		СтрокаВидыЗапасов.Сумма         = 0;
