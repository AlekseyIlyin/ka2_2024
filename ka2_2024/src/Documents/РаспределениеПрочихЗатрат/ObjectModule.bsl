...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ УправленческийУчет И НЕ РегламентированныйУчет И НЕ НалоговыйУчет Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Укажите в каких видах учета должен отражаться документ (управленческий, регламентированный, налоговый).'"),
			ОбщегоНазначенияУТ.КлючДанныхДляСообщенияПользователю(Ссылка),
			"УправленческийУчет",
			"Объект",
			Отказ);
