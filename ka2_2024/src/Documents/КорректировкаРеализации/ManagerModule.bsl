...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "CreditNote") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"Документ.КорректировкаРеализации.ПФ_MXL_DebitCreditNote");
...
Функция СформироватьПечатнуюФормуДебетНота(СтруктураТипов, ОбъектыПечати, ПараметрыПечати, КодЯзыка) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ДанныеДляПечати = ДанныеДляПечатиДебетКредитНота(ПараметрыПечати, СтруктураОбъектов.Значение, "ДебетНота", КодЯзыка);
...
Функция СформироватьПечатнуюФормуКредитНота(СтруктураТипов, ОбъектыПечати, ПараметрыПечати, КодЯзыка) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ДанныеДляПечати = ДанныеДляПечатиДебетКредитНота(ПараметрыПечати, СтруктураОбъектов.Значение, "КредитНота", КодЯзыка);
...
Процедура ЗаполнитьРеквизитыШапкиДебетКредитНота(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаТовары, Предварительный, КодЯзыка)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.КорректировкаРеализации.ПФ_MXL_DebitCreditNote", КодЯзыка);
...
Функция ВыбратьРеквизит(ИспользоватьМеждународный, ЛокальныйРеквизит, МеждународныйРеквизит)
	...
	Если ИспользоватьМеждународный Тогда
		...
		Если ЗначениеЗаполнено(МеждународныйРеквизит) Тогда
			
			ВозвращаемоеЗначение = МеждународныйРеквизит;
...
Функция ТекстЗапросаВТДанныеПредыдущейКорректировки() Экспорт
	...
	|		Документ.КорректировкаРеализации КАК КорректировкаРеализации
	|	ГДЕ
	|		КорректировкаРеализации.Дата < &Дата
	|		И КорректировкаРеализации.ДокументОснование = &ДокументОснование
	|		И КорректировкаРеализации.Проведен
	|	
	|	СГРУППИРОВАТЬ ПО
	|		КорректировкаРеализации.ДокументОснование) КАК ПоследняяКорректировка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК КорректировкаРеализации
	|		ПО ПоследняяКорректировка.ДокументОснование = КорректировкаРеализации.ДокументОснование
	|			И ПоследняяКорректировка.Дата = КорректировкаРеализации.Дата";
...
Функция ТекстЗапросаВТПроверкиДанныхПоПоследнейКорректировке()
	...
	|	И КорректировкаРеализацииТовары.Ссылка.Ссылка <> &Ссылка
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Хозяйственная операция"" и новый реквизит ""Тип корректировки"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.КорректировкаРеализации КАК КорректировкаРеализации
	|ГДЕ
	|	КорректировкаРеализации.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
	|	ИЛИ КорректировкаРеализации.ТипКорректировки = ЗНАЧЕНИЕ(Перечисление.ТипыКорректировки.ПустаяСсылка)";
