...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("31f49035-b0bb-43d7-8e67-11bfc5713ef6");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.ПорчаТоваровУХранителя.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|			ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ОбновляемыеДанные.Ссылка КАК Документ.ПорчаТоваровУХранителя) КАК Ссылка
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(Выборка.Ссылка, Выборка.ВерсияДанных, Параметры.Очередь); // ДокументОбъект.ПорчаТоваровУХранителя
