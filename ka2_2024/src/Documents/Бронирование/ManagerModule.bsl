...
Функция ТекстЗапросаВТКурсыВалютСтрокОтчета(Запрос, ТекстыЗапроса)
	...
	|		(СтрокиОтчета.СуммаИтог - КурсыВалютАвансов.Получено) * КоэффициентыПересчетаВалют.КоэффициентПересчетаУпр КАК СуммаУпр,
	|		(СтрокиОтчета.СуммаИтог - КурсыВалютАвансов.Получено) * КоэффициентыПересчетаВалют.КоэффициентПересчетаРегл КАК СуммаРегл
	|	ИЗ
	|		СтрокиОтчета КАК СтрокиОтчета
	|		
	|			ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалютАвансов КАК КурсыВалютАвансов
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.Бронирование.Операции КАК ТЧОперации
	|			ГДЕ
	|				ТЧОперации.Ссылка = Документ.Ссылка
	|				И ТЧОперации.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|				И ТЧОперации.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	//-- НЕ УТ
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
