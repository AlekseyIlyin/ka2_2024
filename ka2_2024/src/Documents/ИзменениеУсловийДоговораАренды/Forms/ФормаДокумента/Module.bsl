...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ИзменениеУсловийДоговораАренды.ПараметрыВыбораСтатейИАналитик(
			ВнеоборотныеАктивы.СозданДляПереходаНаФСБУ25_2018(Объект.Дата, Объект.Организация));
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПараметрыВыбораСтатейИАналитик = Документы.ИзменениеУсловийДоговораАренды.ПараметрыВыбораСтатейИАналитик(
				ВнеоборотныеАктивы.СозданДляПереходаНаФСБУ25_2018(ТекущийОбъект.Дата, ТекущийОбъект.Организация));
...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(Знач ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("Дата")
		...
		Элементы.ОССтоимостьБУ.Заголовок = ВнеоборотныеАктивыКлиентСервер.ДобавитьКСтрокеВалюту(
			НСтр("ru = 'БУ'"), 
			СлужебныеПараметрыФормы.ВалютаРеглПредставление);
		
		Элементы.ОССтоимостьНУ.Заголовок = ВнеоборотныеАктивыКлиентСервер.ДобавитьКСтрокеВалюту(
			НСтр("ru = 'НУ'"), 
			СлужебныеПараметрыФормы.ВалютаРеглПредставление);
		
		Элементы.ОССтоимостьУУ.Заголовок = ВнеоборотныеАктивыКлиентСервер.ДобавитьКСтрокеВалюту(
			НСтр("ru = 'УУ'"), 
...
Процедура ИнициализироватьСлужебныеПараметрыФормы()
	...
	ЗначениеПараметра = ВнеоборотныеАктивы.ИспользуетсяУчетАрендыПоФСБУ25_2018(Объект.Организация, Объект.Дата, Истина);
...
Процедура ЗаполнитьАрендованнымиНаСервере()
	...
	|ИЗ
	|	РегистрСведений.АрендованныеОС.СрезПоследних(&ПериодГраница) КАК АрендованныеОС
	|ГДЕ
...
Процедура ЗаполнитьГрафикПоРасчетамНаСервере(Знач РассчитатьСтавкуИлиСтоимость)
	...
	СозданДляПерехода = ВнеоборотныеАктивы.СозданДляПереходаНаФСБУ25_2018(Объект.Дата, Объект.Организация);
	...
	Иначе
		...
		ВнеоборотныеАктивы.ЗаполнитьГрафикОплатИНачисленийПоРасчетамИзменениеУсловий(Объект, СлужебныеПараметрыФормы.РеквизитыДоговора);
	КонецЕсли;
