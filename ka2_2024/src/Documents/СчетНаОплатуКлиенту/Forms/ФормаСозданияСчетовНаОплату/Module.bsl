...
Процедура ОбновитьСервер()
	...
	Иначе
		...
		ИмяДокумента = ДокументОснование.Метаданные().ПолноеИмя();
		...
		|	ДанныеДокумента.Ссылка                    КАК ДокументОснование,
		|	&ТекстПоляСуммаДокумента КАК СуммаДокумента,
		|	ДанныеДокумента.Номер                     КАК НомерДокумента,
		|	ДанныеДокумента.БанковскийСчет            КАК БанковскийСчет,
		|	ДанныеДокумента.Организация.Префикс       КАК Префикс,
		|	ДанныеДокумента.Касса                     КАК Касса,
		|	//ДанныеДокументаКонтактноеЛицо
		|	//ДанныеДокументаЭтоЗаказКакСчет
		|	ДанныеДокумента.ФормаОплаты               КАК ФормаОплаты,
		|	ДанныеДокумента.Ссылка                    КАК Документ,
		|	&ТекстПолейСтатусХозяйственнаяОперация,
		|	НЕ ДанныеДокумента.Проведен               КАК ЕстьОшибкиПроведен
		|ИЗ
		|	&ОсновнаяТаблица КАК ДанныеДокумента
		|ГДЕ
		...
		Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
			...
			ТекстЗапросаРасчеты = СтрЗаменить(ТекстЗапросаРасчеты,
				"&ШаблонПоляОплачено",
				СтрШаблон("%1 + %2",
			...
			ТекстЗапросаРасчеты = СтрЗаменить(ТекстЗапросаРасчеты, "&ДанныеОтчета", "4");
		...
		Если ИмяДокумента = "Документ.ЗаказКлиента"
			Или ИмяДокумента = "Документ.ЗаявкаНаВозвратТоваровОтКлиента" Тогда
			...
			|	ЛОЖЬ КАК Оплачена,
			|	1 КАК ИндексКартинки,
			|	ЕСТЬNULL(СУММА(РасчетыСКлиентами.КОплате), 0) КАК СуммаПлатежа,
			|	ЕСТЬNULL(СУММА(РасчетыСКлиентами.КОплате), 0) КАК СуммаКОплате,
		...
		Если ИмяДокумента = "Документ.ЗаявкаНаВозвратТоваровОтКлиента" Тогда
			
			ТекстПоляСуммаДокумента =
		...
		ИначеЕсли ИмяДокумента = "Документ.ЗаказКлиента" Тогда
		...
		Если ИмяДокумента <> "Документ.ОтчетКомиссионера"
			И ИмяДокумента <> "Документ.ОтчетКомитентуОЗакупках" Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"ТекстЗапросаДокумент.ЭтапыГрафикаОплаты КАК ЭтапыГрафикаОплаты",
			СтрШаблон("%1.ЭтапыГрафикаОплаты КАК ЭтапыГрафикаОплаты", ИмяДокумента)
		);
		
		Если ИмяДокумента = "Документ.ЗаказКлиента" Тогда
		...
		Если ВыборкаШапка.Следующий() Тогда
			...
			Если ИмяДокумента <> "Документ.ОтчетКомиссионера" Тогда
		...
		Если ВыборкаДокументовЭтапы.Количество() = 0 Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В документе %1 не заполнены этапы графика оплаты'"),
				ДокументОснование
			);
			ВызватьИсключение ТекстОшибки;
		...
		Если ТаблицаЭтапов.Итог("СуммаПлатежа") = 0 Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не требуется вводить счета на оплату. Сумма платежа по документу %1 равна 0.'"),
				ДокументОснование
			);
			ВызватьИсключение ТекстОшибки;
