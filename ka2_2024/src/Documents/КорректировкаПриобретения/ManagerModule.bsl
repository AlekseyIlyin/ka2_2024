...
Функция ПараметрыВыбораСтатейИАналитикКорректировкаДоВводаОстатков(ПараметрыОпределенияДоступности) Экспорт
	...
	ПредставленияРеквизитов.Вставить("СтатьяРасходов",				НСтр("ru = 'Статья расходов/активов'"));
...
Функция РаспределенныеВидыЗапасов(ОсновнойЗапрос)
	...
	Запрос.Текст = СтрСоединить(ТекстыЗапроса.ВыгрузитьЗначения(), ОбщегоНазначенияУТ.РазделительЗапросовВПакете());
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.СобственныйТоварПоНеотфактурованнойПоставке)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ЗакупкаУПоставщикаФактуровкаПоставки)
	|		КОГДА ТаблицаВидыЗапасов.Количество = 0
	|		 И НАЧАЛОПЕРИОДА(&ПериодПоступления, МЕСЯЦ) <> НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.КорректировкаПриобретенияПрошлогоПериода)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ВозвратТоваровПоставщику) КОНЕЦ КАК НастройкаХозяйственнойОперации
	|
	|ИЗ
	|	Документ.КорректировкаПриобретения КАК ТаблицаДокумента
	...
	|		ИНАЧЕ ТаблицаВидыЗапасов.СуммаБезНДСУпр
	|	КОНЕЦ											КАК СтоимостьБезНДС,
	// Корректировка стоимости отражается как доп расходы только при корректировке прошлого периода.
	|	-ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.Количество = 0
	|		 И НАЧАЛОПЕРИОДА(&ПериодПоступления, МЕСЯЦ) <> НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|			ТОГДА 0
...
Процедура ОформитьКорректировкуТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеОснование.ХозяйственнаяОперация  КАК ХозяйственнаяОперация,
	...
	|	И &НеотфактурованныеПоставкиУслугРабот
	|	И ИсточникДанных.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), 
	|										ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|";
...
Функция ТекстЗапросаВТДанныеПредыдущейКорректировки() Экспорт
	...
	|	ПоследняяКорректировка.ДокументОснование КАК ДокументОснование,
	|	ПоследняяКорректировка.Дата КАК Дата
	...
	|		Документ.КорректировкаПриобретения КАК КорректировкаПриобретения
	|	ГДЕ
	|		КорректировкаПриобретения.Дата < &Дата
	|		И КорректировкаПриобретения.ДокументОснование = &ДокументОснование
	|		И КорректировкаПриобретения.Проведен
	|	
	|	СГРУППИРОВАТЬ ПО
	|		КорректировкаПриобретения.ДокументОснование) КАК ПоследняяКорректировка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения КАК КорректировкаПриобретения
	|		ПО ПоследняяКорректировка.ДокументОснование = КорректировкаПриобретения.ДокументОснование
	|			И ПоследняяКорректировка.Дата = КорректировкаПриобретения.Дата";
...
Функция ТекстЗапросаВТПроверкиДанныхПоПоследнейКорректировке()
	...
	|	И КорректировкаТовары.Ссылка.Ссылка <> &Ссылка
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("080c1928-b32c-4502-83ae-ab327ca63ef2");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение в документе ""Корректировка приобретения"" нового реквизита ""Тип корректировки""'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.КорректировкаПриобретения КАК КорректировкаПриобретения
	|ГДЕ
	|	КорректировкаПриобретения.ТипКорректировки = ЗНАЧЕНИЕ(Перечисление.ТипыКорректировки.ПустаяСсылка)";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ДокументОбъект.ТипКорректировки = Перечисления.ТипыКорректировки.ПустаяСсылка() Тогда
					ДокументОбъект.ТипКорректировки = Перечисления.ТипыКорректировки.ОбычнаяКорректировка;
