...
Процедура ПровестиИЗакрыть(Команда)
	...
	СоздатьЗаданиеНаСледующийМесяц(Объект.Организация, Объект.Дата);
...
Процедура СоздатьЗаданиеНаСледующийМесяц(Организация, Дата)
	// При ручном создании документов создаем задание на следующий месяц.
	Период = ДобавитьМесяц(Дата,1);
	Если Документы.НачислениеПроцентовПоАренде.ТребуетсяНачислениеПроцентовПоАренде(Период,Организация) Тогда
		НомерЗадания = РегистрыСведений.ЗаданияКЗакрытиюМесяца.УвеличитьНомерЗадания();
		
		Задания = РегистрыСведений.ЗаданияКЗакрытиюМесяца;
		
		Задания.СоздатьЗаписьРегистра(Период, 
										Неопределено, 
										Организация,
