...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) < ДокументЗаказКлиента.СуммаАвансаДоОбеспечения
		|	ТОГДА
		...
		|		И ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеРасход, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) < ДокументЗаказКлиента.СуммаПредоплатыДоОтгрузки + ДокументЗаказКлиента.СуммаАвансаДоОбеспечения
		|	ТОГДА
		...
		|				ИЛИ ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ОплатаФакт, 0) < ДокументЗаказКлиента.СуммаДокумента
		...
		|		И ДокументЗаказКлиента.СуммаДокумента > 0
		|	ТОГДА
		|		ВЫРАЗИТЬ (ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ОплатаФакт, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) КАК ЧИСЛО(31,2))
		|	ИНАЧЕ
		|		0
		...
		|	ТОГДА
		|		ВЫРАЗИТЬ ((ЕСТЬNULL(ОплаченоПоЗаказам.КОплатеРасход, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0)) * 100 / ДокументЗаказКлиента.СуммаДокумента КАК ЧИСЛО(20, 10))
		|	КОГДА
		|		ДокументЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
		|		И (ДокументЗаказКлиента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию)
		|			ИЛИ ДокументЗаказКлиента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи))
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("3dc464cc-20ca-4120-948a-9c9e3ab539e2");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.НеСогласован)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			И ТоварыДокумента.КодСтроки <> 0
		|			И НЕ ЗаказыКлиентов.ЭтоЗаказКакСчет
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.НеСогласован)) КАК Набор
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		|
		...
		|ИЗ
		|	Документ.ЗаказКлиента КАК ДокументыКРегистрации
		|		
		...
		|			И ДокументыКРегистрации.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.НеСогласован)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|				= ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.НеСогласован) КАК УстановитьРеквизитЭтоЗаказКакСчет
		|ИЗ
		|	Документ.ЗаказКлиента КАК ДокументыКРегистрации
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЕстьНакладныеНоНетДвижений КАК ЕстьНакладныеНоНетДвижений
	...
	СтатусНеСогласован = Перечисления.СтатусыЗаказовКлиентов.НеСогласован;
