...
Функция ДанныеДокументаДляПроведения(ДокументСсылка, СписокРегистров, ДопПараметры) Экспорт
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ПроверятьНастройкуПеречисленияЧерезПромежуточныйСчет",
		Константы.ВидимостьДатыНачалаПеречисленияЗарплатыЧерезПереводыВПути.Получить());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.9.25";
	Обработчик.Процедура = "Документы.ПодтверждениеЗачисленияЗарплаты.ЗаполнитьРеквизиты";
	Обработчик.РежимВыполнения = "Отложенно";
	
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты документа: Операция, Статья ДДС, Сумма документа, Валюта'");
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("fa24ed1d-abfb-4700-9500-273cc20b4502");
	Обработчик.БлокируемыеОбъекты = Метаданные.Документы.ПодтверждениеЗачисленияЗарплаты.ПолноеИмя();
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	
	ПроцедураЗаполненияДанныхОбновления = "Документы.ПодтверждениеЗачисленияЗарплаты.ЗарегистрироватьДанныеКЗаполнению";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = ПроцедураЗаполненияДанныхОбновления;
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Документы.ПодтверждениеЗачисленияЗарплаты.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	Обработчик.ИзменяемыеОбъекты = Метаданные.Документы.ПодтверждениеЗачисленияЗарплаты.ПолноеИмя();
	
	Обработчик.Многопоточный = Истина;

КонецПроцедуры

Процедура ЗарегистрироватьДанныеКЗаполнению(Параметры) Экспорт

	ПараметрыВыборки = Параметры.ПараметрыВыборки;
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ПодтверждениеЗачисленияЗарплаты";
	...
	Запрос.Текст = 
		...
		|	ГДЕ
		|		ПодтверждениеЗачисленияЗарплаты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
		...
		|		И ПодтверждениеЗачисленияЗарплаты.СтатьяДвиженияДенежныхСредств = ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		НЕ ПодтверждениеЗачисленияЗарплаты.ПометкаУдаления
		|		И ПодтверждениеЗачисленияЗарплаты.Валюта = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		НЕ ПодтверждениеЗачисленияЗарплаты.ПометкаУдаления
		|		И ПодтверждениеЗачисленияЗарплаты.СуммаДокумента = 0) КАК ВложенныйЗапрос";
...
Процедура ЗаполнитьРеквизиты(Параметры) Экспорт
	...
	Если СписокДокументов.Количество() <> 0 Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ЕСТЬNULL(ЗарплатныеПроекты.Валюта, &Валюта) КАК ВалютаЗарплатногоПроекта,
			...
			|	ПодтверждениеЗачисленияЗарплаты.ЗарплатныйПроект КАК ЗарплатныйПроект,
			...
			|	ПодтверждениеЗачисленияЗарплаты.СуммаДокумента КАК СуммаДокумента
			|ИЗ
			|	Документ.ПодтверждениеЗачисленияЗарплаты КАК ПодтверждениеЗачисленияЗарплаты
			...
			|ГДЕ
			|	ПодтверждениеЗачисленияЗарплаты.Ссылка В(&Список)";
		
		Запрос.УстановитьПараметр("Список", СписокДокументов.ВыгрузитьКолонку("Ссылка"));
		Запрос.УстановитьПараметр("Валюта", ЗарплатаКадрыПовтИсп.ВалютаУчетаЗаработнойПлаты());
		...
		Для каждого ТекущийДокумент Из СписокДокументов Цикл
			...
			Попытка
				...
				ЭлементБлокировки = Блокировка.Добавить("Документ.ПодтверждениеЗачисленияЗарплаты");
				ЭлементБлокировки.УстановитьЗначение("Ссылка", ТекущийДокумент.Ссылка);
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
				Блокировка.Заблокировать();
				
				ДанныеДокумента = ДанныеДокументов.Найти(ТекущийДокумент.Ссылка, "Документ");
				
				ДокументОбъект = ТекущийДокумент.Ссылка.ПолучитьОбъект();
				
				Если Не ЗначениеЗаполнено(ДанныеДокумента.ХозяйственнаяОперация) Тогда
				...
				Если Не ЗначениеЗаполнено(ДанныеДокумента.СтатьяДвиженияДенежныхСредств) Тогда
					ДокументОбъект.СтатьяДвиженияДенежныхСредств = 
						ЗначениеНастроекПовтИсп.ПолучитьСтатьюДвиженияДенежныхСредств(ДокументОбъект.ХозяйственнаяОперация);
				КонецЕсли;
				
				Если Не ЗначениеЗаполнено(ДанныеДокумента.Валюта) Тогда
					
					Если Не ЗначениеЗаполнено(ДанныеДокумента.ВалютаЗарплатногоПроекта) Тогда
						ДокументОбъект.Валюта = ЗарплатаКадрыПовтИсп.ВалютаУчетаЗаработнойПлаты();
					Иначе
						ДокументОбъект.Валюта = ДанныеДокумента.ВалютаЗарплатногоПроекта;
					КонецЕсли;
					
				КонецЕсли;
				
				Если ДанныеДокумента.СуммаДокумента = 0 Тогда
					ДокументОбъект.СуммаДокумента = ТекущийДокумент.Ссылка.Сотрудники.Итог("Сумма");
				КонецЕсли;
...
Процедура ДобавитьТекстЗапросаТаблицаДенежныеСредстваВПути(ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	НЕОПРЕДЕЛЕНО,
	               |	ДанныеДокумента.Валюта,
	               |	СУММА(ДанныеЗачисления.Сумма),
	               |	СУММА(ДанныеЗачисления.Сумма * &КоэффициентПересчетаВВалютуУпр),
	               |	СУММА(ДанныеЗачисления.Сумма * &КоэффициентПересчетаВВалютуРегл),
	               |	ДанныеДокумента.ХозяйственнаяОперация,
	               |	ДанныеДокумента.СтатьяДвиженияДенежныхСредств,
	               ...
	               |	И ДанныеЗачисления.РезультатЗачисленияЗарплаты = ЗНАЧЕНИЕ(Перечисление.РезультатыЗачисленияЗарплаты.Зачислено)
	               |	И ВЫБОР
	               |			КОГДА &ПроверятьНастройкуПеречисленияЧерезПромежуточныйСчет
	               |				ТОГДА ДанныеДокумента.Дата >= &ДатаНачалаПеречисленияЧерезПромежуточныйСчет
...
Функция ДобавитьТекстЗапросаТаблицаПрочиеАктивыПассивы(ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	И ВЫБОР
	               |			КОГДА &ПроверятьНастройкуПеречисленияЧерезПромежуточныйСчет
	               |				ТОГДА ПодтверждениеЗачисленияЗарплаты.Дата >= &ДатаНачалаПеречисленияЧерезПромежуточныйСчет
	               |						И &ДатаНачалаПеречисленияЧерезПромежуточныйСчет <> ДАТАВРЕМЯ(1, 1, 1)
	               |			ИНАЧЕ ИСТИНА
	               |		КОНЕЦ";
