...
Функция ТаблицаОстатковТоваровКОформлениюЗаявленийОВвозеТоваров(ДокументПоступления, ЗаполнениеНаОсновании = Ложь, ПериодОформления = Неопределено) Экспорт
	...
	Для каждого СтрокаТаблицыТовары Из ТаблицаТоваров Цикл
		...
		Если ДокументПоступления.Валюта <> ВалютаРегламентированногоУчета Тогда
			СтруктураКурса = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(ДокументПоступления.Валюта, СтрокаТаблицыТовары.ДатаПринятияНаУчет);
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода)
	|	КОНЕЦ КАК ТипЗаписи,
	|	0																		КАК Количество,
	|	0																		КАК Стоимость,
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)
	|	КОНЕЦ 													КАК РазделУчета,
	|	ЕСТЬNULL(ТоварыПереработчика.ВидЗапасов, ТоварыДокумента.ВидЗапасов) КАК ВидЗапасов,
	|	ДанныеДокумента.Организация												КАК Организация,
	|
	//	партионный учет версии 2.2
	|	ВЫБОР КОГДА &ПартионныйУчетВерсии22 И &ФИФОСкользящаяОценка
	...
	|		КОГДА &ПартионныйУчетВерсии22 ТОГДА
	|			ВЫБОР
	|				КОГДА &ФормироватьВидыЗапасовПоСделкам И ЕСТЬNULL(ОтчетПереработчика2_5.Сделка.ОбособленныйУчетТоваровПоСделке, ЛОЖЬ)
	|					ТОГДА ОтчетПереработчика2_5.Сделка
	|				КОГДА &ФормироватьВидыЗапасовПоПодразделениямМенеджерам
	|				 И ОтчетПереработчика2_5.Подразделение.ВариантОбособленногоУчетаТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыОбособленногоУчетаТоваров.ПоМенеджерамПодразделения)
	|					ТОГДА ОтчетПереработчика2_5.Менеджер
	|				КОГДА &ФормироватьВидыЗапасовПоПодразделениямМенеджерам
	|				 И ОтчетПереработчика2_5.Подразделение.ВариантОбособленногоУчетаТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыОбособленногоУчетаТоваров.ПоПодразделению)
	|					ТОГДА ОтчетПереработчика2_5.Подразделение
	|				ИНАЧЕ НЕОПРЕДЕЛЕНО
	|			КОНЕЦ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ 																	КАК АналитикаФинансовогоУчета,
	...
	|	КОНЕЦ)	 																КАК НДСРегл,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(ОтчетПереработчика2_5.Дата, МЕСЯЦ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаСтоимости)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретенияПрошлогоПериода) 
	|	КОНЕЦ КАК ТипЗаписи,
	|
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ТоварыПереработчика
	|	ПО 
	|		ТоварыПереработчика.Ссылка = ТоварыДокумента.ДокументПоступления
	|		И ТоварыПереработчика.АналитикаУчетаНоменклатуры = ТоварыДокумента.АналитикаУчетаНоменклатуры
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ОтчетПереработчика2_5 КАК ОтчетПереработчика2_5
	|	ПО
	|		ТоварыДокумента.ДокументПоступления = ОтчетПереработчика2_5.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаАналитикУчетаПартий КАК ТаблицаАналитикУчетаПартийПоступления
	|	ПО ТаблицаАналитикУчетаПартийПоступления.НомерСтроки 	   = ТоварыДокумента.НомерСтроки
	|	 И ТаблицаАналитикУчетаПартийПоступления.ИмяТабличнойЧасти = ""ТоварыПоступления""
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)
	|	КОНЕЦ,
	|	ЕСТЬNULL(ТоварыПереработчика.ВидЗапасов, ТоварыДокумента.ВидЗапасов),
	|	ДанныеДокумента.Организация,
	|	ВЫБОР КОГДА &ПартионныйУчетВерсии22 И &ФИФОСкользящаяОценка
	|		ТОГДА ТоварыДокумента.ДокументПоступления
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
...
Функция ТекстЗапросаТоварыКОформлениюЗаявленийОВвозе(ЗаполнениеНаОсновании = Ложь)
	...
	|	ОтчетПереработчикаВидыЗапасов.АналитикаУчетаНоменклатуры.Подразделение,
	|	"""",
	|	""""
	|ИЗ
	...
	|	NULL,
	|	NULL,
	|	ЗаключениеДоговораАрендыОС.Подразделение,
	|	"""",
	...
	|	ТоварыБезГруппировки.Подразделение КАК Подразделение,
	...
	|	ДокументПоступленияТовары.Подразделение 		КАК Подразделение,
...
Функция РаспределитьСуммыДляУслугАренды(ЗапросАренды)
	...
	ЗапросАренды.Текст = "ВЫБРАТЬ
		...
		|	ТоварыЗаявления.Подразделение КАК Подразделение,
