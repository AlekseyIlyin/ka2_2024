...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) < ДокументЗаявка.СуммаАвансаДоОбеспечения
		|	ТОГДА
		...
		|		ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеРасход, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) < ДокументЗаявка.СуммаПредоплатыДоОтгрузки + ДокументЗаявка.СуммаАвансаДоОбеспечения
		|	ТОГДА
		...
		|			ИЛИ ДокументЗаявка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КОбеспечению))
		|		И ДокументЗаявка.СуммаЗамены > 0
		|		И (ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеРасход, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) >= ДокументЗаявка.СуммаПредоплатыДоОтгрузки + ДокументЗаявка.СуммаАвансаДоОбеспечения ИЛИ
		...
		|	ТОГДА
		|		ВЫРАЗИТЬ (ЕСТЬNULL(ОплаченоПоЗаказам.КОплатеРасход, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0) КАК ЧИСЛО(31,2))
		|	ИНАЧЕ
		|		0
		...
		|	ТОГДА
		|		ВЫРАЗИТЬ ((ЕСТЬNULL(ОплаченоПоЗаказам.КОплатеРасход, 0)
		|			- ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ВозвратОплатыБезОбъектаРасчетовВозврат, 0)) * 100 / ДокументЗаявка.СуммаЗамены КАК ЧИСЛО(20, 10))
		|	ИНАЧЕ
		|		0
		...
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	ВтДанныеПоРасчетамСКлиентами КАК ВтДанныеПоРасчетамСКлиентами
		|ПО
		|	ДокументЗаявка.ОбъектРасчетов = ВтДанныеПоРасчетамСКлиентами.ОбъектРасчетов
		|
		|ГДЕ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("72475604-4d6f-4439-9020-601c4036ddc7");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебный реквизит ""Это заказ как счет"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ЕстьНакладныеНоНетДвижений
		|ИЗ(
		|	ВЫБРАТЬ
		|		ТоварыДокумента.ЗаказКлиента КАК Ссылка
		|	ИЗ
		|		Документ.РеализацияТоваровУслуг.Товары КАК ТоварыДокумента
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаказыКлиентов
		|			ПО ЗаказыКлиентов.Ссылка = ТоварыДокумента.ЗаказКлиента
		|	ГДЕ
		|		ТоварыДокумента.Ссылка.Проведен
		|			И ТоварыДокумента.Ссылка.РеализацияПоЗаказам
		|			И ТоварыДокумента.КодСтроки <> 0
		|			И НЕ ЗаказыКлиентов.ЭтоЗаказКакСчет
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		УслугиДокумента.ЗаказКлиента КАК Ссылка
		|	ИЗ
		|		Документ.АктВыполненныхРабот.Услуги КАК УслугиДокумента
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаказыКлиентов
		|			ПО ЗаказыКлиентов.Ссылка = УслугиДокумента.ЗаказКлиента
		|	ГДЕ
		|		УслугиДокумента.Ссылка.Проведен
		|			И УслугиДокумента.Ссылка.АктПоЗаказам
		|			И НЕ ЗаказыКлиентов.ЭтоЗаказКакСчет
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|	ГДЕ
		|		ТоварыДокумента.Ссылка.Проведен
		|			И ТоварыДокумента.Ссылка.ПередачаПоЗаказам
		|			И ТоварыДокумента.КодСтроки <> 0
		|			И НЕ ЗаказыКлиентов.ЭтоЗаказКакСчет
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)) КАК Набор
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		|
		...
		|ВЫБРАТЬ
		|	ДокументыКРегистрации.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ДокументыКРегистрации
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЕстьНакладныеНоНетДвижений КАК ЕстьНакладныеНоНетДвижений
		|		ПО ЕстьНакладныеНоНетДвижений.Ссылка = ДокументыКРегистрации.Ссылка
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.ИспользоватьРасширенныеВозможностиЗаказаКлиента КАК ВключеныЗаказыКакСчет
		|		ПО НЕ ВключеныЗаказыКакСчет.Значение
		|ГДЕ
		// Регистрация к обновлению признака ЭтоЗаказКакСчет
		|	НЕ ЕстьНакладныеНоНетДвижений.Ссылка ЕСТЬ NULL
		| 		ИЛИ НЕ ВключеныЗаказыКакСчет.Значение ЕСТЬ NULL
		|			И НЕ ДокументыКРегистрации.ЭтоЗаказКакСчет
		|			И ДокументыКРегистрации.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|			И НЕ ЗаказыКлиентов.ЭтоЗаказКакСчет
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)
		|			И ЗаказыКлиентов.Ссылка В(&Ссылки)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			И НЕ ЗаказыКлиентов.ЭтоЗаказКакСчет
		|			И ЗаказыКлиентов.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)
		|			И ЗаказыКлиентов.Ссылка В(&Ссылки)) КАК Набор
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		|
		...
		| 		ИЛИ НЕ ВключеныЗаказыКакСчет.Значение ЕСТЬ NULL
		|			И НЕ ДокументыКРегистрации.ЭтоЗаказКакСчет
		|			И ДокументыКРегистрации.Статус
		|				= ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована) КАК УстановитьРеквизитЭтоЗаказКакСчет
		|ИЗ
		|	Документ.ЗаявкаНаВозвратТоваровОтКлиента КАК ДокументыКРегистрации
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ЕстьНакладныеНоНетДвижений КАК ЕстьНакладныеНоНетДвижений
		...
		|		ПО НЕ ВключеныЗаказыКакСчет.Значение
		|ГДЕ
		|	ДокументыКРегистрации.Ссылка В(&Ссылки)";
		
	Выборка = Запрос.Выполнить().Выбрать();
	
	СтатусНеСогласован = Перечисления.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				...
				Если Выборка.УстановитьРеквизитЭтоЗаказКакСчет Тогда
					
					Если Не ДокументОбъект.ЭтоЗаказКакСчет И ДокументОбъект.Статус = СтатусНеСогласован Тогда
						ДокументОбъект.ЭтоЗаказКакСчет = Истина;
					КонецЕсли;
