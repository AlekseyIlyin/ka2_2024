...
Процедура ТекстЗапросаТаблицаВтОплатаОбеспечительногоПлатежа(ТекстыЗапроса)
	...
	|		И
	|			РасшифровкаСписаниеБезналичныхДС.ТипПлатежаПоАренде = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
	|ГДЕ
	|	Начисления.Ссылка = &Ссылка
	...
	|		И
	|			РасшифровкаРасходныйКассовыйОрдер.ТипПлатежаПоАренде = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
	|ГДЕ
	|	Начисления.Ссылка = &Ссылка
	|	И Начисления.ТипНачисления = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаТаблицаРасчетыПоФинансовымИнструментам(Запрос, ТекстыЗапроса, Регистры)
	...
	#Область ПрименяетсяУСН
	
	#Область УслугаСНДС_УСН // Дт <76.07.1> - Кт <70.07.2, 70.07.3, 70.07.4>
	ТекстЗапроса =
	"//УслугаСНДС_УСН // Дт <76.07.1> - Кт <70.07.2, 70.07.3, 70.07.4>
	|ВЫБРАТЬ
	|	&Период                                                                 КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)                                  КАК ВидДвижения,
	|	
	...
	|	&ВалютаВзаиморасчетов                                                   КАК Валюта,
	|	
	|	ТаблицаСуммы.СуммаБезНДСВзаиморасчетов + ТаблицаСуммы.СуммаНДСВзаиморасчетов КАК Сумма,
	|
	|	ВЫБОР
	|		КОГДА НЕ &ОтразитьУслугиВРасходахУпр
	|			ТОГДА ТаблицаСуммы.СуммаБезНДСУпр + ТаблицаСуммы.СуммаНДСУпр
	...
	#Область УслугаНаРасходыСНДС // Дт <Расходы> - Кт <76.07.2>
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	&Период                                                                 КАК Период,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)                                  КАК ВидДвижения,
	...
	|	ВЫБОР 
	|		КОГДА &ОтразитьУслугиВРасходахРегл И &ОтразитьУслугиВРасходахУпр
	|			ТОГДА ТаблицаСуммы.СуммаБезНДСВзаиморасчетов + ТаблицаСуммы.СуммаНДСВзаиморасчетов
	|		ИНАЧЕ 0
	|	КОНЕЦ  КАК Сумма,
	|
	|	ВЫБОР 
...
Процедура ТекстЗапросаТаблицаВтСуммыДокументовВВалютахУчета(ТекстыЗапроса)
	...
	|			ТОГДА 
	|				ВЫБОР
	|					КОГДА &ВалютныйДоговор ИЛИ КурсыНаДатуДокумента.Курс ЕСТЬ NULL ТОГДА
	|						(Начисления.СуммаВзаиморасчетов - Начисления.СуммаНДСВзаиморасчетов) * КурсыОбеспечительногоПлатежа.Курс
	|					ИНАЧЕ
	|						(Начисления.СуммаВзаиморасчетов - Начисления.СуммаНДСВзаиморасчетов) 
	|						* (КурсыОбеспечительногоПлатежа.Курс + КурсыНаДатуДокумента.Курс)
	|						/ 2 
	|					КОНЕЦ
	...
	|					КОГДА &ВалютныйДоговор ИЛИ КурсыНаДатуДокумента.Курс ЕСТЬ NULL ТОГДА
	|						(Начисления.СуммаВзаиморасчетов - Начисления.СуммаНДСВзаиморасчетов) 
	|						* КурсыОбеспечительногоПлатежа.Курс
	|					ИНАЧЕ
	|						(Начисления.СуммаВзаиморасчетов - Начисления.СуммаНДСВзаиморасчетов) 
	|						* (КурсыОбеспечительногоПлатежа.Курс + КурсыНаДатуДокумента.Курс)
	|						/ 2
	...
	|					ИНАЧЕ
	|						Начисления.СуммаНДСВзаиморасчетов 
	|						* (КурсыОбеспечительногоПлатежа.Курс + КурсыНаДатуДокумента.Курс)
	|						/ 2
	|					КОНЕЦ
	...
	|
	|			ВЫБОР
	|				КОГДА &ВалютныйДоговор ИЛИ КурсыОбеспечительногоПлатежа.КурсУпр ЕСТЬ NULL ТОГДА
	|					(Начисления.СуммаСНДС - Начисления.СуммаНДС) * &КоэффициентПересчетаВВалютуУпр
	|				ИНАЧЕ
	|					(Начисления.СуммаСНДС - Начисления.СуммаНДС) 
	|					* (КурсыОбеспечительногоПлатежа.Курс / КурсыОбеспечительногоПлатежа.КурсУпр 
	|						+ КурсыНаДатуДокумента.Курс / КурсыНаДатуДокумента.КурсУпр)
	|					/ 2
	|			КОНЕЦ
...
Процедура ТекстЗапросаТаблицаВтРасходы(Запрос, ТекстыЗапроса)
	...
	Если ТекстЗапроса = Неопределено Тогда
		...
		|	ТаблицаСуммы.СуммаНДС      КАК СуммаНДСУпр,
...
ИмяРегистра = "ВтИсходныеПрочиеРасходы";
	...
	|
	|	0 КАК СуммаБезНДС,
	|	ТаблицаРасходы.СуммаНДСУпр КАК СуммаБезНДСУпр,
	|	ТаблицаРасходы.СуммаРегл КАК СуммаСНДСРегл,
	|	ТаблицаРасходы.СуммаБезНДСРегл КАК СуммаБезНДСРегл,
	|	0 КАК ПостояннаяРазница,
	|	ТаблицаРасходы.ВременнаяРазница КАК ВременнаяРазница,
