...
Функция ПараметрыПодписания(Объект, КлючСтрокиСубъекта, НастройкиКомплекта) Экспорт
	...
	ПараметрыПодписания.Вставить("ДобавитьМеткуДоверенногоВремени",	
		НастройкиКомплекта.ДобавитьМеткуДоверенногоВремени = Истина);
...
Функция ГотовностьКОтправке(ПараметрыПроверки, ТолькоПроверка, Прогрессор) Экспорт
	...
	Пока ДанныеДляПроверки.Следующий() Цикл
		...
		ПараметрыФайлаОтчета = ЗаполнениеФинОтчетностиВБанки.НовыеПараметрыИмениФайлаОтчета();
		...
		Отчет.ПредставлениеXML = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
					ИмяФайлаСРасширениемXML,
...
Процедура ПодготовитьАрхивВФоне(Параметры, АдресХранилища, АдресАрхива) Экспорт
	...
	Результат = ВыгрузитьФайлыПакета(Параметры.Предмет, Параметры.ФорматыСохранения, Прогрессор);
	Если Результат.Выполнено Тогда
		
		// Добавляем опись файлов, которые передаются в банк.
		СформироватьОписьОтправляемойОтчетности(Параметры, Результат, Истина);
...
Функция ИмяФайлаОтчетаБезРасширения(ВыборкаОтчетность, ПараметрыЗаполнения, РезультатЗаполнения)
	...
	ИмяФайлаБезРасширения = ЗаполнениеФинОтчетностиВБанки.ПолучитьИмяФайлаФинОтчетностиВБанки(ПараметрыФайлаОтчета);
	...
	ИначеЕсли ЗаполнениеФинОтчетностиВБанкиКлиентСервер.ЭтоОтчетБРО(ДанныеДляПроверки.ВидОтчета) Тогда
		...
		Если НужнаВыгрузка Тогда // для проверки нужно выгрузить отчет в файл
			...
			Если ПустаяСтрока(РезультатВыгрузкиОтчета.ОписаниеОшибки) Тогда
				...
				Если ПараметрыПроверки.ФорматИменованияФайлов = ЗаполнениеФинОтчетностиВБанки.СокращенныйПериодБезУИД()
					...
					ИмяФайлаБазРасширения = ЗаполнениеФинОтчетностиВБанки.ПолучитьИмяФайлаФинОтчетностиВБанки(
							ПараметрыПроверки.ПараметрыФайлаОтчета);
					
					РезультатВыгрузкиОтчета.ИмяФайлаВыгрузки = ИмяФайлаБазРасширения + "." + РасширениеФайла;
					
				КонецЕсли;
					
				РезультатВыгрузкиОтчета.ИмяФайлаВыгрузки = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
...
Процедура ГотовностьКОтправкеПрисоединенногоФайла(ДанныеДляПроверки, ПараметрыПроверки, РезультатПроверки)
	...
	ПредставлениеОтчета = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
		ИмяФайлаВыгрузки,
		ПараметрыПроверки.ФорматИменованияФайлов,
		ПараметрыПроверки.ПрефиксФайла,
		ПараметрыПроверки.УникальныеИменаФайловВПакете);
...
Функция ВыгрузитьФайлыПакета(ФинОтчет, ФорматыСохранения, Прогрессор)
	...
	СокращенныйПериодБезУИД = 
		(НастройкиКомплекта.ФорматИменованияФайлов
		= ЗаполнениеФинОтчетностиВБанки.СокращенныйПериодБезУИД());
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ОтчетСсылка) Тогда
			Если ЗначениеЗаполнено(Выборка.ФайлОтчета)
			   И Выборка.ВерсияДанныхСсылка = Выборка.ВерсияДанныхОтчетность Тогда
				...
				ИмяФайла = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
					ИмяФайлаВыгрузки,
					НастройкиКомплекта.ФорматИменованияФайлов,
					НастройкиКомплекта.ПрефиксФайла,
					УникальныеИменаФайловВПакете);
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.ФайлОтчета) Тогда // выгрузим присоединенный файл
			...
			Если ИсходноеИмяXML <> Неопределено Тогда
				...
				УнифицированноеИмяФайлаXML = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
					ИсходноеИмяXML,
					НастройкиКомплекта.ФорматИменованияФайлов,
					НастройкиКомплекта.ПрефиксФайла,
					УникальныеИменаФайловВПакете);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ОтчетСсылка) Тогда // выгрузить рег.отчет и подписать результат выгрузки
			...
			Если ПустаяСтрока(РезультатВыгрузкиОтчета.ОписаниеОшибки) Тогда
				...
				Если НастройкиКомплекта.ФорматИменованияФайлов	= ЗаполнениеФинОтчетностиВБанки.СокращенныйПериодБезУИД() 
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.ФайлОтчета) Тогда // подписать присоединенный файл
			...
			ИнформацияОФайле.ИмяФайла = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
				ИмяФайлаВыгрузки,
				НастройкиКомплекта.ФорматИменованияФайлов,
				НастройкиКомплекта.ПрефиксФайла,
				УникальныеИменаФайловВПакете);
			...
			Если ПодписатьФайлОтчетаXML Тогда
				...
				ИнформацияОФайлеXML.ИмяФайла = ЗаполнениеФинОтчетностиВБанки.УнифицироватьИмяФайлаКВыгрузке(
...
Функция СформироватьОписьОтправляемойОтчетности(ПараметрыПодписания, РезультатПодготовки, СохранитьВФайл = Ложь)
	...
	Для каждого ОписаниеОтправки Из ДанныеОписи Цикл
		...
		Если СохранитьВФайл Тогда
			...
			ОписаниеОтправки.ДанныеФайла.Записать(РезультатПодготовки.Каталог + ОписаниеОтправки.ИмяФайла);
