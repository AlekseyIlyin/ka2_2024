...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса) Экспорт
	...
	//++ НЕ УТ
	ВнеоборотныеАктивыСлужебный.ТекстЗапросаТаблицаВтПорядокУчетаОС(ТекстыЗапроса, "Документ.ПрочееОприходованиеТоваров");
	ВнеоборотныеАктивыСлужебный.ТекстЗапросаТаблицаВтМестонахождениеОС(ТекстыЗапроса, "Документ.ПрочееОприходованиеТоваров");
	//-- НЕ УТ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "11.5.11.25";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("109ee490-304a-447f-8b5d-b1b3b4465928");
	...
	|- заполняет новый реквизит ""Дата завершения эксплуатации""'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") Тогда
		...
		|	Документ.ПрочееОприходованиеТоваров.Товары КАК ТабличнаяЧасть
		|ГДЕ
		|	ТабличнаяЧасть.ДатаЗавершенияЭксплуатации = ДАТАВРЕМЯ(1, 1, 1)
		|	И ЕСТЬNULL(ТабличнаяЧасть.Партия.ДатаЗавершенияЭксплуатации, ДАТАВРЕМЯ(1, 1, 1)) <> ДАТАВРЕМЯ(1, 1, 1)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратИзЭксплуатации
				И ЭтоУправлениеТорговлей Тогда
				Для Каждого ДанныеСтроки Из ДокументОбъект.Товары Цикл
					Если ДанныеСтроки.ДатаЗавершенияЭксплуатации = '000101010000'
						И ЗначениеЗаполнено(ДанныеСтроки.Партия) Тогда
						ДанныеСтроки.ДатаЗавершенияЭксплуатации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСтроки.Партия, "ДатаЗавершенияЭксплуатации");
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
