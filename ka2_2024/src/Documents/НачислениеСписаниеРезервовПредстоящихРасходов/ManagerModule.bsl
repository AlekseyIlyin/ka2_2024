...
Функция ТекстЗапросаТаблицаВтСтоимостьОС(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	РегистрНакопления.СтоимостьОС.Остатки(&НачалоПериода, &ЭтоРезервыЛиквидационныхОбязательств
...
Функция ТекстЗапросаТаблицаВтАмортизацияОС(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	РегистрНакопления.АмортизацияОС.Остатки(&НачалоПериода, &ЭтоРезервыЛиквидационныхОбязательств
	|	И (Организация, ОсновноеСредство) В
	|		(ВЫБРАТЬ
	|			&Организация,
	|			Т.АналитикаРасходов
...
Функция ТекстЗапросаТаблицаВтСоставЕГДС(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	РегистрСведений.СоставЕГДС.СрезПоследних(&НачалоПериода, &ЭтоРезервыЛиквидационныхОбязательств
...
Функция ТекстЗапросаТаблицаВтОбесценениеВНА(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	РегистрНакопления.ОбесценениеВНА.Остатки(&НачалоПериода, &ЭтоРезервыЛиквидационныхОбязательств
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("333ab2ed-59bb-11ed-9ece-3c7c3fd89daa");
	...
	Обработчик.Комментарий = НСтр("ru = 'В документе ""Начисление и списание резервов предстоящих расходов"" заполняет данные по налоговому учету'");
	...
	Блокируемые.Добавить(Метаданные.Документы.НачислениеСписаниеРезервовПредстоящихРасходов.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.НачислениеСписаниеРезервовПредстоящихРасходов";
	...
	|ИЗ
	|	Документ.НачислениеСписаниеРезервовПредстоящихРасходов.Резервы КАК Резервы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ОбъектыУчетаРезервовПредстоящихРасходов КАК ОбъектыУчетаРезервовПредстоящихРасходов
	|		ПО Резервы.ОбъектУчетаРезервов = ОбъектыУчетаРезервовПредстоящихРасходов.Ссылка
	|ГДЕ
	|	НЕ ОбъектыУчетаРезервовПредстоящихРасходов.ДопускаетсяРазличноеФормированиеРезервовБУиНУ
	|	И (Резервы.ТекущаяСуммаРегл <> Резервы.ТекущаяСуммаНУ
	|	ИЛИ Резервы.НоваяСуммаРегл <> Резервы.НоваяСуммаНУ)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.НачислениеСписаниеРезервовПредстоящихРасходов";
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыРезервов.Ссылка КАК Ссылка,
	|	ДокументыРезервов.ВерсияДанных КАК ВерсияДанных,
	|	ДокументыРезервов.Резервы.(
	|		ОбъектУчетаРезервов,
	|		ОбъектУчетаРезервов.ДопускаетсяРазличноеФормированиеРезервовБУиНУ КАК ДопускаетсяРазличноеФормированиеРезервовБУиНУ)
	|ИЗ
	|	Документ.НачислениеСписаниеРезервовПредстоящихРасходов КАК ДокументыРезервов
	|ГДЕ
	|	ДокументыРезервов.Ссылка В
	|		(ВЫБРАТЬ
	|			ВТСсылкиДляОбработки.Ссылка КАК Ссылка
	|		ИЗ
	|			ВТСсылкиДляОбработки)";
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			Блокировка.Добавить(ПолноеИмяОбъекта).УстановитьЗначение("Ссылка", ВыборкаДокументов.Ссылка);
			Блокировка.Заблокировать();
			
			Объект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(ВыборкаДокументов.Ссылка,
			...
			Если Объект <> Неопределено Тогда
				...
				РезультатЗапросаРезервы = ВыборкаДокументов.Резервы; //РезультатЗапроса
				Выборка = РезультатЗапросаРезервы.Выбрать();
				Пока Выборка.Следующий() Цикл
					
					ПараметрыОтбора = Новый Структура("ОбъектУчетаРезервов", Выборка.ОбъектУчетаРезервов);
					СтрокиДляОбработки = Объект.Резервы.НайтиСтроки(ПараметрыОтбора);
					
					Для каждого СтрокаРезерва Из СтрокиДляОбработки Цикл
						Если Выборка.ДопускаетсяРазличноеФормированиеРезервовБУиНУ Тогда
							Продолжить;
						КонецЕсли;
						Если СтрокаРезерва.ТекущаяСуммаНУ <> СтрокаРезерва.ТекущаяСуммаРегл Тогда
							СтрокаРезерва.ТекущаяСуммаНУ = СтрокаРезерва.ТекущаяСуммаРегл;
							ОбъектИзменен = Истина;
						КонецЕсли;
						Если СтрокаРезерва.НоваяСуммаНУ <> СтрокаРезерва.НоваяСуммаРегл Тогда
							СтрокаРезерва.НоваяСуммаНУ = СтрокаРезерва.НоваяСуммаРегл;
							СтрокаРезерва.УстановитьНовуюСуммуНУ = СтрокаРезерва.УстановитьНовуюСуммуРегл;
							ОбъектИзменен = Истина;
						КонецЕсли;
					КонецЦикла;
				КонецЦикла;
