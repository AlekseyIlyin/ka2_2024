...
Процедура УправлениеФормой()
	...
	ДоступнаОтправкаУведомленийПоНДФЛЗаПервуюПоловинуПериода =
		ДоступнаОтправкаУведомленийПоНДФЛЗаПервуюПоловинуПериода(Объект.Дата);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Заполнить",
		"Видимость",
		НЕ ДоступнаОтправкаУведомленийПоНДФЛЗаПервуюПоловинуПериода);
...
&НаСервереБезКонтекста
Функция ДоступнаОтправкаУведомленийПоНДФЛЗаПервуюПоловинуПериода(Знач ДатаУведомления)
	
	Если Не ЗначениеЗаполнено(ДатаУведомления) Тогда
	...
	Если ЗначениеЗаполнено(НачалоПередачиЧастичныхУведомленийПоНДФЛ)
		И Год(ДатаУведомления) = 2023 И ДатаУведомления >= НачалоПередачиЧастичныхУведомленийПоНДФЛ Тогда
		// Согласно поправкам к закону 263-ФЗ, предоставление уведомлений по НДФЛ дважды в месяц возможно в течение 2023 года
		// с момента вступления в силу по истечении одного месяца со дня их официального опубликования
		Возврат Истина;
	КонецЕсли;
