...
Процедура УправлениеДоступностьюКнопокОплаты(Форма, ИнформацияОбОплате = Неопределено)
	...
	ОплатаНачатаЧекНеПробит = (Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЧековККМ.ТоварЗарезервирован") 
									И ИнформацияОбОплате.ИтогоОплачено <> 0);
									
	Элементы.ОплатитьНаличными.Доступность            = Не ОплатаНачатаЧекНеПробит;
	Элементы.ОплатитьКартой.Доступность               = Не ОплатаНачатаЧекНеПробит;
	Элементы.ОплатитьПодарочнымСертификатом.Доступность = Не ОплатаНачатаЧекНеПробит;
...
Процедура НастроитьРМКЛокализация(ДоступныеВидыОплаты, ЭлементыНаПерегруппировку)
	...
	СуммаСБПВыполнена = СуммаОплатыСБППоДокументу(Объект.ОплатаПлатежнымиКартами);
	СуммаСБПОжидание = СуммаОжиданиеОплатыСБППоДокументу(Объект.ОплатаПлатежнымиКартами);
...
Процедура УправлениеДоступностьюКнопокОплатыЛокализация(Форма, ИнформацияОбОплате)
	...
	Элементы = Форма.Элементы;
	Элементы.ОплатитьСБП.Доступность = Не ОплатаНачатаЧекНеПробит;
	Элементы.ОплатитьЭСФСС.Доступность = Не ОплатаНачатаЧекНеПробит И Форма.ЕстьТоварыСВозможностьюОплатыЭС;
...
Функция СуммаОплатыСБППоДокументу(ЗНАЧ ОплатыПлатежнойКартой)
	...
	Возврат РозничныеПродажиЛокализация.СуммаОплатыСБППоДокументу(ОплатыПлатежнойКартой.Выгрузить(), Истина);
	
КонецФункции

&НаСервереБезКонтекста
Функция СуммаОжиданиеОплатыСБППоДокументу(ЗНАЧ ОплатыПлатежнойКартой)
