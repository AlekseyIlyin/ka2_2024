...
Функция ОписаниеДействующихПараметров(Организация, ДатаДействия = Неопределено, ДействующиеНастройки = Неопределено) Экспорт
	...
	ЗаголовокСтавки = НСтр("ru='Ставка налога для движимого имущества, принятого на учет с 1 января 2013 года'");
	Если ДействующиеНастройки.СнижениеНалоговойСтавкиДвижимоеИмущество Тогда
		ЗаголовокСтавки = ЗаголовокСтавки + " " + НСтр("ru='(сниженная)'");
	КонецЕсли;
	СтрокаОписанияНастроек = СтрокаОписанияНастроек + СтрШаблон(СтрокаШаблон, 
		ЗаголовокСтавки,
		Строка(ДействующиеНастройки.НалоговаяСтавкаДвижимоеИмущество) + "%");
