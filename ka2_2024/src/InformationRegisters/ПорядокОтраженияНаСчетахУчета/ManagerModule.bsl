...
Функция СписокСчетовУчетаПоРазделуУчета(РазделУчета) Экспорт
	...
	Если РазделУчета = "ВнеоборотныеАктивы" Тогда
		СчетаУчетаРегл.Добавить("СтоимостьВНА", НСтр("ru = 'Стоимость'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4"));
		СчетаУчетаРегл.Добавить("АмортизацияВНА", НСтр("ru = 'Амортизация'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4"));
		СчетаУчетаРегл.Добавить("РезервДооценкиВНА", НСтр("ru = 'Резерв дооценки'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4"));
		СчетаУчетаРегл.Добавить("ВыбытиеВНА", НСтр("ru = 'Выбытие'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4"));
		СчетаУчетаРегл.Добавить("СтоимостьВНА_ЦФ", НСтр("ru = 'Стоимость'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") И ПолучитьФункциональнуюОпцию("ИспользоватьУчетВнеоборотныхАктивовПоНаправлениямДеятельности"));
		СчетаУчетаРегл.Добавить("АмортизацияВНА_ЦФ", НСтр("ru = 'Амортизация'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") И ПолучитьФункциональнуюОпцию("ИспользоватьУчетВнеоборотныхАктивовПоНаправлениямДеятельности"));
		СчетаУчетаРегл.Добавить("ЗабалансовыйУчетВНА", НСтр("ru = 'Залоговая стоимость'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4"));
		СчетаУчетаРегл.Добавить("ОбесценениеВНА", НСтр("ru = 'Обесценение'"), ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") И ПолучитьФункциональнуюОпцию("ИспользоватьОбесценениеВНА"));
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение новых видов счетов НДС с авансов (выданных/полученных) и резервов по сомнительным долгам значениями по умолчанию.
	|Для всех настроек исключений, сделанных по пустой организации, заполняем организацию (если она одна).'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ЗарегистрироватьНДССАвансовИРезервыПоСомнительнымДолгам(ТекстыЗапросов);
	ЗарегистрироватьНастройкиИсключенийПоПустойОрганизации(ТекстыЗапросов);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	#Область НДССАвансовИРезервыПоСомнительнымДолгам
	ТекстЗапроса =
	"ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ДанныеДляОбработки.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыПолученные)
	|		ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСпоАвансамИПредоплатам)
	|		КОГДА ДанныеДляОбработки.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыВыданные)
	|		ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСпоАвансамИПредоплатамВыданным)
	|		КОГДА ДанныеДляОбработки.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РезервыПоСомнительнымДолгам)
	|		ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РезервыПоСомнительнымДолгам)
	...
	#Область НастройкиИсключенийПоПустойОрганизации
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
		// Исправление ошибки 00-00499206, когда для ИБ с одной организацией сохранялись исключения по пустой организации.
		...
		|		ПО Организации.ПометкаУдаления = ЛОЖЬ
		...
		|		И ПорядокОтраженияНаСчетахУчета.МестоУчета = ДанныеДляОбработки.МестоУчета
		|		И ПорядокОтраженияНаСчетахУчета.Организация = ДанныеДляОбработки.Организация
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК НастройкиИсключенийПоОрганизации
		|		ПО ПорядокОтраженияНаСчетахУчета.ВидСчета = ДанныеДляОбработки.ВидСчета
		|		И ПорядокОтраженияНаСчетахУчета.АналитикаУчета = ДанныеДляОбработки.АналитикаУчета
		|		И ПорядокОтраженияНаСчетахУчета.МестоУчета = ДанныеДляОбработки.МестоУчета
		|		И ПорядокОтраженияНаСчетахУчета.Организация = Организации.Ссылка
		|		И ПорядокОтраженияНаСчетахУчета.Долгосрочный = ПорядокОтраженияНаСчетахУчета.Долгосрочный
		|ГДЕ
		|	ДанныеДляОбработки.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|	И ДанныеДляОбработки.МестоУчета <> НЕОПРЕДЕЛЕНО
		|	И ЕСТЬNULL(НастройкиИсключенийПоОрганизации.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)";
		ТекстыЗапроса.Добавить(ТекстЗапроса);
...
Процедура ДополнитьТекстЗапросаОперацийДаннымиРасчетовСПартнерами(СписокРазделов, МассивТекстовЗапросаДляДобавления)
	...
	|	ЛОЖЬ КАК ЕстьРасчетыПоПроцентам,
	|	ЛОЖЬ КАК ЕстьНеотфактурованныеПоставки,
	|	ДанныеРегистра.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.УслугаПоАренде) КАК ЕстьЛизинговыеУслуги,
...
Процедура ЗарегистрироватьНДССАвансовИРезервыПоСомнительнымДолгам(ТекстыЗапросов)
	
	// Заполнение новых счетов НДСАвансыПолученные счетом по умолчанию
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыПолученные) КАК ВидСчета,
	|	ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Организация,
	|	ГФУ.Ссылка КАК АналитикаУчета,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчета
	|ИЗ
	|	Справочник.ГруппыФинансовогоУчетаРасчетов КАК ГФУ
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ЗаполненныйПорядокОтраженияНаСчетахУчета
	|		ПО ЗаполненныйПорядокОтраженияНаСчетахУчета.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыПолученные)
	|			И ЗаполненныйПорядокОтраженияНаСчетахУчета.АналитикаУчета = ГФУ.Ссылка
	|ГДЕ
	|	ГФУ.РасчетыСКлиентами
	|	И ЗаполненныйПорядокОтраженияНаСчетахУчета.СчетУчета ЕСТЬ NULL";
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ЗаполненныйПорядокОтраженияНаСчетахУчета
	|		ПО ЗаполненныйПорядокОтраженияНаСчетахУчета.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыПолученные)
	|			И ЗаполненныйПорядокОтраженияНаСчетахУчета.АналитикаУчета = ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаРасчетов.ПустаяСсылка)
	|ГДЕ
	|	ЗаполненныйПорядокОтраженияНаСчетахУчета.СчетУчета ЕСТЬ NULL";
	ТекстыЗапросов.Добавить(ТекстЗапроса);
	
	// Заполнение новых счетов НДСАвансыВыданные счетом по умолчанию
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыВыданные) КАК ВидСчета,
	|	ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Организация,
	|	ГФУ.Ссылка КАК АналитикаУчета,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчета
	|ИЗ
	|	Справочник.ГруппыФинансовогоУчетаРасчетов КАК ГФУ
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ЗаполненныйПорядокОтраженияНаСчетахУчета
	|		ПО ЗаполненныйПорядокОтраженияНаСчетахУчета.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСАвансыВыданные)
	|			И ЗаполненныйПорядокОтраженияНаСчетахУчета.АналитикаУчета = ГФУ.Ссылка
	|ГДЕ
	|	(ГФУ.РасчетыСПоставщиками ИЛИ ГФУ.РасчетыСКомитентамиПоЗакупке ИЛИ ГФУ.РасчетыСАрендодателями)
	|	И ЗаполненныйПорядокОтраженияНаСчетахУчета.СчетУчета ЕСТЬ NULL";
	ТекстыЗапросов.Добавить(ТекстЗапроса);
	...
	// Заполнение новых счетов РезервыПоСомнительнымДолгам счетом по умолчанию
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РезервыПоСомнительнымДолгам) КАК ВидСчета,
	|	ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Организация,
	...
	|	Справочник.ГруппыФинансовогоУчетаРасчетов КАК ГФУ
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ЗаполненныйПорядокОтраженияНаСчетахУчета
	|		ПО ЗаполненныйПорядокОтраженияНаСчетахУчета.ВидСчета = ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РезервыПоСомнительнымДолгам)
	|			И ЗаполненныйПорядокОтраженияНаСчетахУчета.АналитикаУчета = ГФУ.Ссылка
	|ГДЕ
	|	НЕ ГФУ.РасчетыСКредиторами
	|	И ЗаполненныйПорядокОтраженияНаСчетахУчета.СчетУчета ЕСТЬ NULL";
	ТекстыЗапросов.Добавить(ТекстЗапроса);
...
// Исправление ошибки 00-00499206, когда для ИБ с одной организацией сохранялись исключения по пустой организации.
Процедура ЗарегистрироватьНастройкиИсключенийПоПустойОрганизации(ТекстыЗапросов)
	...
	|	РегистрСведений.ПорядокОтраженияНаСчетахУчета КАК ПорядокОтраженияНаСчетахУчета
	|ГДЕ
	|	ПорядокОтраженияНаСчетахУчета.МестоУчета <> НЕОПРЕДЕЛЕНО
