...
Функция ТекстЗапросаРасчетов()
	...
	|		СУММА(0) КАК ВозвратОплатыБезОбъектаРасчетовВозврат
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		...
		"СУММА(ВЫБОР 
		|			КОГДА РасчетыСКлиентами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|				И РасчетыСКлиентами.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту)
		|				И РасчетыСКлиентами.Регистратор ССЫЛКА Документ.ОперацияПоЯндексКассе
		|				ТОГДА РасчетыСКлиентами.Сумма
		|			ИНАЧЕ 0
		|		КОНЕЦ)");
