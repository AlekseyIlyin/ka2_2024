...
Процедура ОтправитьНаМодерацию(Команда)  
	...
	СтруктураЗаписи = Новый Структура("Номенклатура, Характеристика, Упаковка, Статус");
...
Процедура ОбновитьПредставлениеТовара()
	...
	Если Запись.Номенклатура<>Справочники.Номенклатура.ПустаяСсылка() Тогда
		...
		БазовыйПрайсЛист = ИнтеграцияСЯндексМаркетСервер.ДанныеУчетнойЗаписиЯндексМаркет().ЦенаПродажи; 
...
Процедура ОбновитьДопИнфо()
	...
	Если Запись.ЦенаПродажи <> "" Тогда 		
		...
		|'");
		БазоваяЦена = СтрШаблон(ШаблонСообщения, Запись.ЦенаПродажи);
	Иначе	
...
Функция ЗаписьСуществует(СтруктураЗаписи)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	                ...
	                |ГДЕ
	                |	СтатусыПубликацииТоваровЯндексМаркет.Номенклатура = &Номенклатура
