...
Процедура ОбработатьНастройкуСчетовОСВ(РезультатНастройки, ДополнительныеПараметры) Экспорт
	...
	ЗапланироватьОтправкиПредшествующихПериодов(Организация, Отметки);
...
&НаСервере
Процедура ЗапланироватьОтправкиПредшествующихПериодов(Организация, Отметки)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЖурналПереданнойОтчетностиВБанк.ИмяПакета КАК ИмяПакета
		|ИЗ
		|	РегистрСведений.ЖурналПереданнойОтчетностиВБанк КАК ЖурналПереданнойОтчетностиВБанк
		|ГДЕ
		|	ЖурналПереданнойОтчетностиВБанк.Организация = &Организация
		|	И ЖурналПереданнойОтчетностиВБанк.ВидОтчета = ""ОСВПоСчетам""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЖурналПередачиОтчетностиВБанк.ИмяПакета
		|ИЗ
		|	РегистрСведений.ЖурналПередачиОтчетностиВБанк КАК ЖурналПередачиОтчетностиВБанк
		|ГДЕ
	...
	Если ДатаПодключения >= Дата(ОтчетныйГод, Январь, 1)
	   И ДатаПодключения <= КонецДня(Дата(ОтчетныйГод, Май, 8)) Тогда
		...
		ДатаПредшествующейОтправки = Дата(ОтчетныйГод - 1, Ноябрь, 9); // Отправка: 9 ноября – 9 декабря
		НачалоПредшествующегоКвартала = Дата(ОтчетныйГод - 1, 7, 1); // 3 квартал
	...
	ИначеЕсли ДатаПодключения >= Дата(ОтчетныйГод, Май, 8)
		...
		ДатаПредшествующейОтправки = Дата(ОтчетныйГод, Апрель, 7); // Отправка: 7 апреля – 7 мая
		НачалоПредшествующегоКвартала = Дата(ОтчетныйГод - 1, 10, 1); // 4 квартал
	...
	ИначеЕсли ДатаПодключения >= Дата(ОтчетныйГод, Июнь, 12)
		...
		ДатаПредшествующейОтправки = Дата(ОтчетныйГод, Май, 11); // Отправка: 11 мая – 11 июня
		НачалоПредшествующегоКвартала = Дата(ОтчетныйГод, 1, 1); // 1 квартал
	...
	ИначеЕсли ДатаПодключения >= Дата(ОтчетныйГод, Сентябрь, 6)
		...
		ДатаПредшествующейОтправки = Дата(ОтчетныйГод, Август, 5); // Отправка: 5 августа – 5 сентября
		НачалоПредшествующегоКвартала = Дата(ОтчетныйГод, 4, 1); // 2 квартал
	...
	ИначеЕсли ДатаПодключения >= Дата(ОтчетныйГод, Декабрь, 10)
		...
		ДатаПредшествующейОтправки = Дата(ОтчетныйГод, Ноябрь, 9); // Отправка: 9 ноября – 9 декабря
		НачалоПредшествующегоКвартала = Дата(ОтчетныйГод, 7, 1); // 3 квартал
