...
Процедура ПереустановитьКомпонентыПечатиМашиночитаемыхФормПослеПодключенияРасширения(Подключено, ПараметрыСохранения) Экспорт
	Если Подключено Тогда
		...
		КаталогКомпонентыМЧБ = КаталогКомпонентыМЧБ + ПолучитьРазделительПути();
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Движение средств по счету в банке за пределами РФ"
		И ОписаниеРегОтчета(Ссылка).ВыбраннаяФорма = "ФормаОтчета2023Кв4" Тогда
		ПоказатьПредупреждение(, НСтр(
		"ru='Печать со штрихкодом PDF417 формы отчета
		|в ред. постановления Правительства РФ
		|от 14.11.2023 № 1911 будет реализована
...
Процедура ВходящиеТребованияПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для Каждого СтрокаСписка Из Строки Цикл
		...
		// Ответ
		ЗадатьСтатусОтветаНаТребование(СтрокаСписка, ЕстьОплата);
		Если НЕ СтрокаСписка.Значение.Данные.Ссылка = Неопределено Тогда
			ЗадатьДатуОтветаНаТребование(СтрокаСписка, Сегодня, ЕстьОплата);
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ЗадатьДатуОтветаНаТребование(СтрокаСписка, Сегодня, ПоддерживаетсяОплата)
...
&НаСервереБезКонтекста
Процедура ЗадатьСтатусОтветаНаТребование(СтрокаСписка, ПоддерживаетсяОплата)
