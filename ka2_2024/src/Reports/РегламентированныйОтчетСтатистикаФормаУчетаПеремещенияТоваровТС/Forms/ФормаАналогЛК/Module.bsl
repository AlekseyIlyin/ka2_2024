...
Процедура Инициализация(БезОткрытияФормы = Ложь, Знач ДокОснование = Неопределено) Экспорт
	...
	Попытка
		МодульУчетНДС = ОбщегоНазначения.ОбщийМодуль("УчетНДС");
		ДанныеЗаявленияОВвозе = МодульУчетНДС.ТаблицыСтатистическойФормыУчетаПеремещенияТоваров(Новый Массив, "ИМ");
		СтруктураРеквизитовФормы.Вставить("ЗаполнениеНаОснованииРеализацииИВвоза", Истина);
	Исключение
	...
	Попытка
		...
		СтруктураРеквизитовФормы.Вставить("ЗаполнениеНаОснованииВвоза", Истина);
	Исключение
	...
	Попытка
		Если ЗначениеЗаполнено(ДокОснование) Тогда
			...
			Если Графа4 = 1 Тогда
				ДанныеПервичногоДокумента = МодульУчетНДС.ТаблицыСтатистическойФормыУчетаПеремещенияТоваров(МассивДокументов, "ИМ");
			Иначе
				ДанныеПервичногоДокумента = МодульУчетНДС.ТаблицыСтатистическойФормыУчетаПеремещенияТоваров(МассивДокументов, "ЭК");
			КонецЕсли;
...
Процедура ЗаполнитьДаннымиПервичногоДокумента(Ссылка)
	Попытка
		...
		Если Графа4 = 1 Тогда
			...
			Иначе
				МодульЗаявлениеОВвозеТоваров = ОбщегоНазначения.ОбщийМодуль("Документы.ЗаявлениеОВвозеТоваров");
				ДанныеПервичногоДокумента = МодульЗаявлениеОВвозеТоваров.ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(МассивДокументов);
			...
			КонецЕсли;
		Иначе
			ТекстСообщения = ТекстСообщения + "реализации";
			МодульУчетНДС = ОбщегоНазначения.ОбщийМодуль("УчетНДС");
			ДанныеПервичногоДокумента = МодульУчетНДС.ТаблицыСтатистическойФормыУчетаПеремещенияТоваров(МассивДокументов, "ЭК");
		КонецЕсли;
