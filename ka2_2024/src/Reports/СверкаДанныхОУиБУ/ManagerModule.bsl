...
Функция ТекстЗапросаСебестоимость()
	...
	|		СебестоимостьТоваров.РезервПодОбесценениеРегл <> 0 КАК Резерв,
	|		ВЫБОР
	|			КОГДА НЕ СебестоимостьТоваров.ХозяйственнаяОперация В (
	//++ Устарело_Переработка24
	...
	|		КОГДА НЕ ЕСТЬNULL(Склады.ЦеховаяКладовая, ЛОЖЬ) И Аналитика.ТипМестаХранения <> ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Подразделение) И ДанныеСебестоимости.РазделУчета В
	...
	|		КОГДА (ЕСТЬNULL(Склады.ЦеховаяКладовая, ЛОЖЬ) ИЛИ Аналитика.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Подразделение)) И ДанныеСебестоимости.РазделУчета В
	|			(ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаОтветхранение), ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажи))
	...
	|		КОГДА (ЕСТЬNULL(Склады.ЦеховаяКладовая, ЛОЖЬ) ИЛИ Аналитика.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Подразделение))
	|			И ДанныеСебестоимости.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку)
