...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2023_1(Данные, УникальныйИдентификатор)
	...
	Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.СведРасхУсл Цикл 
		...
		Если Не ЗначениеЗаполнено(СведРасхУсл.НомерСвед) Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СведРасхУсл.НомерСвед) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан / неправильно указан номер сведений", "СведРасхУсл", "НомерСвед", СведРасхУсл.УИД));
		КонецЕсли;
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СведРасхУсл.Фамилия) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана фамилия", "СведРасхУсл", "Фамилия", СведРасхУсл.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СведРасхУсл.Имя) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указано имя", "СведРасхУсл", "Имя", СведРасхУсл.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СведРасхУсл.ДатаРожд) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указана дата рождения", "СведРасхУсл", "ДатаРожд", СведРасхУсл.УИД));
		КонецЕсли;
		УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(СведРасхУсл, ТаблицаОшибок, "СведРасхУсл", "ИННФЛ", Ложь, Ложь);
		Если Не ЗначениеЗаполнено(СведРасхУсл.ИННФЛ)
			Или УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(СведРасхУсл, "КодВидДок,СерНомДок,ДатаДок") Тогда 
			Если Не ЗначениеЗаполнено(СведРасхУсл.КодВидДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указан код документа", "СведРасхУсл", "КодВидДок", СведРасхУсл.УИД));
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СведРасхУсл.СерНомДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указаны серия/номер документа", "СведРасхУсл", "СерНомДок", СведРасхУсл.УИД));
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СведРасхУсл.ДатаДок) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указана дата выдачи документа", "СведРасхУсл", "ДатаДок", СведРасхУсл.УИД));
			КонецЕсли;
		КонецЕсли;
		
		Если СведРасхУсл.ПрПациент = "0" Тогда 
			Если Не ЗначениеЗаполнено(СведРасхУсл.ФамилияПолучатель) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указана фамилия", "СведРасхУсл", "ФамилияПолучатель", СведРасхУсл.УИД));
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СведРасхУсл.ИмяПолучатель) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указано имя", "СведРасхУсл", "ИмяПолучатель", СведРасхУсл.УИД));
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СведРасхУсл.ДатаРождПолучатель) Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"Не указана дата рождения", "СведРасхУсл", "ДатаРождПолучатель", СведРасхУсл.УИД));
			КонецЕсли;
			УведомлениеОСпецрежимахНалогообложения.ПроверкаИНН(СведРасхУсл, ТаблицаОшибок, "СведРасхУсл", "ИННФЛПолучатель", Ложь, Ложь);
			Если Не ЗначениеЗаполнено(СведРасхУсл.ИННФЛПолучатель)
				Или УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(СведРасхУсл, "КодВидДокПолучатель,СерНомДокПолучатель,ДатаДокПолучатель") Тогда 
				
				Если Не ЗначениеЗаполнено(СведРасхУсл.КодВидДокПолучатель) Тогда 
					ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
						"Не указан код документа", "СведРасхУсл", "КодВидДокПолучатель", СведРасхУсл.УИД));
				КонецЕсли;
				Если Не ЗначениеЗаполнено(СведРасхУсл.СерНомДокПолучатель) Тогда 
					ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
						"Не указаны серия/номер документа", "СведРасхУсл", "СерНомДокПолучатель", СведРасхУсл.УИД));
				КонецЕсли;
				Если Не ЗначениеЗаполнено(СведРасхУсл.ДатаДокПолучатель) Тогда 
					ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
						"Не указана дата выдачи документа", "СведРасхУсл", "ДатаДокПолучатель", СведРасхУсл.УИД));
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли СведРасхУсл.ПрПациент = "1" Тогда 
			Если УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(СведРасхУсл, 
				...
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
					"При признаке ""1"" сведения кому оказаны услуги не заполняются", "СведРасхУсл", "ФамилияПолучатель", СведРасхУсл.УИД));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПроверитьСправкуСВыводомВТаблицу_Форма2023_1(Данные, УникальныйИдентификатор)
	...
	Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.СведРасхУсл Цикл 
		...
		Если Не ЗначениеЗаполнено(СведРасхУсл.НомерСвед) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан номер сведений", "СведРасхУсл", "НомерСвед", СведРасхУсл.УИД));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СведРасхУсл.ПрПациент) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан признак", "СведРасхУсл", "ПрПациент", СведРасхУсл.УИД));
...
Функция СформироватьСправку(Объект, УИДТекущаяСтраница) Экспорт
	...
	Для Каждого Стр Из СтруктураПараметров.ДанныеМногостраничныхРазделов.СведРасхУсл Цикл 
