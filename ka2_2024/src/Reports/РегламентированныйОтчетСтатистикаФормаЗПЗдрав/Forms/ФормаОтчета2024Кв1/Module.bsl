...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Квартал Тогда
		...
		СтрЭлементыФормы = "ФормаПроверитьКонтрольныеСоотношенияИзШаблона;ПроверитьВыгрузку;ПроверитьВИнтернете;
		|ОтправитьЧерезПредставителя;ОтправитьВКонтролирующийОрган;Выгрузить;Загрузить;Сравнить;Проверка;Отправка";
		
		МассивКнопок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЭлементыФормы, ";", Истина);
		Для Каждого ЭлМасс Из МассивКнопок Цикл
			Элемент = Элементы.Найти(ЭлМасс);
