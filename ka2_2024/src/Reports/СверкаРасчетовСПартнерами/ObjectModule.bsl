...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если КомпоновщикНастроек.Настройки.Отбор.Элементы.Количество() > 0 Тогда
		...
		Если (ТипЗнч(МассивДокументов) = Тип("ДокументСсылка.СверкаВзаиморасчетов")
				Или ТипЗнч(МассивДокументов) = Тип("ДокументСсылка.СверкаВзаиморасчетов2_5_11"))
			И ЗначениеЗаполнено(МассивДокументов) Тогда
...
Функция СверкиВзаиморасчетовОтчета()
	...
	Если ТаблицаСверокВзаиморасчетов.Колонки.Найти("СверкаВзаиморасчетов") <> Неопределено Тогда
		Для Каждого Строка Из ТаблицаСверокВзаиморасчетов Цикл
			Если ЗначениеЗаполнено(Строка.СверкаВзаиморасчетов) Тогда
				Результат.Добавить(Строка.СверкаВзаиморасчетов);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	Возврат Результат;
