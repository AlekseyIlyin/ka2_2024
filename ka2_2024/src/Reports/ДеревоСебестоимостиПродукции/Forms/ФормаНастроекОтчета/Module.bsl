...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""Настройки отчета"".'");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""Вариант наименование"".'");
	КонецЕсли;
	...
	Если ТипЗнч(ИдентификаторЭлементаСтруктурыНастроек) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		...
		Если Не Параметры.Свойство("ТипЭлементаСтруктурыНастроек", ТипЭлементаСтруктурыНастроек) Тогда
			ВызватьИсключение НСтр("ru = 'Не передан служебный параметр ""Тип элемента структуры настроек"".'");
		КонецЕсли;
	Иначе
...
Процедура ОбновитьПараметрыДанных()
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		НастроитьДоступныеЗначенияПоВариантуОтчета(НайденныйПараметр, Строка);

	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура НастроитьДоступныеЗначенияПоВариантуОтчета(ПараметрСКД, Строка)

	Если ПараметрСКД.Имя = "ДанныеПоСебестоимости" Тогда

		ДоступныеЗначения = Строка.ДоступныеЗначения;
		ДоступныеЗначения.Очистить();

		Если КлючТекущегоВарианта = "ДеревоСебестоимостиПродукцииПредприятия" Тогда

			ДоступныеЗначения.Добавить(1, НСтр("ru = 'В валюте упр. учета с НДС'"));
			ДоступныеЗначения.Добавить(2, НСтр("ru = 'В валюте упр. учета без НДС'"));

		ИначеЕсли КлючТекущегоВарианта = "ДеревоСебестоимостиПродукцииОрганизации" Тогда

			Если РасчетСебестоимостиПовтИсп.УправленческийУчетОрганизаций() Тогда
				ДоступныеЗначения.Добавить(3, НСтр("ru = 'В валюте упр. учета'"));
			КонецЕсли;
			ДоступныеЗначения.Добавить(4, НСтр("ru = 'В валюте регл. учета'"));
		...
		Если ДоступныеЗначения.НайтиПоЗначению(Строка.Значение) = Неопределено Тогда
			Строка.Значение = ДоступныеЗначения[0].Значение;
			Строка.ПредставлениеЗначения = ДоступныеЗначения[0].Представление;
		КонецЕсли;
