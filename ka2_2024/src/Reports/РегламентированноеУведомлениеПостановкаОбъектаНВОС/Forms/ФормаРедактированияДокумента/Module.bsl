...
Процедура ДобавитьФайлЗавершение(Результат, АдресФайла, ВыбИмяФайла, Парам) Экспорт
	...
	ПозицияПоследнегоРазделителя = СтрНайти(ВыбИмяФайла, ПолучитьРазделительПути(), 
		НаправлениеПоиска.СКонца, , ?(Прав(ВыбИмяФайла, 1) = ПолучитьРазделительПути(), 2, 1));
	Каталог = Лев(ВыбИмяФайла, ПозицияПоследнегоРазделителя);
...
Процедура СохранитьНаДискФайл()
	Попытка
		Если Не ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда 
			Возврат;
		КонецЕсли;
		НачатьПолучениеФайлаССервера(АдресДДФайла(), ИмяФайла, Новый ПараметрыДиалогаПолученияФайлов("Выберете папку для сохранения", Истина));
	Исключение
