...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтернетПоддержкаПодключена = ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.БазоваяФункциональностьБИП");
	Если ИнтернетПоддержкаПодключена Тогда 
		...
		ИнтернетПоддержкаПодключена = (ТипЗнч(БИПДанныеАутентификации) = Тип("Структура"));
	КонецЕсли;
	...
	Элементы.ГруппаПодписаниеИОтправка.Видимость = ПодачаДокументовВЭлектронномВидеДоступна;
...
Процедура УправлениеФормой(Форма)
	...
	Форма.Элементы.ГруппаПодписаниеИОтправка.Видимость = (Не Форма.УведомлениеЗаполненоВПомощнике);
...
Процедура ИныеДокументыНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	РегистрационныеФормыКлиент.ОткрытьПрисоединенныеФайлы(ЭтотОбъект, Элемент.Имя);
КонецПроцедуры
...
Процедура ПодписатьДокументы(Команда)
	...
	КонецЕсли;
	Если Модифицированность Тогда 
		ТекстВопроса = "Перед формированием пакета необходимо сохранить заявление. Сохранить изменения?";
		ОписаниеОповещения = Новый ОписаниеОповещения("ПодписатьДокументыПослеСохранения", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 0);
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ПодписатьДокументыПослеСохранения(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		СохранитьДанные();
		ПроверитьИПодписатьДокументы();
	КонецЕсли;
...
Процедура ПроверитьИПодписатьДокументы()
	...
	Если ФайлыПрикрепленныеДокументы.ИныеДокументы.Количество() > 0 Тогда
		...
		РегистрационныеФормыКлиент.ДобавитьФайлВОписаниеФайлов(
			ВидДокумента, ФайлыПрикрепленныеДокументы.ИныеДокументы, ПараметрыФормирования.ОписаниеФайлов);
	КонецЕсли;
