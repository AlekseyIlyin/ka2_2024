...
Процедура УстановитьТекстЗапроса(СхемаКомпоновкиДанных)
	...
	|
	|	ВЫБРАТЬ
	|		ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ПринятиеКУчету.Дата, ДЕНЬ), ДЕНЬ, -1) КАК Период,
	|		ПринятиеКУчету.Организация КАК Организация,
	|		ТаблицаОС.ОсновноеСредство КАК ОсновноеСредство,
	|		ТаблицаОС.СтоимостьБУ КАК СтоимостьНУ,
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса, 
		"&НормаАмортизации", 
		ВнеоборотныеАктивыЛокализация.ТекстПоляНормаАмортизации("ПорядокУчетаБУ_НУ.АмортизационнаяГруппа"));
