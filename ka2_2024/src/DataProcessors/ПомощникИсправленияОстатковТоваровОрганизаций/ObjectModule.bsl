...
Процедура ЗаполнитьМесяцИсправления() Экспорт
	...
	ТекстыЗапросовСторноТаможенныхДеклараций = Новый Массив;
	ТекстЗапросаСторноТаможенныхДеклараций =
		"ВЫБРАТЬ
		|	&ШаблонПериод КАК Период,
		...
		|	И ТоварыОрганизаций.Период <= &ШаблонПериод";
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Пока ТекМесяц <= КонецТекущегоМесяца Цикл
			...
			ТекстЗапроса = ТекстЗапросаСторноТаможенныхДеклараций;
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ШаблонПериод", "Период" + Формат(ТекМесяц, "ДФ=yyyyMM"));
			ТекстыЗапросовСторноТаможенныхДеклараций.Добавить(ТекстЗапроса);
		...
		Запрос.Текст = 
			...
			|	ВложенныйЗапрос.Организация КАК Организация,
			|	ВЫРАЗИТЬ(ВложенныйЗапрос.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры) КАК АналитикаУчетаНоменклатуры,
			|	ВложенныйЗапрос.ВидЗапасов КАК ВидЗапасов,
			|	ВложенныйЗапрос.НомерГТД КАК НомерГТД,
			|	СУММА(ВложенныйЗапрос.Количество) КАК Количество
			|ПОМЕСТИТЬ ТаможенныеДекларацииИмпорт
			|ИЗ
			|	ТекстЗапросаСторноТаможенныхДеклараций КАК ВложенныйЗапрос
			|
			|СГРУППИРОВАТЬ ПО
			|	ВложенныйЗапрос.Период,
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ТаможенныеДекларацииИмпорт.Период КАК Период,
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ОстаткиНаКонецПериодаВсе.Период КАК Период,
			...
			|	Данные.ВидЗапасов,
			|	Данные.НомерГТД,
			|	СУММА(ЕСТЬNULL(Данные.Количество, 0))
			|ИЗ
			|	(ВЫБРАТЬ
			|		ТаможенныеДекларацииИмпорт.Период КАК Период,
			|		ТаможенныеДекларацииИмпорт.Организация КАК Организация,
			...
			|				И (ТоварыОрганизаций.Период <= ДокументыПоступления.Период)
			|		
			|	СГРУППИРОВАТЬ ПО
			|		ДокументыПоступления.Период,
			|		ДокументыПоступления.Организация,
		...
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст, 
			"ТекстЗапросаСторноТаможенныхДеклараций", 
			"(" + СтрСоединить(ТекстыЗапросовСторноТаможенныхДеклараций, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении()) + ")");
...
Процедура СформироватьНовыеРезервы(ПоНовымНастройкам = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	ОстаткиНаКонецПериодаВсе.НомерГТД КАК НомерГТД,
		|	ОстаткиНаКонецПериодаВсе.КоличествоОстаток КАК КоличествоОстаток,
		|	ОстаткиНаКонецПериодаВсе.КоличествоПоРНПТОстаток КАК КоличествоПоРНПТОстаток
		|ПОМЕСТИТЬ ОстаткиНаКонецПериода
		|ИЗ
		|	ОстаткиНаКонецПериодаВсе КАК ОстаткиНаКонецПериодаВсе
		|
		...
		|		ТаможенныеДекларацииИмпорт.НомерГТД КАК НомерГТД,
		|		ТаможенныеДекларацииИмпорт.Количество КАК Количество,
		|		ТаможенныеДекларацииИмпорт.КоличествоПоРНПТ КАК КоличествоПоРНПТ
		|	ИЗ
		|		ТаможенныеДекларацииИмпорт КАК ТаможенныеДекларацииИмпорт
		|	ГДЕ
		|		ТаможенныеДекларацииИмпорт.Регистратор В
		...
		|		ДокументыПоступления.НомерГТД,
		|		-МИНИМУМ(ЕСТЬNULL(ТоварыОрганизаций.Количество, 0)),
		|		-МИНИМУМ(ЕСТЬNULL(ТоварыОрганизаций.КоличествоПоРНПТ, 0))
		|	ИЗ
		|		ДокументыПоступления КАК ДокументыПоступления
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций КАК ТоварыОрганизаций
		|			ПО (ТоварыОрганизаций.Организация = ДокументыПоступления.Организация)
...
Процедура ЗаполнитьТребуетсяИсправлениеРазвернутогоСальдо() Экспорт
	...
	ЗапросОрганизацииИсточникиБезВидовЗапасов = ЗапросОрганизацииИсточникиБезВидовЗапасов(МенеджерВременныхТаблиц, Истина, Ложь, Истина, Истина);
	...
	Пока ВыборкаОрганизацииСОтрицательнымОстатком.Следующий() Цикл
		...
		ЗапросОрганизацииИсточникиБезВидовЗапасов.УстановитьПараметр("Организация", ВыборкаОрганизацииСОтрицательнымОстатком.Организация);
		
		РезультатЗапроса = ЗапросОрганизацииИсточникиБезВидовЗапасов.Выполнить();
		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
...
Процедура ЗаполнитьСписокДокументовКПерезаполнению()
	...
	Если РезультатЗапроса[9].Пустой() <> РезультатЗапроса[10].Пустой() Тогда
		ТекстИсключения = 
		...
		ВызватьИсключение ТекстИсключения;
	Иначе
		ДокументыКПерезаполнению.Загрузить(РезультатЗапроса[10].Выгрузить());
	КонецЕсли;
...
Функция ЗапросОрганизацииИсточники(МенеджерВременныхТаблиц, ГруппироватьОстаткиПоНомерамГТД, ТолькоПоСовпадающимНомерамГТД, 
	...
	Запрос.Текст = ТекстЗапросаОтбораОстатков() + ОбщегоНазначенияУТ.РазделительЗапросовВПакете() +
		...
		|	ОстаткиНаКонецПериода КАК ОстаткиНаКонецПериода
		|ГДЕ
		|	(ОстаткиНаКонецПериода.АналитикаУчетаНоменклатуры, &ТекстИмяОтбораНомерГТД) В
		|			(ВЫБРАТЬ
...
Функция ЗапросОрганизацииИсточникиБезВидовЗапасов(МенеджерВременныхТаблиц, ГруппироватьОстаткиПоНомерамГТД, ТолькоПоСовпадающимНомерамГТД, 
			ТолькоПервыйИсточник, ВключаяСобственные)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц; 
	Запрос.Текст = ТекстЗапросаОтбораОстатковБезВидовЗапасов() + ОбщегоНазначения.РазделительПакетаЗапросов() +
		"ВЫБРАТЬ
		|	ТоварыОрганизацийОстатки.Организация КАК Организация,
		|	ТоварыОрганизацийОстатки.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	ТоварыОрганизацийОстатки.ВидЗапасов КАК ВидЗапасов,
...
Функция ТекстЗапросаОтбораОстатковБезВидовЗапасов()
...
Функция ТипыЗачетаОстатковРазвернутогоСальдо()
	...
	ТипЗачета.НомерПакетаОстаткиКРаспределению = 14;
	ТипЗачета.НомерПакетаОтрицательныеОстатки = 15;
