...
Процедура ИнициализироватьПомощник()
	...
	КонецЕсли;
	//++ НЕ УТ
	ПроизводствоБезЗаказов = ИспользуетсяПроизводствоБезЗаказа();
	//-- НЕ УТ
...
Процедура НастроитьЭлементыФормы()
	...
	Если НомерСтраницы = 6 Тогда
		...
		//++ НЕ УТ
		Если ПроизводствоБезЗаказов Тогда
...
//++ НЕ УТ
&НаСервереБезКонтекста
Функция ИспользуетсяПроизводствоБезЗаказа(Склад = Неопределено)
	...
	Возврат Справочники.СтруктураПредприятия.ИспользуетсяПроизводствоБезЗаказа(Подразделение);
...
Функция ОформитьПроизводствоБезЗаказаНаСервере(ТекущийИдентификатор, Знач ПараметрыДокументов)
	...
	Если Не ИспользуетсяПроизводствоБезЗаказа(ТекущиеДанные.Склад) Тогда
		
		ТекстСообщения = НСтр("ru = 'В подразделении склада ""%1"" не используется производство без заказа. Необходимо оформить оприходование товаров.'");
