...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УчетнаяЗапись = Справочники.УчетныеЗаписиМаркетплейсов.ОсновнаяУчетнаяЗапись(ПредопределенноеЗначение("Перечисление.ВидыМаркетплейсов.МаркетплейсOzon"));
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.УправлениеПродажамиНаOzon.Форма.НастройкаОтображенияОстатков" Тогда
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			ПараметрыОтображенияОстатковЦен = ПараметрыОтображенияОстатковЦенПеременная; // Структура
			УстановитьПараметрыСпискаТоваров();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "УчетнаяЗаписьДеактивирована" Тогда
		Если Параметр = УчетнаяЗапись Тогда
		...
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "ИзмененаВалютаУчетнойЗаписиМаркетплейса" И Параметр.УчетнаяЗапись = УчетнаяЗапись Тогда
...
Процедура ТоварыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	// Остаток
	СопоставленныеСклады = ИнтеграцияСМаркетплейсомOzonСервер.ПолучитьСопоставленныеСклады(Настройки.ДополнительныеСвойства.УчетнаяЗапись);
	Если ПараметрыОтображенияОстатковЦен.ОстатокПоУчету.ОтображатьВТаблице
		...
		Если Настройки.ДополнительныеСвойства.ИспользуетсяПроизводительныйРежим Тогда
		...
		Иначе
			ТекстРеквизитаОстатка = "СУММА(ЕСТЬNULL(ИсточникОстатков.Свободно, 0))";
			ТекстТаблицыОстатка = "
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ИсточникОстатков
			|		ПО (ИсточникОстатков.Номенклатура = ТаблицаТоваров.Номенклатура)
			|			И (ИсточникОстатков.Характеристика = ТаблицаТоваров.Характеристика)
			|			И (ИсточникОстатков.Склад В (&Склады))
			|			И (ИсточникОстатков.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка))
			|			И (ИсточникОстатков.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе))
			|
			|СГРУППИРОВАТЬ ПО
			|	ТаблицаТоваров.Номенклатура,
			|	ТаблицаТоваров.Характеристика,
	...
	Запрос.УстановитьПараметр("ВидыЦен", ИнтеграцияСМаркетплейсомOzonСервер.ИспользуемыеВидыЦен(Настройки.ДополнительныеСвойства.УчетнаяЗапись));
	...
	СтруктураОтбора.Вставить("УчетнаяЗаписьМаркетплейса", Настройки.ДополнительныеСвойства.УчетнаяЗапись);
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьДанныеФормы", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ТоварыСОшибкамиПриАктивизацииСтроки(Элемент)
	...
	Элементы.ТоварыСОшибкамиУдалитьТовар.Доступность = ТекущиеДанные.ДоступноУдаление;
	Элементы.ТоварыСОшибкамиКонтекстноеМенюУдалить.Доступность = ТекущиеДанные.ДоступноУдаление;
...
Процедура ТоварыСОшибкамиПередУдалением(Элемент, Отказ)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьДанныеФормы", ЭтаФорма);
	ДлительнаяОперация = УдалитьТоварыНаСервере(УчетнаяЗапись, Элементы.ТоварыСОшибками.ВыделенныеСтроки, УникальныйИдентификатор);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
...
Процедура Подобрать(Команда)
	...
	НастройкиУчетнойЗаписи = ИнтеграцияСМаркетплейсомOzonВызовСервера.НастройкиУчетнойЗаписи(УчетнаяЗапись);
...
Процедура ВыгрузитьОстатки(Команда)
	...
	ПараметрыОжидания.ОповещениеПользователя.Текст = НСтр("ru = 'Выгрузка остатков'");
	ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Завершена фоновая выгрузка остатков товаров на маркетплейс.'");

	ДлительнаяОперация = ВыгрузитьОстаткиНаСервере(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки);
...
Процедура ВыгрузитьЦены(Команда)
	...
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	ПараметрыОжидания.ОповещениеПользователя.Показать = Истина;
	ПараметрыОжидания.ОповещениеПользователя.Текст = НСтр("ru = 'Выгрузка цен'");
	ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Завершена фоновая выгрузка цен товаров на маркетплейс.'");

	ДлительнаяОперация = ВыгрузитьЦеныНаСервере(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Неопределено, ПараметрыОжидания);
...
Процедура ЗагрузитьОстатки(Команда)
	...
	ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
	ПараметрыОжидания.ОповещениеПользователя.Показать = Истина;
	ПараметрыОжидания.ОповещениеПользователя.Текст = НСтр("ru = 'Загрузка остатков'");
	ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Завершена фоновая загрузка остатков товаров с маркетплейса.'");
...
Процедура ЗагрузитьЦены(Команда)
	...
	ДлительнаяОперация = ЗагрузитьЦеныНаСервере(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Неопределено, ПараметрыОжидания);
...
Процедура ВыполнитьОбновлениеТоварногоКаталога(Команда)
	...
	ДлительнаяОперация = ВыполнитьОбновлениеТоварногоКаталогаНаСервере(УчетнаяЗапись);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Неопределено, ПараметрыОжидания);
...
Процедура НастроитьИнформационнуюПанель(Команда)
	...
	ОчиститьСообщения();
	ПараметрыФормы = Новый Структура("УчетнаяЗапись, ПараметрыОтображенияОстатковЦен", УчетнаяЗапись, ПараметрыОтображенияОстатковЦен);
	ОткрытьФорму("Обработка.УправлениеПродажамиНаOzon.Форма.НастройкаОтображенияОстатков", ПараметрыФормы, ЭтаФорма);
...
Процедура ДобавитьПоОтбору(Команда)
	...
	ОткрытьФорму("Обработка.ПодборТоваровПоОтбору.Форма.Форма", ПараметрыФормы, ЭтаФорма,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПеренестиВАрхивЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ДлительнаяОперация = ВыгрузитьНулевыеОстатки(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Неопределено, ПараметрыОжидания);
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьДанныеФормы", ЭтаФорма, ДополнительныеПараметры);
		ДлительнаяОперация = ПеренестиВАрхивНаСервере(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки, УникальныйИдентификатор);
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
	КонецЕсли;
...
Процедура ВернутьИзАрхива(Команда)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьДанныеФормы", ЭтаФорма, ДополнительныеПараметры);
	ДлительнаяОперация = ВернутьИзАрхиваНаСервере(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки, УникальныйИдентификатор);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
...
Процедура ПерейтиКУстановкеЦен(Команда)
	...
	ПараметрыОткрытияФормы = ПараметрыОткрытияПрайсЛиста(УчетнаяЗапись, Элементы.Товары.ВыделенныеСтроки);
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьДанныеФормы", ЭтаФорма);
	ОткрытьФорму("Обработка.ПрайсЛист.Форма", ПараметрыОткрытияФормы, ЭтаФорма,,,, ОповещениеОЗавершении);
...
Процедура ИнициализироватьПараметрыОтображенияОстатковЦен()
	...
	ПараметрыОтображенияОстатковЦенПеременная = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(ИмяФормы,
		"ПараметрыОтображенияОстатковЦен", ИнтеграцияСМаркетплейсомOzonСервер.НовыеПараметрыОтображенияОстатковЦен());
	ПараметрыОтображенияОстатковЦен = ПараметрыОтображенияОстатковЦенПеременная; // Структура;
...
Процедура УстановитьПараметрыСпискаТоваров()
	...
	ТоварыСОшибками.Параметры.УстановитьЗначениеПараметра("УчетнаяЗапись", УчетнаяЗапись);
	...
	СвойстваСпискаТоваров.Вставить("УчетнаяЗапись", УчетнаяЗапись);
	СвойстваСпискаТоваров.Вставить("ПараметрыОтображенияОстатковЦен", ПараметрыОтображенияОстатковЦен);

	НастройкиУчетнойЗаписи = ИнтеграцияСМаркетплейсомOzonСервер.НастройкиУчетнойЗаписиСЗагружаемымиВидамиЦен(УчетнаяЗапись);
	...
	СвойстваСпискаТоваров.Вставить("ИспользуетсяПроизводительныйРежим", РаспределениеЗапасов.ЭтоПроизводительныйРежим());
...
Процедура УстановитьДоступностьЭлементовДляСтатуса()
	...
	Элементы.ПеренестиВАрхив.Доступность = Не СтатусВАрхиве И Не ПустаяСтрока(ТекущиеДанные.ИдентификаторТовара);
	Элементы.ТоварыУдалить.Доступность = ТекущиеДанные.ДоступноУдаление;
	Элементы.УдалитьТовар.Доступность = ТекущиеДанные.ДоступноУдаление;
	Элементы.ТоварыКонтекстноеМенюУдалить.Доступность = ТекущиеДанные.ДоступноУдаление;
...
Процедура ОбновитьСтатистикуТоваров()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьСтатистикуТоваровПродолжение", ЭтаФорма);
	ДлительнаяОперация = ПолучитьСчетчикВыгрузки(УчетнаяЗапись, УникальныйИдентификатор);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
...
//  УчетнаяЗапись					 - СправочникСсылка.УчетныеЗаписиМаркетплейсов - Учетная запись для дозаполнения колонки таблицы товаров.
...
Процедура ОтправитьНаМодерациюВФоне(ВыделенныеСтроки)
	...
	ДлительнаяОперация = ОтправитьНаМодерациюНаСервере(УчетнаяЗапись, ВыделенныеСтроки);
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьДанныеФормы", ЭтаФорма, ДополнительныеПараметры);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, ОповещениеОЗавершении, ПараметрыОжидания);
...
&НаСервереБезКонтекста
Функция ОтправитьНаМодерациюНаСервере(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки)
...
&НаСервереБезКонтекста
Функция ВыгрузитьОстаткиНаСервере(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки)
...
&НаСервереБезКонтекста
Функция ВыгрузитьЦеныНаСервере(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки)
...
&НаСервереБезКонтекста
Функция ЗагрузитьЦеныНаСервере(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки)
...
&НаСервереБезКонтекста
Функция ВыполнитьОбновлениеТоварногоКаталогаНаСервере(УчетнаяЗапись)
...
&НаСервереБезКонтекста
Функция ВыгрузитьНулевыеОстатки(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки)
	...
	ИмяМетода = "ИнтеграцияСМаркетплейсомOzonСервер.ВыгрузитьОстаткиТоваров";
	Возврат ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, ИмяМетода, УчетнаяЗапись, ТаблицаТоваров, Истина);

КонецФункции

&НаСервереБезКонтекста
Функция ПеренестиВАрхивНаСервере(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки, УникальныйИдентификатор)
...
&НаСервереБезКонтекста
Функция ВернутьИзАрхиваНаСервере(УчетнаяЗапись, ЗНАЧ ВыделенныеСтроки, УникальныйИдентификатор)
...
&НаСервереБезКонтекста
Функция ПолучитьСчетчикВыгрузки(УчетнаяЗапись, УникальныйИдентификатор)
	...
	ИмяМетода = "ИнтеграцияСМаркетплейсомOzonСервер.ПолучитьСчетчикВыгрузки";
...
Процедура УстановитьДоступностьВидимостьЭлементов()
	...
	УстановитьВидимостьКолонокЦеныОстатки();
	УчетнаяЗаписьВыбрана = Не УчетнаяЗапись.Пустая();
	...
	Элементы.ОтправитьНаМодерацию.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана;
	Элементы.ВыгрузитьОстатки.Доступность = УчетнаяЗаписьВыбрана;
	Элементы.ВыгрузитьЦены.Доступность = УчетнаяЗаписьВыбрана;
	...
	Элементы.ПеренестиВАрхив.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана;
	...
	Элементы.ТоварыСОшибкамиОтправитьНаМодерацию.Доступность = СтрокаТаблицыВыбрана И УчетнаяЗаписьВыбрана;
