...
Процедура ПриОткрытии(Отказ)
	...
	Если КоличествоПодключений = 0 Тогда
		ОткрытьФорму("Обработка.УправлениеПродажамиНаOzon.Форма.ПодключениеКСервису",, ЭтотОбъект.ВладелецФормы);
		Отказ = Истина;
	КонецЕсли;
...
Процедура СписокПодключенийВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	ПараметрыОткрытияФормы = Новый Структура("УчетнаяЗапись", Значение);
...
Процедура СписокПодключенийПередНачаломИзменения(Элемент, Отказ)
	...
	СписокПодключенийВыборЗначения(Элемент, Элементы.СписокПодключений.ТекущиеДанные, СтандартнаяОбработка);
