...
Процедура НастроитьФормуПоПравамИФункциональнымОпциям()
	...
	Если ЗначениеЗаполнено(Распоряжения) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоПередачаВПереработку2_4") Тогда
			ШаблонЗаголовка = НСтр("ru = 'Документы передачи в переработку (%1) (к оформлению)'");
		Иначе
			ШаблонЗаголовка = НСтр("ru = 'Документы передачи в переработку 2.4 (%1) (к оформлению)'");
		КонецЕсли;
		
		Заголовок = СтрШаблон(ШаблонЗаголовка, НРег(Элементы.Страницы.ТекущаяСтраница.Заголовок));
		
	Иначе
		
		Если Не ПолучитьФункциональнуюОпцию("ИспользуетсяТолькоПередачаВПереработку2_4") Тогда
			АвтоЗаголовок = Ложь;
			Заголовок = НСтр("ru = 'Документы передачи в переработку 2.4 (к оформлению)'");
		КонецЕсли;
