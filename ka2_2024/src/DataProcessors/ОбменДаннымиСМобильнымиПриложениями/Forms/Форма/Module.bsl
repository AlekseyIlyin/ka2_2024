...
Процедура ПриОткрытии(Отказ)
	...
	Если УзелОбмена.Пустая()
		Или КомпонентаОбменаСМобильнымиПриложениями = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьОбменДаннымиСМобильнымПриложением()
	...
	ТипСвязиЧислом = ТипСвязиЧислом(ТипСвязи);
	ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].УстановитьПараметрыСвязи(КодМобильногоКомпьютера, ТипСвязиЧислом, АдресСервера, Порт, ИмяСервисаIRDA);
	
	Попытка
		ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].НачатьСеансОбмена(КодМобильногоКомпьютера, ИмяМобильнойБазы, ИмяПользователя, 
	...
	ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].УстановитьРежимНачальнойИнициализации(КодМобильногоКомпьютера, НачальнаяИнициализацияИБ);
	...
	Попытка
		ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].УстановитьПриложение(КодМобильногоКомпьютера, ПриложениеСтрокой);
	Исключение
		ПоказатьПредупреждение(Неопределено, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;	
	...
	Попытка
		ДанныеКлиента = ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].ПолучитьДанныеКлиента(КодМобильногоКомпьютера);
	Исключение
		ПоказатьПредупреждение(Неопределено, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	...
	Если ПередаватьДанныеКлиенту Тогда
		...
		Попытка
			ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].ПередатьДанныеКлиенту(КодМобильногоКомпьютера, ДанныеДляОбмена);
		Исключение
			ПоказатьПредупреждение(Неопределено, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат;
		КонецПопытки;
	...
	Попытка
		ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].ЗавершитьСеансОбмена(КодМобильногоКомпьютера, ИмяМобильнойБазы, ИмяПользователя);
	Исключение
		ПоказатьПредупреждение(Неопределено, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Функция МассивБазКлиента()
	...
	СписокБазКлиента = ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].ПолучитьСписокИнформационныхБазКлиента(КодМобильногоКомпьютера, ИмяПользователя);
...
Процедура ПроверитьВозможностьРаботы(Отказ)
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"] = Неопределено Тогда
