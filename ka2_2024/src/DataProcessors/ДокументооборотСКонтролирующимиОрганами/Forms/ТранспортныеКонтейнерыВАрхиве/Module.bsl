...
Процедура ЗаполнитьСписокТК(МассивОбъектов)
	...
	ТекстЗапрос = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТранспортноеСообщениеКонтейнерыПрисоединенныеФайлы.Ссылка КАК Ссылка
	|ИЗ
	...
	|			ПО (ТранспортноеСообщениеКонтейнерыПрисоединенныеФайлы.ВладелецФайла = ТранспортноеСообщение.Ссылка)
	|		ПО ЦиклыОбмена.Ссылка = ТранспортноеСообщение.ЦиклОбмена
	|ГДЕ
	...
	|		ПО (ТранспортноеСообщениеКонтейнерыПрисоединенныеФайлы.ВладелецФайла = ТранспортноеСообщение.Ссылка)
	|ГДЕ
	|	ТранспортноеСообщение.Ссылка В(&МассивОбъектов)
	|	И СостояниеАрхивированияФайловДОСКонтролирующимиОрганами.Архивный = ИСТИНА";
...
Процедура ОтправитьОбращениеИТС(Режим = Истина)
	...
	ОбщегоНазначенияКлиент.ОбщийМодуль("СообщенияВСлужбуТехническойПоддержкиКлиент").ОтправитьСообщение(ДанныеОбращения, Вложения, Неопределено, ОповещениеЗавершения);
...
Процедура УстановитьВидимостьЭлементов()
	...
	ИначеЕсли РежимФормы = "ДлительнаяОперация" ИЛИ РежимФормы = "Расшифровка" Тогда
		...
		Элементы.ГруппаВыборРасшифовка.Видимость = СписокТК.Количество() < ВсегоВАрхиве;
...
Функция СформироватьПеречень(ОтборФайлов = Истина)
	...
	|			ИЛИ ТранспортноеСообщениеКонтейнерыПрисоединенныеФайлы.Ссылка В (&ТранспортныеКонтейнеры))
	|	И СостояниеАрхивированияФайловДОСКонтролирующимиОрганами.Архивный = ИСТИНА";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Если ОтборФайлов Тогда
		Запрос.УстановитьПараметр("ТранспортныеКонтейнеры", СписокТК.ВыгрузитьЗначения());
		Запрос.УстановитьПараметр("ВсеКонтейнеры", Ложь);
	Иначе
		Запрос.УстановитьПараметр("ТранспортныеКонтейнеры", Новый Массив);
