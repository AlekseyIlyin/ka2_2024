...
Процедура ЗакрытиеМесяцаСДаты(Параметры, Разделы = Неопределено)
	...
	Пока ТестируемыйМесяц <= Параметры.КонецРасчета Цикл
		...
		Если Не ТестированиеЗакрытияМесяца Тогда
			РеглУчетПроведениеСервер.ОтразитьВсе(КонецМесяца(ТестируемыйМесяц));
		КонецЕсли;
	...
	Для Каждого РазделРасчета Из ВыбранныеРазделы Цикл
		...
		Если РазделРасчета = "РеглУчет" Тогда
			...
			РегистрыСведений.ОтражениеДокументовВРеглУчете.ЗарегистрироватьДокументыКОтражению(
				НачалоМесяца(Параметры.НачалоРасчета), КонецМесяца(Параметры.КонецРасчета), , ТипыДляСнятияОтражения);
