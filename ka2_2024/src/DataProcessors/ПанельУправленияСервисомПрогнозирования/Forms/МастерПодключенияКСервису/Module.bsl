...
Процедура СформироватьДеревоВыгрузкиМетаданных()
	...
	Настройки = СервисПрогнозирования.ПолучитьНастройкиСервиса();
	КоллекцииПоПорядку = СервисПрогнозированияПереопределяемый.КоллекцииПоПорядку();
...
Процедура ЗаполнитьСводкуНастроек()
	...
	Если ЗначениеКонстанты <> Неопределено
		...
		Если НастройкиОтборов <> Неопределено Тогда
			...
			ЕстьФильтрВыгрузкиДанных = ОтборыНастроек.Элементы.Количество() > 0;
		КонецЕсли;
	КонецЕсли;
	...
	ШаблонСценарийВыгрузкиКонтрольныхПланов = НСтр("ru='Контрольные планы продаж будут выгружаться по сценарию планирования %1'");
	...
	СводкаНастроек = Новый ФорматированнаяСтрока(СтрокиСводки);
...
Процедура УстановитьВидимостьДоступность()
	...
	ИначеЕсли ВыгрузкаРанееПроизводилась Тогда
		Элементы.СтраницыТестовыйПрогнозЗапуститьИлиПоказатьСтатус.ТекущаяСтраница = Элементы.СтраницыТестовыйПрогнозДовыгрузка;
		...
		КонецЕсли;
	Иначе
		Элементы.СтраницыТестовыйПрогнозЗапуститьИлиПоказатьСтатус.ТекущаяСтраница = Элементы.СтраницыТестовыйПрогнозЗапустить;
...
Процедура НачатьПрогнозированиеНаСервере(ТребуетсяПолнаяВыгрузка = Ложь, ТребуетсяРасчетПрогноза = Истина, ТребуетсяПроверкаВидаПлана = Ложь)
	...
	Если ТребуетсяПроверкаВидаПлана Тогда
		...
		ТребуемыеРеквизиты = "ТипПлана, НачалоПрогнозирования, ЗаполнятьПартнера, ЗаполнятьПартнераВТЧ, ЗаполнятьПоХарактеристикамНоменклатуры";
		...
		Иначе
			...
			ЕстьОшибка = Ложь;
			Если (РеквизитыВидаПлана.ЗаполнятьПартнера
					Или РеквизитыВидаПлана.ЗаполнятьПартнераВТЧ)
...
Процедура ЗаписатьНастройки()
	...
	Если СуществующийОтборВыгрузкиЗначение <> Неопределено
		...
		ИзмененыОтборыВыгрузки = МассивыРавны(
...
Процедура ОтправитьЗаявкуВСервисНаСервере(ВыполнениеПроверено = Ложь)
	Если ВыполнениеПроверено
		Или ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗаданияОтправитьЗаявкуВСервис) Тогда
...
Процедура ОбновитьСтатусПодключенияРезультатНаСервере(ВыполнениеПроверено = Ложь)
	Если ВыполнениеПроверено
		Или ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗаданияОбновитьСтатусПодключения) Тогда
...
Процедура ПодключитьсяКСервисуНаСервере(ВыполнениеПроверено = Ложь)
	Если ВыполнениеПроверено
		Или ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗаданияПодключитьсяКСервису) Тогда
...
Процедура ПовторитьРегистрациюНаСервере(ВыполнениеПроверено = Ложь)
	Если ВыполнениеПроверено
		Или ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗаданияПовторитьРегистрацию) Тогда
