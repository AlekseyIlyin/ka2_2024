...
&НаСервере
Функция ДобавитьПодобранныеТовары(ПодобранныеТовары, СтрокиТовары, Упаковка, Цена, КоэффициентУпаковки, ТипНоменклатуры = Неопределено, ТоварыСКоличественнымИзмерением)
	...
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		...
		Для Каждого СтрокаСерии Из СтрокиСерий Цикл
			...
			СтрокаССерией.ХарактеристикиИспользуются = ХарактеристикиИспользуются;
		...
		Если Не ОкруглениеКорректно
				...
				ПоискТовара = ТоварыСКоличественнымИзмерением.Найти(СтрокаТовары.Номенклатура);
				Если ПоискТовара <> Неопределено Тогда
					ОкруглениеКорректно = НоменклатураСервер.ОкруглениеКоличестваВыполненоКорректно(СтруктураОбъект, ПараметрыПроверки);
				КонецЕсли;
...
Функция ПараметрыТовара()
	...
	ПараметрыТовара.Вставить("ХарактеристикиИспользуются",  ХарактеристикиИспользуются);
...
Функция ПодобранныеТовары()
	...
	Если Объект.ВариантКомплектацииНоменклатуры.Пустая() Тогда
		...
		СтрокаДокументаДобавляестяВпервые = ДобавитьПодобранныеТовары(
			...
			Объект.Цена,
			КоэффициентУпаковки,,
			ТоварыСКоличественнымИзмерением());
	...
	Иначе
		...
		ТоварыСКоличественнымИзмерением = ТоварыСКоличественнымИзмерением();
		...
		Для Каждого ПозицияНабора Из Объект.СоставНабора Цикл
			...
			Если Подобрано > 0 Тогда
				...
				ПараметрыТовара.ВариантОбеспечения = Объект.ВариантОбеспечения;
				ПодобранныеТовары.Добавить(ПараметрыТовара);
				ПараметрыТовара.ХарактеристикиИспользуются = ХарактеристикиИспользуютсяПоКомлектующим.Получить(ПозицияНабора.Номенклатура);
...
&НаСервере
Функция ТоварыСКоличественнымИзмерением()
	...
	|		ПО ТаблицаНоменклатуры.ЕдиницаИзмерения = ТаблицаУпаковок.Ссылка
	|ГДЕ
	|	ТаблицаНоменклатуры.Ссылка В (&МассивНоменклатуры)
	|	И ТаблицаУпаковок.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук)";
