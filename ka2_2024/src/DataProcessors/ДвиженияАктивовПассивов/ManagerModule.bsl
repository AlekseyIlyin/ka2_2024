...
Процедура ДобавитьТекстЗапросаРасчетыПоФинансовымИнструментам(Запрос, ТаблицаДвижений)
	...
	|		И НЕ (ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.ЗаключениеДоговораАренды)
...
Процедура ДобавитьПростойТекстЗапросаПоРегистру(Запрос, ТаблицаДвижений, ПараметрыЗапроса)
	...
	Если ПараметрыЗапроса.ИмяСтатьиАП = "ПрибылиИУбытки" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Т.Аналитика", "ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка)");
	Иначе
