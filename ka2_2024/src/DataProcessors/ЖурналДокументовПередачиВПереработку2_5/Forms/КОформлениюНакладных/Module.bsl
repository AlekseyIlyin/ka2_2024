...
Процедура ЗаполнитьСписокВыбораМенеджера()
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		МассивРолей,
		ПраваПользователяПовтИсп.ИменаРолейСПравомДобавления(Метаданные.Документы.ПередачаТоваровХранителю.ПолноеИмя()));
		
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		МассивРолей,
		ПраваПользователяПовтИсп.ИменаРолейСПравомДобавления(Метаданные.Документы.ПоступлениеТоваровОтХранителя.ПолноеИмя()));
		
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		МассивРолей,
		ПраваПользователяПовтИсп.ИменаРолейСПравомДобавления(Метаданные.Документы.ОтчетПереработчика2_5.ПолноеИмя()));
	
	ПользователиСДоступнымиРолями = ОбщегоНазначенияУТ.ПользователиСДоступнымиРолями(МассивРолей);
	
	МенеджерСписокВыбора = Элементы.Менеджер.СписокВыбора;
	МенеджерСписокВыбора.Очистить();
	
	Для каждого ПользовательСДоступнымиРолями Из ПользователиСДоступнымиРолями Цикл
		ЗаполнитьЗначенияСвойств(МенеджерСписокВыбора.Добавить(), ПользовательСДоступнымиРолями);
	КонецЦикла;
