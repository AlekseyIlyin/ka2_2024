...
Процедура ВыполнитьПереоценкуНаДату(МассивОрганизаций, Период, Параметры) Экспорт
	...
	|		РасчетыСКлиентамиНУОстатки.СуммаНУОстаток                   КАК СуммаНУОстаток,
	|		РасчетыСКлиентамиНУОстатки.ОтложенныйДоходОстаток           КАК ОтложенныйДоходОстаток,
	|		РасчетыСКлиентамиНУОстатки.ОтложенныйРасходОстаток          КАК ОтложенныйРасходОстаток,
	|		0                                                           КАК ПредоплатаОстаток,
	|		0                                                           КАК ПредоплатаРеглОстаток,
	|		ВЫБОР КОГДА АналитикиУчетаПоПартнерам.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.УстановленныйВДоговоре)
	|				ТОГДА ЕСТЬNULL(КурсыВалютПоДоговорам.КурсЧислитель,1)
	|			КОГДА АналитикиУчетаПоПартнерам.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки)
	|					И РасчетыСКлиентамиНУОстатки.ОбъектРасчетов.ТипОбъектаРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовРасчетов.Накладная)
	|				ТОГДА 
	...
	|						ВтАналитикиУчетаПоПартнерам КАК АналитикиУчетаПоПартнерам
	|					)
	|			) КАК РасчетыСКлиентамиНУОстатки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикиУчетаПоПартнерам КАК АналитикиУчетаПоПартнерам
	|				ПО РасчетыСКлиентамиНУОстатки.АналитикаУчетаПоПартнерам = АналитикиУчетаПоПартнерам.АналитикаУчетаПоПартнерам
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтносительныеКурсыВалют.СрезПоследних(&ГраницаКонецДня, ) КАК КурсыВалют
	|				ПО РасчетыСКлиентамиНУОстатки.Валюта = КурсыВалют.Валюта
	|				И КурсыВалют.БазоваяВалюта = АналитикиУчетаПоПартнерам.ВалютаРегламентированногоУчета
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам.СрезПоследних(&ГраницаКонецДня, ) КАК КурсыВалютПоДоговорам
	|				ПО КурсыВалютПоДоговорам.Договор = АналитикиУчетаПоПартнерам.Договор
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВалютыИКурсыДокументов КАК КурсыВалютПоДокументам
	|				ПО РасчетыСКлиентамиНУОстатки.ОбъектРасчетов.Объект = КурсыВалютПоДокументам.Документ
	|	ГДЕ
	|		&РасчетыСКлиентами
	|		И &ДатаРасчета >= ДАТАВРЕМЯ(2022,1,1)
	|		И &ИспользоватьРеглУчет
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|		0                                                           КАК ДолгОстаток,
	|		0                                                           КАК ДолгРеглОстаток,
	|		РасчетыСПоставщикамиНУОстатки.СуммаНУОстаток                КАК СуммаНУОстаток,
	|		РасчетыСПоставщикамиНУОстатки.ОтложенныйДоходОстаток        КАК ОтложенныйДоходОстаток,
	|		РасчетыСПоставщикамиНУОстатки.ОтложенныйРасходОстаток       КАК ОтложенныйРасходОстаток,
	|		0                                                           КАК ПредоплатаОстаток,
	|		0                                                           КАК ПредоплатаРеглОстаток,
	|		ВЫБОР КОГДА АналитикиУчетаПоПартнерам.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.УстановленныйВДоговоре)
	|				ТОГДА ЕСТЬNULL(КурсыВалютПоДоговорам.КурсЧислитель,1)
	|			КОГДА АналитикиУчетаПоПартнерам.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки)
	|					И РасчетыСПоставщикамиНУОстатки.ОбъектРасчетов.ТипОбъектаРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовРасчетов.Накладная)
	|				ТОГДА 
	|					ЕСТЬNULL(КурсыВалютПоДокументам.КурсЧислительВалютыВзаиморасчетов,1)
	|			ИНАЧЕ ЕСТЬNULL(КурсыВалют.КурсЧислитель,1)
	|		КОНЕЦ                                                           КАК КурсЧислительРегл,
	...
	|						ВтАналитикиУчетаПоПартнерам КАК АналитикиУчетаПоПартнерам
	|					)
	|			) КАК РасчетыСПоставщикамиНУОстатки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикиУчетаПоПартнерам КАК АналитикиУчетаПоПартнерам
	|				ПО РасчетыСПоставщикамиНУОстатки.АналитикаУчетаПоПартнерам = АналитикиУчетаПоПартнерам.АналитикаУчетаПоПартнерам
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтносительныеКурсыВалют.СрезПоследних(&ГраницаКонецДня, ) КАК КурсыВалют
	|				ПО РасчетыСПоставщикамиНУОстатки.Валюта = КурсыВалют.Валюта
	|				И КурсыВалют.БазоваяВалюта = АналитикиУчетаПоПартнерам.ВалютаРегламентированногоУчета
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам.СрезПоследних(&ГраницаКонецДня, ) КАК КурсыВалютПоДоговорам
	|				ПО КурсыВалютПоДоговорам.Договор = АналитикиУчетаПоПартнерам.Договор
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВалютыИКурсыДокументов КАК КурсыВалютПоДокументам
	|				ПО РасчетыСПоставщикамиНУОстатки.ОбъектРасчетов.Объект = КурсыВалютПоДокументам.Документ
	|	ГДЕ
	|		&РасчетыСПоставщиками
	|		И &ДатаРасчета >= ДАТАВРЕМЯ(2022,1,1)
	|		И &ИспользоватьРеглУчет
	...
	Для Каждого СтрокаПереоценки Из ОбъектыКПереоценке Цикл
		...
		Для Каждого ЗаписьНУ Из СтрокиДетальныхЗаписейНУ Цикл
			...
			Если Период = КонецМесяца(Период) И Окр(СуммыНУ.КурсоваяРазницаОстатка,2) <> 0 Тогда
			...
			ИначеЕсли ЗаписьНУ.ДолгОстаток = 0 И Окр(СуммыНУ.КурсоваяРазницаОплаты,2) <> 0 Тогда
...
Функция ПолучитьОплаты(Запрос, ГлобальныеПеременные)
	...
	|		ЛОЖЬ                                                             КАК ПриходДобавлен,
	|		ЛОЖЬ                                                             КАК ГрафикСписан,
	|		РасчетыПоСрокам.Предоплата + РасчетыПоСрокам.Долг                КАК КОплате,
	|		РасчетыПоСрокам.Предоплата + РасчетыПоСрокам.Долг                КАК Сумма,
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	|			ТОГДА 0
	|		КОГДА &ФиксированныйКурсНакладной
	|			ТОГДА &ФиксированныйКурсРегл
...
Функция ПолучитьКурсыВалют(Запрос, ДатаНачала, ГлобальныеПеременные)
	...
	|		И НЕ &КурсУстановленныйВДоговоре
	|	
	|	ОБЪЕДИНИТЬ ВСЕ 
	|
	|	ВЫБРАТЬ
	...
	|		0 КАК КурсУпр
	|	ИЗ
	|		РегистрСведений.КурсыВалютРасчетовПоДоговорам КАК КурсыВалютыРасчетов
	|	ГДЕ КурсыВалютыРасчетов.Договор = &Договор
	|		И КурсыВалютыРасчетов.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
	|		И &КурсУстановленныйВДоговоре
	|	
	|	ОБЪЕДИНИТЬ ВСЕ 
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ &КурсУстановленныйВДоговоре
	|	
	|	ОБЪЕДИНИТЬ ВСЕ 
	|	
	|	ВЫБРАТЬ
...
Функция ПолучитьДанныеНакладных(Запрос)
	...
	|ГДЕ
	|	НЕ СвязанныеДокументы.СвязанныйДокумент ССЫЛКА Документ.ПервичныйДокумент
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ВЫБРАТЬ
	|	СвязанныеДокументы.СвязанныйДокумент КАК Накладная,
	|	ВЫРАЗИТЬ(СвязанныеДокументы.СвязанныйДокумент КАК Документ.ПервичныйДокумент).СуммаВзаиморасчетов КАК Сумма,
	|	ВЫРАЗИТЬ(СвязанныеДокументы.СвязанныйДокумент КАК Документ.ПервичныйДокумент).СуммаРегл КАК СуммаРегл,
	|	ВЫРАЗИТЬ(СвязанныеДокументы.СвязанныйДокумент КАК Документ.ПервичныйДокумент).СуммаУпр КАК СуммаУпр
	|ИЗ
	|	ВтСвязанныеДокументы КАК СвязанныеДокументы
	|ГДЕ
	...
	Для Каждого СтрокаОтгрузки Из СтрокиОтгрузки Цикл
		...
		Если ГлобальныеПеременные.ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки Тогда
			...
			Если СтрокаОплаты <> Неопределено Тогда
				СуммаПереоценкиРегл = Окр(СтрокаОтгрузки.Сумма * СтрокаОтгрузки.КурсРегл, 2) - СтрокаОтгрузки.СуммаРеглДляПереоценки;
				СуммаПереоценкиУпр = Окр(СтрокаОтгрузки.Сумма * СтрокаОтгрузки.КурсУпр, 2) - СтрокаОтгрузки.СуммаУпрДляПереоценки;
...
Процедура ЗаполнитьДвиженияНУзаПериод(Организация, Период, ЭтоРасчетыСКлиентами, ДатаНачалаПересчета = Неопределено)
	...
	|	И (&РасчетыСКлиентами И РасчетыКР.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереоценкаРасчетовСКлиентами)
	|		ИЛИ НЕ &РасчетыСКлиентами И РасчетыКР.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереоценкаРасчетовСПоставщиками))
	...
	|	ГДЕ
	|		&РасчетыСКлиентами
	|		И &НачалоПериода = &НачалоВступленияВСилу67ФЗ
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("РасчетыСКлиентами", ЭтоРасчетыСКлиентами);
	Запрос.УстановитьПараметр("МассивОрганизаций", МассивОрганизаций);
	...
	Пока Месяцы.Следующий() Цикл
		...
		Пока Регистраторы.Следующий() Цикл
			...
			Если НачалоМесяца(Месяцы.Месяц) <> НачалоМесяца(Период) Тогда
				...
				ВыполнитьПереоценкуЗаПериод(МассивОрганизаций, Месяцы.Месяц,,ПараметрыПереоценки);
			КонецЕсли;
