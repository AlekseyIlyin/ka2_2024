				...
				Если ТекущаяЦена = 0 Или Не ТолькоНезаполненные Тогда
					...
					Если ЗначениеЗаполнено(Формула) Тогда
						...
						Попытка
							...
							СтрокаТаблицыЦен["ФормулаИзмененаВручную" + ВидЦены.ИмяКолонки] = (ПредставлениеФормулы <> ФормулаПоУмолчанию);
...
Процедура ДеревоЦенУпаковкаПриИзменении(Форма, ИдентификаторСтроки, ИмяТекущейКолонки) Экспорт
	...
	Если ЭтоРегистрацияЦенПоставщика
	...
	Если ДобавлятьТекущуюСтроку И СтрНайти(ИмяТекущейКолонки, СтрокаУпаковка) > 0 Тогда
...
//  ИмяТекущейКолонки - Строка
//
Процедура ДеревоЦенВалютаПриИзменении(Форма, ИдентификаторСтроки, ИмяТекущейКолонки) Экспорт
	
	ЭтоРегистрацияЦенПоставщика = ЭтоРегистрацияЦенПоставщика(Форма);
	ТекущаяСтрока = Форма.ДеревоЦен.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	СтрокаУпаковка = "Валюта";
...
Функция ДобавитьТоварыПредприятия2_5(Форма, Знач ТаблицаТовары, КэшДанных)
	...
	|	Таблица.Индекс КАК Индекс,
	|	Таблица.Номенклатура КАК Номенклатура,
	|	&ТекстПодменыХарактеристикаЦО КАК ХарактеристикаЦО,
	|	&ТекстПодменыСерияЦО КАК СерияЦО,
	|	&ТекстПодменыУпаковкаЦО КАК УпаковкаЦО
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры") Тогда
		ТекстПодменыХарактеристикаЦО = "Таблица.ХарактеристикаЦО";
	Иначе
		ТекстПодменыХарактеристикаЦО = "ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)";
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСерииНоменклатуры") Тогда
		ТекстПодменыСерияЦО = "Таблица.СерияЦО";
	Иначе
		ТекстПодменыСерияЦО = "ЗНАЧЕНИЕ(Справочник.СерииНоменклатурыДляЦенообразования.ПустаяСсылка)";
	КонецЕсли;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры") Тогда
		ТекстПодменыУпаковкаЦО = "Таблица.УпаковкаЦО";
	Иначе
		ТекстПодменыУпаковкаЦО = "ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)";
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстПодменыХарактеристикаЦО", ТекстПодменыХарактеристикаЦО);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстПодменыСерияЦО", ТекстПодменыСерияЦО);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстПодменыУпаковкаЦО", ТекстПодменыУпаковкаЦО);
