...
Процедура ЗаполнитьФормуПояснения5(КонтекстВыполнения, ЗаполнятьПредыдущиеПериоды)
	...
	Для Каждого Строка Из Строки.Строки Цикл
		...
		ЗаполнитьСтрокуПояснений5(КонтекстВыполнения, СтрокаДанных.КодСтроки, СтрокаДанных.КодЯчейки, ДанныеКЗаполнению, Строка.Строки, ПараметрыПоказателя);
		...
		Если ЗаполнятьПредыдущиеПериоды И ЗначениеЗаполнено(СтрокаДанных.КодСтрокиПрошлогоПериода) Тогда
			...
			ЗаполнитьСтрокуПояснений5(КонтекстВыполнения, СтрокаДанных.КодСтрокиПрошлогоПериода,
				СтрокаДанных.КодЯчейкиПрошлогоПериода, ДанныеКЗаполнениюПрошлыйПериод, Строка.Строки, ПараметрыПоказателя);
		КонецЕсли;
...
Процедура ЗаполнитьСтрокуПояснений5(КонтекстВыполнения, КодСтроки, КодЯчейки, ДанныеЗаполнения, ДанныеСтроки, ПараметрыПоказателя)
