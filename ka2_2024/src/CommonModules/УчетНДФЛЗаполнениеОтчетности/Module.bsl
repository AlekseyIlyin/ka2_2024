...
Процедура РасчетПоказателей_6НДФЛ_2024Кв2(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Если ОписаниеТаблиц.Свойство("Раздел2Налоги") Тогда
		...
		Запрос.УстановитьПараметр("ПервыйПериодПо", НачалоОтчетногоКвартала + 22 * 86400);
		Запрос.УстановитьПараметр("ВторойПериодС", НачалоОтчетногоКвартала + 23 * 86400);
		...
		Запрос.УстановитьПараметр("ТретийПериодПо", ДобавитьМесяц(НачалоОтчетногоКвартала, 1) + 22 * 86400);
		Запрос.УстановитьПараметр("ЧетвертыйПериодС", ДобавитьМесяц(НачалоОтчетногоКвартала, 1) + 23 * 86400);
		...
		Запрос.УстановитьПараметр("ПятыйПериодПо", НачалоМесяца(ОкончаниеПериода) + 22 * 86400);
		Запрос.УстановитьПараметр("ШестойПериодС", НачалоМесяца(ОкончаниеПериода) + 23 * 86400);
		...
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК П000020019103,
		|	СУММА(ВЫБОР
		|			КОГДА Налоги.ДатаУдержания МЕЖДУ &ВторойПериодС И &ВторойПериодПо
		|				ТОГДА Налоги.Возвращено
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК П000020019203,
		|	СУММА(ВЫБОР
		|			КОГДА Налоги.ДатаУдержания МЕЖДУ &ТретийПериодС И &ТретийПериодПо
		|				ТОГДА Налоги.Возвращено
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК П000020019303,
		|	СУММА(ВЫБОР
		|			КОГДА Налоги.ДатаУдержания МЕЖДУ &ЧетвертыйПериодС И &ЧетвертыйПериодПо
		|				ТОГДА Налоги.Возвращено
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК П000020019403,
		|	СУММА(ВЫБОР
		|			КОГДА Налоги.ДатаУдержания МЕЖДУ &ПятыйПериодС И &ПятыйПериодПо
		|				ТОГДА Налоги.Возвращено
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК П000020019503,
		|	СУММА(ВЫБОР
		|			КОГДА Налоги.ДатаУдержания МЕЖДУ &ШестойПериодС И &ШестойПериодПо
		|				ТОГДА Налоги.Возвращено
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК П000020019603,
		|	Налоги.СтавкаНалога КАК СтавкаНалога,
		|	Налоги.Ставка КАК Ставка,
		|	Налоги.КБК КАК КБК
		|ИЗ
...
Процедура РасчетПоказателей_6НДФЛ_2024Кв1(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Если ОписаниеТаблиц.Свойство("Раздел2Налоги") Тогда
		...
		Запрос.УстановитьПараметр("ПервыйПериодПо", НачалоОтчетногоКвартала + 22 * 86400);
		Запрос.УстановитьПараметр("ВторойПериодС", НачалоОтчетногоКвартала + 23 * 86400);
		Запрос.УстановитьПараметр("ВторойПериодПо", ДобавитьМесяц(НачалоОтчетногоКвартала, 1) + 22 * 86400);
		Запрос.УстановитьПараметр("ТретийПериодС", ДобавитьМесяц(НачалоОтчетногоКвартала, 1) + 23 * 86400);
		Запрос.УстановитьПараметр("ТретийПериодПо", НачалоМесяца(ОкончаниеПериода) + 22 * 86400);
		Запрос.УстановитьПараметр("ЧетвертыйПериодС", НачалоМесяца(ОкончаниеПериода) + 23 * 86400);
