...
#Область ТекстВозвратНеперечисленнойЗарплатыПоЗарплатномуПроектуЧерезПереводыВПути // (Дт 51 :: Кт 57.01)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	Сумма(ЕСТЬNULL(Суммы.СуммаБезНДСРегл, Операция.СуммаДокумента * ЕстьNULL(КурсВалютыДокумента.Курс, 1))) КАК Сумма,
	|	Сумма(ЕСТЬNULL(Суммы.СуммаБезНДСУпр, Операция.СуммаДокумента * ЕстьNULL(КурсВалютыДокумента.Курс, 1) / ЕстьNULL(КурсВалютыУпрУчета.Курс, 1))) КАК СуммаУУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ДенежныеСредства) КАК ВидСчетаДт,
	|	Операция.БанковскийСчет КАК АналитикаУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
...
Функция ВозвратДСНовыеВзаиморасчеты()
	...
	|
	|	ВЫБОР
	|		КОГДА РасчетыПоПретензиям.Валюта <> &ВалютаРеглУчета И ПерваяЗаписьРегистра.НомерСтроки = ДанныеРегистраДС.НомерСтроки
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТПерваяЗаписьДенежныеСредстваБезналичные КАК ПерваяЗаписьРегистра
	|	ПО ПерваяЗаписьРегистра.Регистратор = ДанныеРегистраДС.Регистратор
	|
	|ГДЕ
	|	&НоваяАрхитектураВзаиморасчетов";
