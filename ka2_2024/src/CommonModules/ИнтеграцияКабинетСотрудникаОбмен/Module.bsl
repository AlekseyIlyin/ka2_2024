...
Функция ВыгрузитьИзмененияПрочихОбъектов(МенеджерОбмена) Экспорт
	...
	ТипыОбъектов.Добавить(ПеречисленияМенеджер.МашиночитаемаяДоверенностьКЭДО);
...
Функция ДанныеОбъектовДляВыгрузки(МенеджерОбмена, ТаблицаИзменений, ТипОбъекта)
	...
	ИначеЕсли ТипОбъекта = ПеречисленияМенеджер.МашиночитаемаяДоверенностьКЭДО Тогда
		
		ДанныеОбъектов = ДанныеМЧД(ТаблицаИзменений);
...
Функция ДанныеМЧД(ТаблицаИзменений)

	Результат = ИнтеграцияУправлениеПерсоналомОбмен.НовыеДанныеДляВыгрузки();
	ТаблицаДанных = НоваяТаблицаМЧД();
	
	МЧД = ТаблицаИзменений.ВыгрузитьКолонку("Ссылка");
	Данные = ИнтеграцияУправлениеПерсоналом.ДанныеМЧД(МЧД);
...
Функция НовыйЭлектронныйДокумент()
	...
	Возврат Новый Структура("ИсходныйДокумент,ПредставленияДокумента,ОригиналВMXL");
