...
Процедура ПроверитьЧтоОСПринятыКУчету(Объект, Отказ) Экспорт
	...
	Иначе
		...
		ПроверитьВВыбранномУчете = НастройкиНалоговУчетныхПолитикПовтИсп.РеглУчетВНАВедетсяНезависимо(
	...
	ДоступенВыборОтраженияВУчетах = НастройкиНалоговУчетныхПолитикПовтИсп.РеглУчетВНАВедетсяНезависимо(
			Объект.Организация, КонецМесяца(?(Объект.Дата <> '000101010000', Объект.Дата, ТекущаяДатаСеанса())));
...
//
Процедура ТекстыЗапросаПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, Регистры) Экспорт
	
	ИмяДокумента = ?(ВидОбъектовУчета = "ОС", "Документ.ПереоценкаОС2_4", "Документ.ПереоценкаНМА2_4");
	
	ТекстЗапросаТаблицаСтоимостьПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры);
	ТекстЗапросаТаблицаАмортизацияПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры);
	ТекстЗапросаТаблицаПрочиеДоходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры);
	ТекстЗапросаТаблицаПрочиеРасходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры);
	ТекстЗапросаТаблицаПартииПрочихРасходовПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры);
	ТекстЗапросаТаблицаДвиженияДоходыРасходыПрочиеАктивыПассивыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры);
...
Процедура ТекстЗапросаТаблицаВтСписаниеОбесцененияОС(ТекстыЗапроса) Экспорт
	...
	|				И втСписаниеСтоимостиОС.Ссылка = &Ссылка
	|ГДЕ
	|	&ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеОСЧастичное)
	|";
...
Функция РеквизитыДоговораАренды(Договор) Экспорт
	...
	|НалогообложениеНДС,ТребуетсяОформлениеЗаявленийОВвозе";
...
Процедура ТекстЗапросаТаблицаВтТаблицаОС(ТекстыЗапроса, ИмяДокумента, ИмяВременнойТаблицы = Неопределено) Экспорт
	...
	Если ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтРасширеннаяТаблицаОС", ТекстыЗапроса)
		И ИмяТаблицы = "втТаблицаОС" Тогда
	...
	"ВЫБРАТЬ
	|	ТаблицаОС.ОсновноеСредство                  КАК ОсновноеСредство,
	|	ЕСТЬNULL(ТаблицаОСУзловКомпонентов.ОСВладелец, ЗНАЧЕНИЕ(Справочник.ОбъектыЭксплуатации.ПустаяСсылка)) КАК ОСВладелец,
	|
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ втРасширеннаяТаблицаОС КАК ТаблицаОСУзловКомпонентов
	|		ПО ТаблицаОС.ОсновноеСредство = ТаблицаОСУзловКомпонентов.ОсновноеСредство
	|
	|ГДЕ
	...
	Если Метаданные.Документы[ЧастиИмени[1]].ТабличныеЧасти[ИмяТЧ].Реквизиты.Найти("ИдентификаторСтроки") = Неопределено Тогда
...
Процедура ТекстЗапросаТаблицаВтПорядокУчетаОС(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ПорядокУчета.Период                    КАК Период,
...
Процедура ТекстЗапросаТаблицаВтПараметрыАмортизацииНМАУУ(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	|	ПараметрыАмортизацииНМАУУ.ОбъемПродукцииРаботДляВычисленияАмортизации КАК ОбъемПродукцииРаботДляВычисленияАмортизации,
...
//  ТекстыЗапроса - СписокЗначений из Строка -
//  ИмяДокумента - Строка - Полное имя документа.
Процедура ТекстЗапросаТаблицаВтВыработкаНМА(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	|	ВыработкаНМА.КоличествоОборот КАК Количество
	...
	|						СписокНМА.НематериальныйАктив
	|					ИЗ
	|						втСписокНМА КАК СписокНМА)) КАК ВыработкаНМА
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НематериальныйАктив";
...
Процедура ТекстЗапросаТаблицаВтСписокНМА(ТекстыЗапроса, ИмяДокумента) Экспорт
	...
	ИначеЕсли ВРег(ИмяДокумента) = ВРег("Документ.ПринятиеКУчетуНМА2_4")
		//++ Локализация
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.ПринятиеКУчетуНМА") 
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.ПереоценкаНМА")
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.СписаниеНМА")
		ИЛИ ВРег(ИмяДокумента) = ВРег("Документ.ПодготовкаКПередачеНМА") 
		//-- Локализация
		Тогда
		ТаблицаНМА = "";
	КонецЕсли;
...
Процедура ПроверитьСоответствиеСтатьиРасходовПорядкуУчетаНУ(Форма) Экспорт
	...
	МассивВариантовРаспределения = ВнеоборотныеАктивыКлиентСервер.ДоступныеВариантыРаспределенияРасходов(
...
Процедура ТекстЗапросаТаблицаСтоимостьПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры)
	...
	ТекстЗапросаВтТаблицаПереоценки(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента);
...
Процедура ТекстЗапросаТаблицаАмортизацияПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры)
...
Функция ТекстЗапросаТаблицаПрочиеДоходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры)
...
Функция ТекстЗапросаТаблицаПрочиеРасходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры) 
	...
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтПрочиеРасходы", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаВтПрочиеРасходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаВтПрочиеРасходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента)
	
	ИмяРегистра = "ВтПрочиеРасходы";
	
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтИсходныеПрочиеРасходы", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаВтИсходныеПрочиеРасходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента)
...
Функция ТекстЗапросаТаблицаПартииПрочихРасходовПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры) Экспорт
	...
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтПартииПрочихРасходов", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаВтПартииПрочихРасходовПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаВтПартииПрочихРасходовПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента)
	
	ИмяРегистра = "ВтПартииПрочихРасходов";
	
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтИсходныеПартииПрочихРасходов", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаВтИсходныеПартииПрочихРасходовПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаВтИсходныеПартииПрочихРасходовПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента)
...
Процедура ТекстЗапросаВтТаблицаПереоценки(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента) Экспорт
	...
	|		ПО АмортизацияВНА.ОбъектУчета = &ОсновноеСредство
	|			И АмортизацияВНА.Ссылка = ТаблицаПереоценки.Ссылка
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокУчетаОС.СрезПоследних(&Период, &ДатаИсправления) КАК ПорядокУчета
	|		ПО &ОсновноеСредство = ПорядокУчета.ОсновноеСредство
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОС.СрезПоследних(&Период, &ДатаИсправления) КАК МестонахождениеОС
...
Процедура ТекстЗапросаТаблицаДвиженияДоходыРасходыПрочиеАктивыПассивыПриПереоценке(ТекстыЗапроса, Запрос, ВидОбъектовУчета, ИмяДокумента, Регистры)
...
Процедура ЗаписатьТаблицыДвижений(ТаблицыДляДвижений, Документ, МенеджерВременныхТаблиц, ИмяОперации = Неопределено, ВидИсточника = Неопределено, Период = Неопределено) Экспорт
...
Функция РегистрПоддерживаетИсправление(ИмяРегистра)
