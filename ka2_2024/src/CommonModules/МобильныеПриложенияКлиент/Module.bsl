...
Процедура ПодключитьКомпонентуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Подключено Тогда
		...
		Попытка
			ПараметрыПриложения.Вставить("СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями", Результат.ПодключаемыйМодуль);
			УстановитьНастройкиПользователяПриРаботеСКомпонентойОбменаДанными();
		Исключение
			ПараметрыПриложения.Вставить("СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями", Неопределено);
...
Процедура ОбработатьВнешнееСобытиеОтМобильногоПриложения(Источник, Событие, Данные) Экспорт
	...
	Если НЕ МобильныеПриложенияВызовСервера.АутентификацияВыполнена(ИмяПользователя, КодМобильногоКомпьютера, ПарольПользователя) Тогда
		...
		Попытка
			ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].СообщитьКлиентуОбОшибке(КодМобильногоКомпьютера, ТекстСообщения);
			Возврат;
		Исключение
	...
	ИначеЕсли Событие = "ПолучитьДанные" Тогда
		...
		Попытка
			НачальнаяИнициализацияИБ = ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].ПолучитьРежимНачальнойИнициализации(КодМобильногоКомпьютера);
		Исключение
			ИмяСобытия = НСтр("ru='Ошибка при вызове метода компоненты обмена данными'");
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытия,"Ошибка",ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
			Возврат;
	...
	ИначеЕсли Событие = "ЗаписатьДанные" Тогда
		...
		Попытка
			ДанныеМобильногоПриложения = ПараметрыПриложения["СтандартныеПодсистемы.ОбъектыУТКАУП.КомпонентаОбменаСМобильнымиПриложениями"].ПолучитьДанныеКлиента(КодМобильногоКомпьютера);
		Исключение
			ИмяСобытия = НСтр("ru='Ошибка при вызове метода компоненты обмена данными'");
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ИмяСобытия,"Ошибка",ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
			Возврат;
