		...
		Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
			...
			Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
				...
				Иначе
					...
					Если ТипЗнч(ДанныеОбработки.codes) = Тип("Массив") Тогда
						
						ЕстьОшибкиОбработки = Ложь;
						
						Для Каждого ЭлементДанных Из ДанныеОбработки.codes Цикл
	
							Если ЭлементДанных.Свойство("printView") Тогда
							...
							Иначе
								...
								Если ДанныеОбработки.Свойство("reqId") Тогда
									ПараметрыКодаМаркировки.РазрешительныйРежимИдентификаторЗапросаГИСМТ = ДанныеОбработки.reqId;
								КонецЕсли;
								
								Если ДанныеОбработки.Свойство("reqTimestamp") Тогда
									ПараметрыКодаМаркировки.РазрешительныйРежимДатаЗапросаГИСМТ = ИнтеграцияИС.ДатаИзСтрокиUNIX(ДанныеОбработки.reqTimestamp);
								КонецЕсли;
