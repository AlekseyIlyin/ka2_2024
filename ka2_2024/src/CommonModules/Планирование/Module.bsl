...
Функция ПроверитьПравилоЗаполнения(Знач ПравилоЗаполнения, Знач АдресСхемыПреобразованияДанных, Знач ВидПлана = Неопределено, Знач ИмяРеквизитаПравила = "", ОтборПоРесурсам) Экспорт 
	...
	Если НЕ СоответствиеПолямДокумента ИЛИ НЕ СоответствиеПолямИсточников ИЛИ НЕ ЕстьПолеЦены Тогда
		ПравилоКорректно = Ложь;
	КонецЕсли;
	...
	ТекстОшибки = НСтр("ru = 'В составе полей, возвращаемых источниками заполнения цены, отсутствует поле ""Цена"". Отредактируйте ""Выбранные поля"" или используйте другой источник'");
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДополнительныеПараметры.ТипПлана = "ПланЗакупок"
		...
		Если ДополнительныеПараметры.ИспользуютсяНазначения И Выборка.КоличествоНазначений > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
		...
		//++ НЕ УТ
		Если ДополнительныеПараметры.ИспользуютсяСпецификации И Выборка.КоличествоСпецификаций > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
		...
		Если ДополнительныеПараметры.ИспользуютсяСклады И Выборка.КоличествоСкладов > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
			ЗаписьОНеоднозначности.Колонка       = "Склад";
		КонецЕсли;
		
		Если ДополнительныеПараметры.ИспользуютсяПартнеры И Выборка.КоличествоПартнеров > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
			ЗаписьОНеоднозначности.Колонка       = "Партнер";
		КонецЕсли;
		
		Если ДополнительныеПараметры.ИспользуютсяСоглашения И Выборка.КоличествоСоглашений > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
			ЗаписьОНеоднозначности.Колонка       = "Соглашение";
		КонецЕсли;
		
		Если ДополнительныеПараметры.ИспользуютсяВидыЦен И Выборка.КоличествоВидовЦен > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
			ЗаписьОНеоднозначности.Колонка       = "ВидЦеныПоставщика";
		КонецЕсли;
		
		Если ДополнительныеПараметры.ТипПлана = "ПланСборкиРазборки" И Выборка.КоличествоВариантовКомплектации > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Выборка.Идентификатор;
...
Функция СоздатьНастроитьЗапрос(ТекстыЗапроса, ЗагружаемыеДанные, ДополнительныеПараметры)
	...
	// Используется план закупок
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеЦенаСумма,", 
		?(ДополнительныеПараметры.ТипПлана = "ПланЗакупок" ИЛИ ДополнительныеПараметры.ТипПлана = "ПланПродаж", "ЗагружаемыеДанные.Цена КАК Цена,
			|ЗагружаемыеДанные.Сумма КАК Сумма,", ""));
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеСрокИсполненияЗаказа,", 
		?(ДополнительныеПараметры.ТипПлана = "ПланЗакупок", "ЗагружаемыеДанные.СрокИсполненияЗаказа КАК СрокИсполненияЗаказа,", ""));
