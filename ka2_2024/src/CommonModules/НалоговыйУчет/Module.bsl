...
Функция ПериодичностьОтчетногоПериода(Период, Организация) Экспорт
	...
	ПараметрыПолитики = НастройкиНалоговУчетныхПолитикПовтИсп.ДействующиеПараметрыНалоговУчетныхПолитик("НастройкиУчетаНалогаНаПрибыль", Организация, Период);
...
Процедура УстановитьПараметрыЗапроса_НалоговыеСтавки(Запрос,
	...
	Запрос.УстановитьПараметр("ВариантНалогообложенияПрибыли",             Неопределено);
	Запрос.УстановитьПараметр("ОсновнойВариантНалогообложенияПрибыли",     Неопределено);
