...
Процедура ФактуровкаПоставок(ПараметрыРасчета) Экспорт
	...
	// 5. Помещение данных в таблицу-приемник и очистка вспомогательных временных таблиц.
	// 	- формирует движения по регистру СебестоимостьТоваров.
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗавершитьРаспределениеПартий(ПараметрыРасчета, Ложь); // этап распределения партий еще не завершен
...
//	в части постойнной разницы должнв быть сформированы из суммы отклонний в регл учете что бы в учете расходов сумма
...
//	Отбор движений для формирования записей с типом "Прошлое" выполняеся по данным временной таблицы ДокументыПоступленияНефактурованнойПоставкиПрошлогоПериода.
...
//	с отбором по типам записией КорректировкаПриобретенияПрошлогоПериода, ДопРасходыСПартией, КорректировкаСтоимости.
// 
...
Функция ТекстЗапросаДляФактуровкиПоставок(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + ТекстДокументыПоступленияНефактурованнойПоставкиПрошлогоПериода() // вт ДокументыПоступленияНефактурованнойПоставкиПрошлогоПериода
		...
		+ ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + ТекстТаблицаРеестраДокументов() // вт с данными из реестра документов
...
Процедура ЗаполнитьРасчетнуюПартиюФактуровкиПоставок(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	Если ЕстьОтклонения Тогда
		...
		// в ппроцедуре СформироватьДвиженияПоОтклонениямВСтоимостиТоваров() #Область ОтклоненияВСтоимостиПрочиеДоходы.
		...
		Иначе
		// Формирование движений по регистру "Прочие доходы" выполняется в общем модуле "РасчетСебестоимостиКорректировкаСтоимости"
		// в ппроцедуре СформироватьДвиженияПоОтклонениямВСтоимостиТоваров() #Область ОтклоненияВСтоимостиПрочиеРасходы.
			Отклонения.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ОтклонениеВСтоимостиНаПрочиеРасходы;
...
Функция ТекстЗапросаДляПартийТоваров(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		+ ОбщегоНазначенияУТ.РазделительЗапросовВПакете() + ТекстРасчетныеОстаткиПартийТоваров() // вт РасчетныеОстаткиПартийТоваров
		+ ОбщегоНазначенияУТ.РазделительЗапросовВПакете()
		// выборка данных
		+ ТекстОписаниеДанныхДляПартийТоваров() // вт Данные
...
Процедура ЗаполнитьРасчетнуюПартиюСебестоимостиТоваров(ПараметрыРасчета, РасчетнаяПартия, Расход, Приход)
	...
	Если РасчетнаяПартия.ВидДвижения = ВидДвиженияНакопления.Приход
	 ...
	 И НЕ (Приход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.Прошлое
		...
		ИначеЕсли Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.ИсправлениеТекущегоПериода
		 И ТипЗнч(Расход.Регистратор) = Тип("ДокументСсылка.Сторно") Тогда
	...
	Если Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки
	 ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки
	 // Для корректировки приобретения отклонения рассчитываем только в случае корректировки прошлого периода. 
	 ...
	 ИЛИ Расход.ТипЗаписи = Перечисления.ТипыЗаписейПартий.КорректировкаПриобретения
		...
		// Если по строке корректировки приобретения рассчитываются отклонения в стоимости, то по этой строке
		// не должны уменьшаться стоимостные показатели в источнике (см. код выше после комментария "Корректируем базу расчета в приходе").
		// Условия проверки для уменьшения стоимостных показателей должны быть одинаковые с условиями ниже.  
		...
		// При неотфактурованной поставке стоимость может быть нулевой, кроме сторно. 
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСНеотфактурованнаяПоставка
		И НЕ Расход.Сторно
		ИЛИ Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаНеотфактурованнаяПоставка
		И НЕ Расход.Сторно) Тогда
