...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварамиРасширенный(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ТоварыОрганизаций", ТекстыЗапроса) Тогда
		Документы.ВозвратТоваровОтКлиента.ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры);
	КонецЕсли; 
...
Процедура ПроверитьВозможностьВводаНаОснованииЧекККМ(Объект, ЧекККМ, Отказ = Неопределено)
	...
	ИначеЕсли РозничныеПродажи.СменаОткрыта(ДанныеЧекККМ.КассоваяСмена, Объект.Дата) Тогда
