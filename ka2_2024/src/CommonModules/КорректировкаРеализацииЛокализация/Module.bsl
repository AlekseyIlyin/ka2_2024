...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|		УчетнаяПолитикаОрганизаций.Ссылка = ВыручкаИСебестоимость.Ссылка
	|		И УчетнаяПолитикаОрганизаций.Организация = ВыручкаИСебестоимость.Организация
	|		И УчетнаяПолитикаОрганизаций.Период = КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ)
	|		И УчетнаяПолитикаОрганизаций.РаздельныйУчетПоНалогуНаПрибыль";
	ТекстыЗапросаПолученияНалогообложенияПрибыли.Добавить(ТекстЗапроса);
	#КонецОбласти
	...
	#Область ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовараПрошлыеПериоды
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
	|	КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ) КАК Период,
	...
	|		УчетнаяПолитикаОрганизаций.Ссылка = Операция.Ссылка
	|		И УчетнаяПолитикаОрганизаций.Организация = Операция.Организация
	|		И КОНЕЦПЕРИОДА(УчетнаяПолитикаОрганизаций.Период, МЕСЯЦ) = КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ)
	|		И УчетнаяПолитикаОрганизаций.РаздельныйУчетПоНалогуНаПрибыль
	|	
	|ГДЕ
	|	НЕ ДанныеДокументаОснования.ПередачаНаКомиссию
	...
	#Область ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовара
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	...
	#Область ТекстКорректировкаВыручкиОтРеализацииСобственногоТовараПрошлыеПериоды
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
	|	КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ) КАК Период,
	...
	#Область ТекстВыручкаОтРеализацииРаботУслугПрошлыйНалоговыеПериоды
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
	|	КОНЕЦПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, МЕСЯЦ) КАК Период,
...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварамиРасширенный(Запрос, ТекстыЗапроса, Регистры)
	...
	|				ИЛИ ТаблицаВидыЗапасов.НомерГТД.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТКомплекта))
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		(ТаблицаВидыЗапасов.НомерГТД.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|				ИЛИ ТаблицаВидыЗапасов.НомерГТД.ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТКомплекта))) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовараПрошлыеПериоды()
	...
	#Область ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовара
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Уменьшение реализации собственных/комиссионных товаров прошлых периодов (Дт 91.02 :: Кт 62.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|			 КОНЕЦ  
	|	КОНЕЦ)  КАК СуммаПРДт,
	|	ЕСТЬNULL(-СуммыДокументовВВалютахУчета.СуммаНДСРегл, Строки.СуммаНДС) КАК СуммаВРДт,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСКлиентами) КАК ВидСчетаКт,
	|	Расчеты.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаКт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	...
	|	ВЫБОР
	|		КОГДА Строки.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|		ТОГДА ""Уменьшение реализации собственных товаров прошлых периодов""
	|		ИНАЧЕ ""Уменьшение реализации комиссионных товаров прошлых периодов""
	|	КОНЕЦ КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
