...
Функция ОперацииСРегламентированнымОтчетом(ИмяОтчета, ИмяФормы) Экспорт
	...
	// Временно, до появления шаблона PDF417 ГНИВЦ.
...
Функция ИспользуютсяРегистрацииВОрганеСтатистики() Экспорт
	...
	ИмяПодсистемы = "РегламентированнаяОтчетность.РегистрацииВОрганеСтатистики";
	#Если НаКлиенте Тогда
		ПризнакИспользования = ОбщегоНазначенияКлиент.ПодсистемаСуществует(ИмяПодсистемы);
	#Иначе
		ПризнакИспользования = ОбщегоНазначения.ПодсистемаСуществует(ИмяПодсистемы);
	#КонецЕсли
...
Процедура ДополнитьОпциональныеПараметрыОтчета(Форма, СписокСохранения) Экспорт
	
	Если ИспользуютсяРегистрацииВОрганеСтатистики() Тогда
		Если Форма.РегистрацияРеспондента <> Неопределено Тогда
			СписокСохранения.Вставить("РегистрацияРеспондента", Форма.РегистрацияРеспондента);
		КонецЕсли;
	КонецЕсли;
