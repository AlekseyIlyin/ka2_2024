...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварамиРасширенный(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ТоварыОрганизаций", ТекстыЗапроса) Тогда
		Документы.ВнутреннееПотребление.ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры);
	КонецЕсли; 
...
Процедура ДополнительнаяОбработкаПриОтраженииДокумента(МенеджерВременныхТаблиц) Экспорт
	...
	|		ПО ВнутреннееПотребление.Ссылка = УчетнаяПолитикаОрганизаций.Ссылка
	|			И ВнутреннееПотребление.Организация = УчетнаяПолитикаОрганизаций.Организация
	|
	|ГДЕ
