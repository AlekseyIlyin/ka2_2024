...
//++ НЕ УТ
Процедура ЗаполнитьДанныеСотрудника(Прокси, ОбъектXDTO, Сотрудник,
		ЗаполнятьПодразделение = Истина, ЗаполнятьРуководителя = Истина)
	...
	Запрос = Новый Запрос(
		...
		|	ЕСТЬNULL(КадроваяИсторияСотрудниковСрезПоследних.Подразделение, НЕОПРЕДЕЛЕНО) КАК Подразделение,
		|	ЕСТЬNULL(КадроваяИсторияСотрудниковСрезПоследних.Должность, НЕОПРЕДЕЛЕНО) КАК Должность,
		|	ЕСТЬNULL(КадроваяИсторияСотрудниковСрезПоследних.Период, НЕОПРЕДЕЛЕНО) КАК ДействуетС,
		|	ЕСТЬNULL(КадроваяИсторияСотрудниковСрезПоследних.ДействуетДо, НЕОПРЕДЕЛЕНО) КАК ДействуетПо,
		|	ЕСТЬNULL(СотрудникиРуководителя.Ссылка, НЕОПРЕДЕЛЕНО) КАК Руководитель
		|ИЗ
		|	ВТСотрудники КАК Сотрудники
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
		|		ПО Сотрудники.ФизическоеЛицо = Пользователи.ФизическоеЛицо
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КадроваяИсторияСотрудников.СрезПоследних(, Сотрудник = &Сотрудник) КАК КадроваяИсторияСотрудниковСрезПоследних
		|		ПО (КадроваяИсторияСотрудниковСрезПоследних.Сотрудник = Сотрудники.Сотрудник)
		|			И (КадроваяИсторияСотрудниковСрезПоследних.ФизическоеЛицо = Сотрудники.ФизическоеЛицо)
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК СотрудникиРуководителя
		|			ПО (СотрудникиРуководителя.ФизическоеЛицо = СведенияКонтрактаДоговораСотрудникаСрезПоследних.ПредставительНанимателя)
		|		ПО Сотрудники.Сотрудник = СведенияКонтрактаДоговораСотрудникаСрезПоследних.Сотрудник
		|			И Сотрудники.ФизическоеЛицо = СведенияКонтрактаДоговораСотрудникаСрезПоследних.ФизическоеЛицо
		|			И Сотрудники.ГоловнаяОрганизация = СведенияКонтрактаДоговораСотрудникаСрезПоследних.ГоловнаяОрганизация");
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(Выборка.Руководитель) Тогда
			ОбъектXDTO.manager = ИнтеграцияС1СДокументооборотБазоваяФункциональность.СоздатьОбъект(
				Прокси,
				"DMEmployee",
		...
		Запрос.Текст =
			...
			|	ЕСТЬNULL(ВидыЗанятостиСотрудников.ВидЗанятости, НЕОПРЕДЕЛЕНО) КАК ВидЗанятости
			|ИЗ
			|	ВТСотрудники КАК Сотрудники
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВидыЗанятостиСотрудниковСрезПоследних КАК ВидыЗанятостиСотрудников
			|		ПО Сотрудники.Сотрудник = ВидыЗанятостиСотрудников.Сотрудник";
		ВыборкаВидЗанятости = Запрос.Выполнить().Выбрать();
		Если ВыборкаВидЗанятости.Следующий()
				И ЗначениеЗаполнено(ВыборкаВидЗанятости.ВидЗанятости) Тогда
			ОбъектXDTO.mainEmployee =
				(ВыборкаВидЗанятости.ВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы);
		КонецЕсли;
...
Процедура ЗаполнитьСсылочныйОбъектXDTOИзПотребителя(Прокси, ОбъектXDTO, СсылкаНаПотребитель, Настройки)
	...
	ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОбъектXDTO, "DMSubdivision") Тогда
		...
		Если ТипЗнч(СсылкаНаПотребитель) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
			МестоВСтруктуреПредприятия = ОрганизационнаяСтруктура.ПодразделениеВСтруктуреПредприятия(
				СсылкаНаПотребитель);
			Если ЗначениеЗаполнено(МестоВСтруктуреПредприятия) Тогда
				Руководитель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
	...
	ИначеЕсли ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПроверитьТип(Прокси, ОбъектXDTO, "DMEmployee") Тогда
		...
		//++ НЕ УТ
		Если ТипЗнч(СсылкаНаПотребитель) = Тип("СправочникСсылка.Сотрудники") Тогда
			ЗаполнитьДанныеСотрудника(Прокси, ОбъектXDTO, СсылкаНаПотребитель);
		КонецЕсли;
		//-- НЕ УТ
		...
		Если ТипЗнч(СсылкаНаПотребитель) = Тип("СправочникСсылка.Пользователи") Тогда
			...
			Запрос = Новый Запрос(
				...
				|			И (КадроваяИсторияСотрудниковСрезПоследних.ФизическоеЛицо = Пользователи.ФизическоеЛицо)
				|ГДЕ
				|	Пользователи.Ссылка = &Пользователь
				...
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
				|		ПО Сотрудники.ФизическоеЛицо = Пользователи.ФизическоеЛицо
				|			И (КадроваяИсторияСотрудниковСрезПоследних.Подразделение = Пользователи.Подразделение.Источник)
				|ГДЕ
				|	Пользователи.Ссылка = &Пользователь
				|	И НЕ Сотрудники.ПометкаУдаления");
			Запрос.УстановитьПараметр("ДатаСреза", ТекущаяДатаСеанса());
			...
			ВыборкаСотрудникиБезПодразделения = МассивРезультатов[1].Выбрать();
			ВыборкаСотрудникиПоПодразделениям = МассивРезультатов[2].Выбрать();
			
			Если ВыборкаСотрудникиБезПодразделения.Количество() = 1 Тогда
				
				ВыборкаСотрудникиБезПодразделения.Следующий();
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьРеквизитыИзПотребителя(
					Прокси,
					ОбъектXDTO,
					ВыборкаСотрудникиБезПодразделения.Сотрудник);
				
			ИначеЕсли ВыборкаСотрудникиПоПодразделениям.Количество() = 1 Тогда
				
				ВыборкаСотрудникиПоПодразделениям.Следующий();
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьРеквизитыИзПотребителя(
					Прокси,
					ОбъектXDTO,
					ВыборкаСотрудникиПоПодразделениям.Сотрудник);
			...
			Иначе
				...
				Запрос.Текст =
					...
					|	Сотрудники.Сотрудник КАК Сотрудник,
					|	ВЫБОР
					|		КОГДА ЕСТЬNULL(ВидыЗанятостиСотрудников.ВидЗанятости, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)
					|			ТОГДА 0
					|		ИНАЧЕ 1
					...
					|УПОРЯДОЧИТЬ ПО
					|	ВидЗанятости";
				ВыборкаСВидомЗанятости = Запрос.Выполнить().Выбрать();
				
				Если ВыборкаСВидомЗанятости.Следующий() Тогда
					ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьРеквизитыИзПотребителя(
						Прокси,
						ОбъектXDTO,
						ВыборкаСВидомЗанятости.Сотрудник);
				КонецЕсли;
		...
		Если ТипЗнч(СсылкаНаПотребитель) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			...
			Запрос = Новый Запрос(
				...
				|ИЗ
				|	Справочник.Сотрудники КАК Сотрудники
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КадроваяИсторияСотрудников.СрезПоследних(, ФизическоеЛицо = &ФизическоеЛицо) КАК КадроваяИсторияСотрудниковСрезПоследних
				|		ПО (КадроваяИсторияСотрудниковСрезПоследних.Сотрудник = Сотрудники.Ссылка)
				|ГДЕ
				...
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	ВТСотрудники.Сотрудник КАК Ссылка
				|ИЗ
				|	ВТСотрудники КАК ВТСотрудники");
			Запрос.УстановитьПараметр("ДатаСреза", ТекущаяДатаСеанса());
			Запрос.УстановитьПараметр("ФизическоеЛицо", СсылкаНаПотребитель);
			...
			Если ВыборкаСотрудники.Количество() = 1 Тогда
				
				ВыборкаСотрудники.Следующий();
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаполнитьРеквизитыИзПотребителя(
					Прокси,
					ОбъектXDTO,
					ВыборкаСотрудники.Ссылка);
