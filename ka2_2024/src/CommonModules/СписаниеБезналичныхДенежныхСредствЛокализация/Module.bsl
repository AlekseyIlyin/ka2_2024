...
//
// Возвращаемое значение:
//   Структура - НомерДоговораСБанком, ДатаДоговораСБанком
//
...
Функция ТекстОплатаДолгаАвансаПоставщикуДругойОрганизацииМежФилиальная()
	...
	Текст = "
	|ВЫБРАТЬ //// Межфилиальная оплата долга/аванса поставщику (Дт 60.01,60.02,60.21,60.22 :: Кт 51,52)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком
	|		ИЛИ Операция.Исправление ТОГДА
	...
	|	Расчеты.СуммаУпр КАК СуммаУУ,
	|
	|	ЕСТЬNULL(РасчетыПоДокументам.ВидСчета, ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСПоставщиками)) КАК ВидСчетаДт,
	|	Расчеты.ГруппаФинансовогоУчета КАК АналитикаУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	Расчеты.Валюта КАК ВалютаДт,
	...
	|			И ПерваяЗаписьРегистра.НомерСтроки = ДанныеРегистраДС.НомерСтроки
	|		ТОГДА ДанныеРегистраДС.СуммаПостоплатыВВалютеПлатежа + ДанныеРегистраДС.СуммаПредоплатыВВалютеПлатежа
	|		КОГДА НЕ ДанныеРегистраДС.СуммаПостоплатыВВалютеПлатежа ЕСТЬ NULL
	|		ТОГДА 0
	|		ИНАЧЕ Расчеты.Сумма
	...
	|	0 КАК СуммаВРКт,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(РасчетыПоДокументам.ВидСчета, ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСПоставщиками)) =
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РасчетыСПоставщикамиМежФилиальные КАК РасчетыПоДокументам
	|	ПО
	|		Расчеты.Ссылка = РасчетыПоДокументам.Ссылка
	|		И Расчеты.Контрагент = РасчетыПоДокументам.Контрагент
	|		И Расчеты.Договор = РасчетыПоДокументам.Договор
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВТПерваяЗаписьДвиженияДенежныеСредстваКонтрагент КАК ПерваяЗаписьРегистра
	|	ПО
	|		ПерваяЗаписьРегистра.Регистратор = ДанныеРегистраДС.Ссылка
	|	
	|ГДЕ
	|	Операция.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаПоставщику),
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ //// Межфилиальная оплата другой организации (Дт 60.01,60.02,60.21,60.22 :: Кт 51,52)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(Операция.Дата) = Операция.ДатаПроведенияБанком
	|		ИЛИ Операция.Исправление ТОГДА
	...
	|	
	|ГДЕ
	|	Операция.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаДенежныхСредствВДругуюОрганизацию))
	|	И Операция.ПроведеноБанком";
