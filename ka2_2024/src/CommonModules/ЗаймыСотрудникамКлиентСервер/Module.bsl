...
Процедура ЗаполнитьРазмерПогашения(РазмерПогашения, Сумма, Срок, ВидПлатежей, ОграничениеПлатежа, ДатаВыдачи, ИзмененияУсловий = Неопределено) Экспорт
	...
	Если День(ДатаВыдачи) = 1 
		Или Срок = 1 Тогда
		РазмерПогашения = ?(ОграничениеПлатежа = 0, Сумма / Срок, Мин(Сумма / Срок, ОграничениеПлатежа));
	Иначе
		КоличествоДнейМесяца = ЗарплатаКадрыКлиентСервер.КоличествоДнейМесяца(ДатаВыдачи);
		Если ИзмененияУсловий = Неопределено Или ИзмененияУсловий.Количество() = 0 Тогда
			ДнейВМесяцеОсновнойДолг     = День(КонецМесяца(ДатаВыдачи)) - День(ДатаВыдачи) + 1;
		...
		Иначе
			...
			// Рассчитаем погашение первого месяца с учётом текущих параметров графика.
			ПоследняяСтрокаИзменениеУсловий = ИзмененияУсловий[ИзмененияУсловий.ВГраница()];
			ПоследняяСтрокаИзменениеУсловий.ДатаИзменений = ДатаВыдачи;
