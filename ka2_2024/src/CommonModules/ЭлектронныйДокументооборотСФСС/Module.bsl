	...
	Для каждого Организация Из Организации Цикл
		...
		Для каждого ДатаПолученияСообщений из ДатыПолученияСообщений Цикл
			...
			Если ЭтоЗапросАктуальныхСоощений
				И (ТекущаяДатаНаСервере() - ДатаПолученияАктуальныхСообщений) < Сутки Тогда
				// Актуальные сообщения уже были сегодня получены.
				Продолжить;
			КонецЕсли;
			
			Если ЭтоЗапросАктуальныхСоощений Тогда
				Шаблон = НСтр("ru='Получение списка актуальных входящих сообщений из СЭДО ФСС по организации %1.'");
				Шаблон = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(Шаблон, Истина);
				ОписаниеСобытия = СтрШаблон(Шаблон, Организация);
			Иначе
			...
			Если МинимальныйИнтервалМеждуЗапросами > 60 Тогда
				Если ТекущаяДатаНаСервере() - ДатаПолученияСообщений.Дата > Сутки Тогда
