...
Функция ТекстЗапросаДоступныхОстатковПоДатамДляПодбора(ЕстьХарактеристика, СоставРазделовЗапроса) Экспорт
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|ИМЕЮЩИЕ
			|	СУММА(РаспределениеЗапасов.ВНаличии) <> 0 ИЛИ СУММА(РаспределениеЗапасов.Свободно) <> 0
			|УПОРЯДОЧИТЬ ПО
			|	СкладНаименование,
			|	Склад,
			|	Состояние,
...
Функция ВременнаяТаблицаСвободныеОстатки(ИмяТаблицыФильтра, ИмяТаблицыРезультат, ПолучатьОтрицательные = Ложь) Экспорт
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|		И &ПодстановкаЗначенияБольшеНуля
			|		И ИСТИНА В(
			|			ВЫБРАТЬ ПЕРВЫЕ 1
		...
		Если ПолучатьОтрицательные Тогда
			ПодстановкаЗначенияБольшеНуля = "ИСТИНА";
		Иначе
			ПодстановкаЗначенияБольшеНуля = "Запасы.Свободно > 0";
		КонецЕсли;
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПодстановкаЗначенияБольшеНуля", ПодстановкаЗначенияБольшеНуля);
...
Процедура ПриИзмененииПроизводительногоРежимаВФоне(Параметры, АдресРезультата) Экспорт
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		...
		Текст =
			...
			|	ИЗ
			|		РегистрНакопления.ЗапасыИПотребности.Остатки КАК Движения
			|		
			...
			|						И Сведения.Запас = Движения.ВНаличииОстаток
								// Расхождения ожидаемых поступлений.
			|					ИЛИ Движения.ПоступитОстаток <> 0
			|						И Движения.Заказ <> НЕОПРЕДЕЛЕНО
			|						И Движения.ДатаСобытия <> ДАТАВРЕМЯ(1, 1, 1)
			...
			|						И Сведения.Запас = Движения.ПоступитОстаток
								// Расхождения запланированных поступлений.
			|					ИЛИ Движения.ЗаказаноОстаток <> 0 И Движения.ЗаказаноОстаток - Движения.ПоступитОстаток > 0
			|						И Движения.Заказ <> НЕОПРЕДЕЛЕНО
			|						И Движения.ДатаСобытия = ДАТАВРЕМЯ(1, 1, 1)
			...
			|						И Сведения.ЗаказНаПоступление = Движения.Заказ
			|						И Сведения.ДатаПоступления = Движения.ДатаСобытия
			|						И Сведения.Запас = Движения.ЗаказаноОстаток - Движения.ПоступитОстаток
								// Расхождения ожидаемых отгрузок.
			|					ИЛИ (Движения.РезервироватьНаСкладеОстаток <> 0
			|							ИЛИ Движения.РезервироватьПоМереПоступленияОстаток <> 0
			|							ИЛИ Движения.ОтложитьРезервированиеОстаток <> 0
			|							ИЛИ Движения.КОбеспечениюОстаток <> 0
			|							ИЛИ Движения.НеОбеспечиватьОстаток <> 0)
			|						И Движения.Заказ <> НЕОПРЕДЕЛЕНО
			...
			|						И Сведения.ЗаказНаПоступление = НЕОПРЕДЕЛЕНО
			|						И Сведения.ДатаПоступления = ДАТАВРЕМЯ(1, 1, 1)
			|						И Сведения.Резервировать = Движения.РезервироватьНаСкладеОстаток
			|						И Сведения.РезервироватьПоМереПоступления = Движения.РезервироватьПоМереПоступленияОстаток
			|						И Сведения.ОтложитьРезервирование = Движения.ОтложитьРезервированиеОстаток
			|						И Сведения.КОбеспечениюБезРезерва = Движения.КОбеспечениюОстаток
			|						И Сведения.НеОбеспечивать = Движения.НеОбеспечиватьОстаток)
			|	ГДЕ
			|		(Движения.ВНаличииОстаток <> 0
			|					И Движения.Заказ = НЕОПРЕДЕЛЕНО
			|					И Движения.ДатаСобытия = ДАТАВРЕМЯ(1, 1, 1)
			...
			|			
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыИПотребности.Остатки КАК Движения
			...
			|					И Движения.Заказ = НЕОПРЕДЕЛЕНО
			|					И Движения.ДатаСобытия = ДАТАВРЕМЯ(1, 1, 1)
			|					И Движения.ВНаличииОстаток = Сведения.Запас
								// Расхождения ожидаемых поступлений.
			...
			|						И Движения.ПоступитОстаток = Сведения.Запас
								// Расхождения запланированных поступлений.
			|					ИЛИ Сведения.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеподтвержденное)
			|						И Сведения.Запас <> 0
			|						И Движения.Заказ = Сведения.ЗаказНаПоступление
			|						И Движения.ДатаСобытия = Сведения.ДатаПоступления
			|						И Движения.ЗаказаноОстаток <> 0 И Движения.ЗаказаноОстаток - Движения.ПоступитОстаток = Сведения.Запас
								// Расхождения ожидаемых отгрузок.
			...
			|						И Движения.РезервироватьНаСкладеОстаток = Сведения.Резервировать
			|						И Движения.РезервироватьПоМереПоступленияОстаток = Сведения.РезервироватьПоМереПоступления
			|						И Движения.ОтложитьРезервированиеОстаток = Сведения.ОтложитьРезервирование
			|						И Движения.КОбеспечениюОстаток = Сведения.КОбеспечениюБезРезерва
			|						И Движения.НеОбеспечиватьОстаток = Сведения.НеОбеспечивать)
			|	ГДЕ
			|		(Сведения.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)
			|					И Сведения.Запас <> 0
			|				ИЛИ Сведения.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление)
