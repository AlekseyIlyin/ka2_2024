...
Процедура ОткрытьПрисоединенныеФайлы(Форма, ИмяЭлемента) Экспорт 
	ДополнительныеПараметры = Новый Структура("Форма, ИмяЭлемента", Форма, ИмяЭлемента);
	Если Не ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда 
		ОО = Новый ОписаниеОповещения("ОткрытьПрисоединенныеФайлыПослеСохранения", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(ОО, "Для добавления файлов необходимо сохранить документ. Выполнить сохранение?", РежимДиалогаВопрос.ДаНет);
		Возврат;
	КонецЕсли;
	ОткрытьФормуВыбораПриложенныхФайлов(ДополнительныеПараметры);
КонецПроцедуры
...
Процедура ОткрытьПрисоединенныеФайлыПослеСохранения(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ДополнительныеПараметры.Форма.Сохранить(Неопределено);
		ОткрытьФормуВыбораПриложенныхФайлов(ДополнительныеПараметры);
	КонецЕсли;
КонецПроцедуры
