...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения , Метаданные.РегистрыНакопления.ТоварыКОформлениюЗаявленийОВвозе.Имя);
	ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения , Метаданные.РегистрыНакопления.ТоварыКОформлениюДокументовИмпорта.Имя);
...
Процедура ИнициализироватьДанныеКонтроляИзменений(Запрос, ТекстыЗапроса, Документ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияТоварыКОформлениюДокументовИмпортаИзменение") Тогда
		...
		ТекстЗапроса =
			...
			|			,
			|			(Организация, Поставщик, АналитикаУчетаНоменклатуры, ВидЗапасов, ДокументПоступления) В
			|				(ВЫБРАТЬ
			|					Таблица.Организация,
			|					Таблица.Поставщик,
...
#Область НДСПредъявленный
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС) КАК КорВидДеятельностиНДС,
	|	Ценности.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ЛОЖЬ КАК РегламентнаяОперация,
	|	Ценности.Подразделение КАК Подразделение,
...
Процедура ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(Запрос, ТекстЗапроса, ТекстЗапросаВременныхТаблиц, ИменаВременныхТаблиц, ИсточникСвойствДокумента) Экспорт
	...
	Если ИсточникСвойствДокумента <> Неопределено
		...
		ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА = 
			ИсточникСвойствДокумента.Свойство(ВнеоборотныеАктивы.ИмяПризнакаВыполненияРасчетаСтоимостиВнеоборотныхАктивов());
		
	ИначеЕсли ИсточникСвойствДокумента <> Неопределено Тогда
	
		ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА = 
			ВнеоборотныеАктивы.ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА(ИсточникСвойствДокумента);
