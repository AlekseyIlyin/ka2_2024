...
#Область ИсправлениеПрочиеРасходы
	...
	|	ДД.Партия.ОсновноеИзделиеНоменклатура КАК АналитикаРасходов,
	// Ресурсы
	|	-(ДД.Стоимость
	|		+ ДД.ДопРасходы
	|		+ ДД.Трудозатраты
	...
	|	ДД.НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.СторноПроизводственныхЗатрат) КАК СтатьяРасходов,
	|	ДД.ПартияПроизводства.ОсновноеИзделиеНоменклатура КАК АналитикаРасходов,
	// Ресурсы
...
Функция ТекстВтНезавершенноеПроизводство() Экспорт
	...
	|					- ЕСТЬNULL(СуммыРасходовРегл.ПостояннаяРазница, 0)
	|					- ЕСТЬNULL(СуммыРасходовРегл.ВременнаяРазница, 0)
	|		ИНАЧЕ
	|			0
	|		КОНЕЦ КАК СуммаНУ,
	|	ВЫБОР
	...
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	|				)
	|			ТОГДА ЕСТЬNULL(СуммыРасходовРегл.ПостояннаяРазница, 0) + ЕСТЬNULL(СуммыРасходовРегл.ВременнаяРазница, 0) 
	|					- ЕСТЬNULL(СуммыРасходовРегл.СуммаРегл, 0)
	|		ИНАЧЕ
	|			0
	...
	// должна возникать отрицательная постоянная разница на сумма НДС.
	|		(ВЫБОР
...
Процедура СкорректироватьСтоимостьСписанияЗапасов(ПараметрыРасчета) Экспорт
	...
	|			)
	|		ИЛИ УчетСебестоимости.ТипЗаписи В
	|			(ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.КорректировкаПриобретения),
	...
	|		 И (УчетСебестоимости.Количество <> 0
	|		 	ИЛИ УчетСебестоимости.Стоимость <> 0)
	...
	//-- Устарело_Переработка24
	|	И НЕ (ТИПЗНАЧЕНИЯ(УчетСебестоимости.Регистратор) В(
	//++ Устарело_Переработка24
	...
	Если ПараметрыРасчета.ФО.ИспользоватьУчетПрочихДоходовРасходов Тогда
		...
		//++ НЕ УТ
		|		ИЛИ ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровУПереработчика)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.УничтожитьВременныеТаблицы(ПараметрыРасчета, 
		"ВтТаблицаКорректировки, ВтДанныеТаблицаКорректировки, ЕстьВозвратныеОтходы,
		|СторноОтчетыКомиссионера, РеквизитыКорДвижений,
...
Процедура СформироватьДвиженияСебестоимостьТоваровСписаниеРезервовЗапросом(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений)
	...
	"ВЫБОР
	|	КОГДА ВосстанавливатьРезервы ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачислениеРезерваПодОбесценениеЗапасов)
	...
	"ВЫБОР
	|	КОГДА ВосстанавливатьРезервы ТОГДА ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.НачислениеРезерваПодОбесценениеЗапасов)
...
Процедура СформироватьДвиженияВыручкаИСебестоимостьПродажВосстановлениеРезервовЗапросом(ПараметрыРасчета, Запрос, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачислениеРезерваПодОбесценениеЗапасов) КАК ХозяйственнаяОперация,
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.НачислениеРезерваПодОбесценениеЗапасов) КАК НастройкаХозяйственнойОперации,
	|	Т.ИдентификаторФинЗаписи
...
Процедура СформироватьДвиженияСтоимостьТМЦВЭксплуатацииЗапросом(ПараметрыРасчета, Запрос, ИмяВременнойТаблицы, СоответствиеВременныхТаблицДвижений)
	...
	|	ПередачиВЭксплуатацию.Серия КАК Серия,
	...
	ИмяРегистра = Метаданные.РегистрыСведений.СтоимостьТМЦВЭксплуатации.Имя;
	ОписаниеПриемника = ПараметрыРасчета.Движения[ИмяРегистра];
	КопируемыеПоля = РасчетСебестоимостиУниверсальныеАлгоритмы.ПолучитьИменаКолонокВременнойТаблицыСтрокой(ПараметрыРасчета, "ВтТаблицаСтоимостьТМЦВЭксплуатации");
	
	ТаблицаОписанияПолей = РасчетСебестоимостиПрикладныеАлгоритмы.СформироватьОписаниеПолейЗапроса(ОписаниеПриемника.Таблица);
	РасчетСебестоимостиПрикладныеАлгоритмы.КопироватьПоляЗапроса(ТаблицаОписанияПолей, КопируемыеПоля);
	
	РасчетСебестоимостиПрикладныеАлгоритмы.ВыполнитьЗапросПоОписаниюПолей(ПараметрыРасчета, ТаблицаОписанияПолей, 
		ИмяРегистра, "ВтТаблицаСтоимостьТМЦВЭксплуатации", ИмяВременнойТаблицы);
...
Процедура СкорректироватьСтоимостьСписанияРезервовПредстоящихРасходов(ПараметрыРасчета)
	...
	Запрос.Текст = 
		...
		|	Движение.СуммаНУ КАК СуммаНУ,
		|	Движение.СуммаРегл - Движение.СуммаНУ КАК ВременнаяРазница,
...
Процедура СписатьОтклоненияВСтоимостиТоваров(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей,
		...
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.НачислениеРезерваПодОбесценениеЗапасов)
		|	КОНЕЦ");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей,
		"НастройкаХозяйственнойОперации",
		"ВЫБОР КОГДА ЭтоРасходы
		...
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.НачислениеРезерваПодОбесценениеЗапасов)
		|	КОНЕЦ");
	РасчетСебестоимостиПрикладныеАлгоритмы.ИзменитьПолеВОписанииПолейЗапроса(ТаблицаОписанияПолей,
		"ТипЗаписи",
		"ВЫБОР КОГДА ЭтоРасходы
...
#Область ПрочиеРасходы_КорДвижения
	...
	|		Т.СуммаРегл,
	|		0 КАК ПостояннаяРазница,
	|		Т.СуммаРегл - Т.СуммаНУ КАК ВременнаяРазница,
	|		Т.СуммаУпр
	|	ИЗ
