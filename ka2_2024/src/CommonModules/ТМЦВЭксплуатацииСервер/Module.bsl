...
Процедура ПроверитьПередачуВЭксплуатацию(Объект, НепроверяемыеРеквизиты, Отказ) Экспорт
	...
	ТаблицаДокумента = Объект[ИмяТЧ].Выгрузить(, СписокКолонок);
...
// Заполняет партию ТМЦВЭксплуатации.
// 
// Параметры:
//  ТекущаяСтрока - ДанныеФормыСтруктура - Текущая строка.
//  СтруктураДействий - Структура - Структура действий.
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	УчетТМЦВЭксплуатацииWE_ПриДобавленииОбработчиковОбновления(Обработчики);

	//++ Локализация
	УчетТМЦВЭксплуатацииУПКА_ПриДобавленииОбработчиковОбновления(Обработчики);
	//-- Локализация
...
#Область УстановкаКонстанты_УчетТМЦВЭксплуатацииWE

Процедура УчетТМЦВЭксплуатацииWE_ПриДобавленииОбработчиковОбновления(Обработчики)
	
	Если НЕ ПолучитьФункциональнуюОпцию("ЛокализацияРФ") Тогда
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Процедура = "ТМЦВЭксплуатацииСервер.УчетТМЦВЭксплуатацииWE_ОбработатьДанныеДляПереходаНаНовуюВерсию";
		Обработчик.Версия = "11.5.9.24";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("fbb4a016-f12b-4995-b6e1-de3233c07696");
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "ТМЦВЭксплуатацииСервер.УчетТМЦВЭксплуатацииWE_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
		Обработчик.ПроцедураПроверки = "";
		Обработчик.Комментарий = НСтр("ru = 'Устанавливает константу ""Учет ТМЦ в эксплуатации WE (служебная)""'");
		
		Читаемые = Новый Массив;
		Читаемые.Добавить(Метаданные.Константы.УчетТМЦВЭксплуатацииWE.ПолноеИмя());
		Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
		
		Изменяемые = Новый Массив;
		Изменяемые.Добавить(Метаданные.Константы.УчетТМЦВЭксплуатацииWE.ПолноеИмя());
...
Процедура УчетТМЦВЭксплуатацииWE_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
...
Процедура УчетТМЦВЭксплуатацииWE_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	Если НЕ ПолучитьФункциональнуюОпцию("ЛокализацияРФ") Тогда
	
		Если НЕ Константы.УчетТМЦВЭксплуатацииWE.Получить() Тогда
			МенеджерЗначения = Константы.УчетТМЦВЭксплуатацииWE.СоздатьМенеджерЗначения();
...
#Область УстановкаКонстанты_УчетТМЦВЭксплуатацииУПКА

Процедура УчетТМЦВЭксплуатацииУПКА_ПриДобавленииОбработчиковОбновления(Обработчики)
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "ТМЦВЭксплуатацииСервер.УчетТМЦВЭксплуатацииУПКА_ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.9.24";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("31f8130a-8b89-4467-8dcc-eb4301e5979a");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ТМЦВЭксплуатацииСервер.УчетТМЦВЭксплуатацииУПКА_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "";
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает константу ""Учет ТМЦ в эксплуатации УП и КА (служебная)""'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.Константы.УчетТМЦВЭксплуатацииУПКА.ПолноеИмя());
	Обработчик.ЧитаемыеОбъекты = СтрСоединить(Читаемые, ",");
	
	Изменяемые = Новый Массив;
	Изменяемые.Добавить(Метаданные.Константы.УчетТМЦВЭксплуатацииУПКА.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
КонецПроцедуры

Процедура УчетТМЦВЭксплуатацииУПКА_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
...
Процедура УчетТМЦВЭксплуатацииУПКА_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	Если НЕ Константы.УчетТМЦВЭксплуатацииУПКА.Получить() Тогда
		МенеджерЗначения = Константы.УчетТМЦВЭксплуатацииУПКА.СоздатьМенеджерЗначения();
		МенеджерЗначения.Значение = Истина;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(МенеджерЗначения);
	КонецЕсли;
