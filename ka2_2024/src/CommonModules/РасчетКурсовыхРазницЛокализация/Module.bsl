...
Процедура ДополнитьПроверкуТребуетсяПереоценка(РегистрыДляРасчета, ТекстЗапроса) Экспорт
	...
	Если РегистрыДляРасчета.Свойство("ДенежныеДокументы") Тогда
		...
		|	0 КАК ОстатокНУ,
		|	ЛОЖЬ КАК ТребуетсяПереоценкаНУ,
...
Функция ТекстПрочиеРасходыРасчетыСКлиентамиНовыеВзаиморасчеты()
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	ЕСТЬNULL(УчетнаяПолитикаОрганизаций.КоэффициентПрименения67ФЗ-1, 1) * ВЫБОР
	|		КОГДА Расчеты.ДолгРегл < 0
	|		ТОГДА -Расчеты.ДолгРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	ЕСТЬNULL(УчетнаяПолитикаОрганизаций.КоэффициентПрименения67ФЗ, 0) * ВЫБОР
	|		КОГДА Расчеты.ДолгРегл < 0
	|		ТОГДА -Расчеты.ДолгРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаВРДт,
