...
//       * СтатьяДоходов  - ПланВидовХарактеристикСсылка.СтатьиДоходов, Неопределено  - статья доходов,
//       * АналитикаДоходов - Характеристика.СтатьиДоходов, Неопределено - аналитика статьи доходов,
//       * СтатьяРасходов - ПланВидовХарактеристикСсылка.СтатьиРасходов, Неопределено - статья расходов,
//       * АналитикаРасходов - Характеристика.СтатьиРасходов, Неопределено - аналитика статьи расходов.
//
Функция СтатьиДоходовИРасходовПоУмолчанию(Знач Организация, Знач ВидОперации, Знач ВидРезерва) Экспорт
	
	СтатьиПоУмолчанию = Новый Структура("СтатьяДоходов, АналитикаДоходов, СтатьяРасходов, АналитикаРасходов");
	...
	|	РезервыПоОплатеТруда.СтатьяРасходов КАК СтатьяРасходов,
	|	РезервыПоОплатеТруда.АналитикаРасходов КАК АналитикаРасходов,
	|	ВЫБОР
	...
	|
	|	ВЫБРАТЬ
	|		АналитикаДругихДокументов.СтатьяРасходов КАК Статья,
	|		АналитикаДругихДокументов.АналитикаРасходов КАК Аналитика,
	|		ТИП(ПланВидовХарактеристик.СтатьиРасходов) КАК ТипСтатьи,
	|		АналитикаДругихДокументов.Приоритет КАК Приоритет,
	|		АналитикаДругихДокументов.Дата КАК Дата
	|	ИЗ
	|		ВТАналитикаДругихДокументов КАК АналитикаДругихДокументов
	|	ГДЕ
	|		АналитикаДругихДокументов.СтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.СледующийПоЗначениюПоля("ТипСтатьи") Цикл
		
		Если Выборка.ТипСтатьи = Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов") Тогда
			СтатьиПоУмолчанию.СтатьяДоходов = Выборка.Статья;
			СтатьиПоУмолчанию.АналитикаДоходов = Выборка.Аналитика;
		ИначеЕсли Выборка.ТипСтатьи = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
			СтатьиПоУмолчанию.СтатьяРасходов = Выборка.Статья;
			СтатьиПоУмолчанию.АналитикаРасходов = Выборка.Аналитика;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Начисление
		...
		Если ЕстьСписания И НЕ ЗначениеЗаполнено(Объект.СтатьяДоходов) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru='Не заполнено поле ""Статья доходов""'"), Объект, "СтатьяДоходов", , Отказ);
		КонецЕсли;
		
		Если ЕстьНачисления И НЕ ЗначениеЗаполнено(Объект.СтатьяРасходов)
			И Объект.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru='Не заполнено поле ""Статья расходов""'"), Объект, "СтатьяРасходов", , Отказ);
		КонецЕсли;
...
Процедура ДобавитьТекстЗапросаВТДанныеДокументов(ТекстыЗапроса)
	...
	|	&АналитикаРасходов КАК АналитикаСтатьиРасходов,
...
Процедура ДобавитьТекстЗапросаВТДанныеДокументовРегл(ТекстыЗапроса)
	...
	|	ДанныеДокумента.АналитикаРасходов КАК АналитикаСтатьиРасходов,
...
Процедура ДобавитьТекстЗапросаВТРасходыДокумента(ТекстыЗапроса)
	...
	|
	|ГДЕ
	|	ДанныеДокументов.ВидОперацииДокумента = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Начисление)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Доначисление резерва по результатам инвентаризации
	|ВЫБРАТЬ
	|	ОценочныеОбязательства.ВидНачисления,
	|	ОценочныеОбязательства.Организация,
	|	ОценочныеОбязательства.Подразделение,
	...
	|
	|ГДЕ
	|	ДанныеДокументов.ВидОперацииДокумента = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация)
	|	И (ОценочныеОбязательства.Сумма > 0 ИЛИ ОценочныеОбязательства.СуммаНУ > 0)";
...
Процедура ДобавитьТекстЗапросаВТРасходыРаспределяемые(ТекстыЗапроса)
	...
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыОценочныхОбязательств.ОценочныеОбязательстваПоВознаграждениям)
	|			ТОГДА ПорядокОтражения.ОтдельнаяСтатьяКалькуляции
	...
	|	ВЫБОР ОценочныеОбязательства.ТипОбязательства
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыОценочныхОбязательств.ОценочныеОбязательстваПоВознаграждениям)
	|			ТОГДА ПорядокОтражения.СтатьяКалькуляции
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыОценочныхОбязательств.ОценочныеОбязательстваПоСтраховымВзносам)
	...
	|
	|	ВЫБОР
	|		КОГДА ОценочныеОбязательства.СуммаУпр >= 0 ТОГДА ОценочныеОбязательства.СуммаУпр
	|		КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Начисление) ТОГДА 0
	|		ИНАЧЕ ОценочныеОбязательства.СуммаУпр
	|	КОНЕЦ КАК СуммаУпр,
	|	ВЫБОР
	|		КОГДА ОценочныеОбязательства.Сумма >= 0 ТОГДА ОценочныеОбязательства.Сумма
	|		КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Начисление) ТОГДА 0
	|		ИНАЧЕ ОценочныеОбязательства.Сумма
	|	КОНЕЦ КАК СуммаРегл,
	|	ВЫБОР
	|		КОГДА ОценочныеОбязательства.СуммаНУ >= 0 ТОГДА ОценочныеОбязательства.СуммаНУ
	|		КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Начисление) ТОГДА 0
	|		ИНАЧЕ ОценочныеОбязательства.СуммаНУ
	|	КОНЕЦ КАК СуммаНУ,
	|
	...
	|
	|ГДЕ
	|	&ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация)
	|	И НЕ (ОценочныеОбязательства.ТипОбязательства = ЗНАЧЕНИЕ(Перечисление.ТипыОценочныхОбязательств.ОценочныеОбязательстваПоВзносамФССНСиПЗ)
...
Процедура ДобавитьТекстЗапросаВТРасходыПостатейные(ТекстыЗапроса)
	...
	|	ДанныеДокументов.СтатьяРасходов,
	|	ВЫБОР ДанныеДокументов.АналитикаСтатьиРасходов
	|		КОГДА ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) ТОГДА ОценочныеОбязательства.Подразделение
	|		КОГДА ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка) ТОГДА ОценочныеОбязательства.ФизическоеЛицо
	|		КОГДА ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) ТОГДА ОценочныеОбязательства.Организация
	|		ИНАЧЕ ДанныеДокументов.АналитикаСтатьиРасходов
	|	КОНЕЦ,
	|	ДанныеДокументов.СтатьяРасходов.ПринятиеКНалоговомуУчету,
	|
	|	ОценочныеОбязательства.СтатьяАктивовПассивов,
	|	ОценочныеОбязательства.АналитикаСтатьиАктивовПассивов,
	|
