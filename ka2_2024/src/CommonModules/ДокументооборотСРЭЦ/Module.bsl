...
Функция ОтправитьОтчет(КонтекстОтправки, СоединениеHTTP = Неопределено) Экспорт
	...
	Для Каждого Пакет Из ДанныеДляВыгрузки Цикл		  
		...
		Попытка     
			...
			Если ТипЗнч(СсылкаНаОтчет) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
				НайденныеСтроки = ОтправкаОбъект.ОтчетыСсылки.НайтиСтроки(Новый Структура("ИсточникОтчета, ТипФайла", СсылкаНаОтчет.ИсточникОтчета, Пакет.ТипФайлаВыгрузки));
			Иначе
			...
			Если ТипЗнч(СсылкаНаОтчет) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
				...
				СтрокаТЧ.ИсточникОтчета = СсылкаНаОтчет.ИсточникОтчета;
			Иначе
