...
Функция ПолучитьДанныеДляПечатнойФормыМХ18(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|		Документ.ДвижениеПродукцииИМатериалов КАК ТабДокументы
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОтносительныеКурсыВалют КАК КурсыВалют
	|			ПО ТабДокументы.Валюта = КурсыВалют.Валюта
	|				И ТабДокументы.Организация.ВалютаРегламентированногоУчета = КурсыВалют.БазоваяВалюта
	|				И ТабДокументы.Дата >= КурсыВалют.Период
	|	ГДЕ
	|		ТабДокументы.Ссылка В(&МассивДокументов)
	...
	|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(Цены.Цена, 0) 
	|				/ ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) 
	|				* ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) 
	|				* ЕСТЬNULL(КурсыВалютВидаЦен.КурсЧислитель, 1)
	|				/ ЕСТЬNULL(КурсыВалютВидаЦен.КурсЗнаменатель, 1) КАК ЧИСЛО(31,2))
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТовары.Цена
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТовары.Сумма
	|				* ЕСТЬNULL(КурсыВалютДокумента.КурсЧислитель, 1)
	|				/ ЕСТЬNULL(КурсыВалютДокумента.КурсЗнаменатель, 1) КАК ЧИСЛО(31,2))
	|	КОНЕЦ																КАК Сумма,
	...
	// 		[2] - Временная таблица по курсам валют вида цен
	// 		[3] - Временная таблица по курсам валют документов
	// 		[4] - Выборка по шапкам документов
	// 		[5] - Выборка по табличным частям документов.
...
Функция ТекстПередачаТоваровЗаБалансомСОбособленногоПодразделения()
	...
	ТекстПередачаТоваровЗаБалансомСОбособленногоПодразделения = "
	|ВЫБРАТЬ //// Перемещение товаров, принятых на хранение с обособленного подразделения (Кт 002, 003, 004.01)
	|	Строки.Ссылка КАК Ссылка,
	|	Строки.Период КАК Период,
	|	Строки.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	Возврат ТекстПередачаТоваровЗаБалансомСОбособленногоПодразделения;
	
КонецФункции

Функция ТекстПередачаТоваровЗаБалансомНаОбособленноеПодразделение()
	
	ТекстПередачаТоваровЗаБалансомНаОбособленноеПодразделение = "
	|ВЫБРАТЬ //// Перемещение товаров за балансом на обособленное подразделение (Дт 002, 003, 004.01)
	|	Строки.Ссылка КАК Ссылка,
	|	Строки.Период КАК Период,
	|	Строки.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|ГДЕ
	|	Строки.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПеремещениеПолуфабрикатовМеждуФилиалами)";
	Возврат ТекстПередачаТоваровЗаБалансомНаОбособленноеПодразделение;
