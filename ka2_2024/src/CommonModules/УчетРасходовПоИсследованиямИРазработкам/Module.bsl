...
Функция ТекстЗапросаПрочиеРасходы(Запрос)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДействующиеУчетныеПолитики_ПрочиеРасходы КАК вт_УчетныеПолитикиБУ
	|		ПО вт_ТаблицаИзменений.Организация = вт_УчетныеПолитикиБУ.Организация
	|ГДЕ
	|	вт_УчетныеПолитикиБУ.ПрименяетсяПризнаниеПоИсследованиям = ИСТИНА";
	...
	|		ПО вт_ТаблицаИзменений.Организация = РеестрДокументов.Организация
...
Функция ПрименяетсяПовышающийКоэффициент(ВидПризнанияРасходов)
	...
	Если ВидПризнанияРасходов = Перечисления.ВидыПризнанияРасходовПоИсследованиямИРазработкам.ПереносНаДругуюТему
		Или ВидПризнанияРасходов = Перечисления.ВидыПризнанияРасходовПоИсследованиямИРазработкам.ВключитьВСоставПроизводственныхЗатрат Тогда
...
Процедура СформироватьЗаданияКРасчетуСтоимостиВНА(НачалоПериода, КонецПериода, МенеджерВТ)
	...
	ВнеоборотныеАктивы.СформироватьЗаданияКРасчетуСтоимостиВНА(Неопределено, ДанныеТаблиц);
