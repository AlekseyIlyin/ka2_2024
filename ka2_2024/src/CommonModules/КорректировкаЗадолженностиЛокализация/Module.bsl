...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	"ВЫБРАТЬ
	|	Операция.Ссылка КАК Ссылка,
	|	МАКСИМУМ(ДоходыРасходы.НомерСтроки = 1 И (ДоходыРасходы.Статья = Операция.УдалитьСтатьяРасходов ИЛИ ДоходыРасходы.Статья = Операция.УдалитьСтатьяДоходов)) КАК СтатьяСовпадаетСШапкой,
	...
	|	Расчеты.Валюта КАК Валюта,
	|	Расчеты.ИдентификаторФинЗаписи = """" ИЛИ ДоходыРасходы.СтатьяСовпадаетСШапкой КАК СтарыйАлгоритм,
	...
	|	Расчеты.Валюта,
	|	Расчеты.ИдентификаторФинЗаписи = """" ИЛИ ДоходыРасходы.СтатьяСовпадаетСШапкой,
	|	ДоходыРасходы.НесколькоСтрок,
	|	ВЫБОР КОГДА Расчеты.ИдентификаторФинЗаписи = """" ИЛИ ДоходыРасходы.СтатьяСовпадаетСШапкой ТОГДА Расчеты.Долг <> 0 ИНАЧЕ ЛОЖЬ КОНЕЦ
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка";
