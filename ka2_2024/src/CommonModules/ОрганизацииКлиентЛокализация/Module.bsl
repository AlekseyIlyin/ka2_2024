...
Процедура ОткрытьПараметрыПФРОбъектЗаписан(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.ПФРСохраненныеЗначения) Тогда
		Форма.ПФРСохраненныеЗначения = ОрганизацииВызовСервераЛокализация.РегистрацииПФРВХранилище(Форма.Объект.Ссылка, Форма.УникальныйИдентификатор);
	КонецЕсли;
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ВидОрганизации", Форма.ВидОрганизации);
	СтруктураПараметров.Вставить("КодОрганаПФР", Форма.КодОрганаПФР);
	СтруктураПараметров.Вставить("НаименованиеТерриториальногоОрганаПФР", Форма.НаименованиеТерриториальногоОрганаПФР);
	СтруктураПараметров.Вставить("РегистрационныйНомерПФР", Форма.РегистрационныйНомерПФР);
	СтруктураПараметров.Вставить("ИПРегистрационныйНомерПФР", Форма.ИПРегистрационныйНомерПФР);
