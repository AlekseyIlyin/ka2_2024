...
Процедура ЗаписатьСообщениеОбОшибкеПроверкиРегистрационногоНомераФССВЖурналРегистрации(Результат, РегистрационныйНомерФСС, СвойстваОрганизации, Отправка, ЕстьНеЦифры = Ложь)
	
	Шаблон = "При %1 сообщения СЭДО ФСС произошла ошибка:
		|%2";
	
	Если НЕ СвойстваОрганизации = Неопределено 
		И СвойстваОрганизации.Свойство("ЭтоДополнительныйКодФСС") Тогда
		
		Если ЕстьНеЦифры Тогда
			ОписаниеОшибки = СтрШаблон(Шаблон,
				?(Отправка, "отправке", "получении"),
				"Указан недопустимый дополнительный код обособленного подразделения страхователя %1. Дополнительный код должен состоять только из цифр.");
		Иначе
			ОписаниеОшибки = СтрШаблон(Шаблон,
				?(Отправка, "отправке", "получении"),
				"Указан недопустимый дополнительный код обособленного подразделения страхователя %1. Текущая длина номера %2, минимальная длина 10 символов.");
		КонецЕсли;
	Иначе
		Если ЕстьНеЦифры Тогда
			ОписаниеОшибки = СтрШаблон(Шаблон,
				?(Отправка, "отправке", "получении"),
				"Указан недопустимый регистрационый номер страхователя %1. Регистрационый номер должен состоять только из цифр.");
		Иначе
			ОписаниеОшибки = СтрШаблон(Шаблон,
				?(Отправка, "отправке", "получении"),
				"Указан недопустимый регистрационый номер страхователя %1. Текущая длина номера %2, минимальная длина 10 символов.");
		КонецЕсли;
	КонецЕсли;
	ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, РегистрационныйНомерФСС, СтрДлина(РегистрационныйНомерФСС));
	
	ШаблонДляЖурналаРегистрации = "Электронный документооборот с контролирующими органами. %1";
	Если Отправка Тогда
