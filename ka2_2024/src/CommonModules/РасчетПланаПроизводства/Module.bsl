...
Процедура ПроброситьПеремещения(МатериалыИУслуги) Экспорт
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Если Уровень > МаксГлубинаРасчета Тогда
			Шаблон = НСтр("ru = 'Превышено максимальное количество перемещений (%1)'");
			ТекстСообщения = СтрШаблон(Шаблон, МаксГлубинаРасчета);
		...
		Для Индекс = -Результат.Количество() + 1 По 0 Цикл
			...
			Материал = МатериалыИУслуги.Найти(Строка.НомерСтроки, "НомерСтроки");
