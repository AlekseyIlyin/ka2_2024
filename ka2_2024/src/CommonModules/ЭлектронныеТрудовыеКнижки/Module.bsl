...
Процедура СформироватьДвиженияМероприятийТрудовойДеятельности(МероприятияТрудовойДеятельности, ДанныеДляПроведения, Знач ПараметрыФормирования = Неопределено) Экспорт
	...
	Если ДанныеДляПроведения <> Неопределено Тогда
		...
		Если ПараметрыФормирования.ДополнитьСведениямиОЗанятости
			...
			ПоляОтбораПериодическихДанных = Новый Структура("МероприятияТрудовойДеятельностиПереданные", ПараметрыМероприятияТрудовойДеятельностиПереданные);
		...
		Для Каждого ДанныеСотрудника Из ДанныеДляПроведения Цикл
			...
			Если ПараметрыФормирования.ДополнитьСведениямиОбУсловияхТрудовогоДоговора Тогда
				СвойстваДоговора = СвойстваДоговоров.Получить(ДанныеСотрудника.Сотрудник);
...
Функция СвойстваДоговоровСотрудников(КадровыеДанныеСотрудников, ДанныеДляПроведения = Неопределено) Экспорт
	...
	Для Каждого КадровыеДанныеСотрудника Из КадровыеДанныеСотрудников Цикл
		...
		СвойстваДоговоров.Вставить(КадровыеДанныеСотрудника.Сотрудник, ДанныеСотрудника);
...
Функция ПредставлениеВыполняемойФункции(ВыборкаДанных) Экспорт 
	...
	Если ВыборкаДанных.ЯвляетсяСовместителем
...
Процедура ЗаполнитьПредставлениеКодаВыполняемойФункции(СтрокаПодраздела11, ВыборкаДанных) Экспорт
	...
	Если ВыборкаДанных.Дата >= ПерсонифицированныйУчетКлиентСервер.ДатаПостановленияЕФС1_2023() Тогда
		УдаленностьРаботы = ПредставлениеУдаленности(ВыборкаДанных);
		Если УдаленностьРаботы <> Неопределено Тогда
			СоставПредставления.Добавить(УдаленностьРаботы);
		КонецЕсли;
		СокращенностьГрафика = ПредставлениеСокращенностиГрафика(ВыборкаДанных);
		Если СокращенностьГрафика <> Неопределено Тогда
			СоставПредставления.Добавить(СокращенностьГрафика);
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеУдаленности(ВыборкаДанных) Экспорт
...
Функция ПредставлениеСокращенностиГрафика(ВыборкаДанных) Экспорт
