...
Функция ЗапросОбработкиДанных() Экспорт
	...
	|			ТОГДА Данные.ВалютнаяСуммаДт - ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.Сумма, 0) 
	|		ИНАЧЕ
	|			Данные.ВалютнаяСуммаДт + ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.Сумма, 0) 
	|	КОНЕЦ КАК ВалютнаяСуммаДт,
	|	Данные.КоличествоДт              КАК КоличествоДт,
	|	ВЫБОР
	...
	|			ТОГДА Данные.ВалютнаяСуммаКт - ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.Сумма, 0) 
	|		ИНАЧЕ
	|			Данные.ВалютнаяСуммаКт  + ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.Сумма, 0)
	|	КОНЕЦ КАК ВалютнаяСуммаКт,
	|	Данные.КоличествоКт             КАК КоличествоКт,
	|	Данные.СуммаНУКт                КАК СуммаНУКт,
	|	Данные.СуммаПРКт                КАК СуммаПРКт,
	...
	|				ИНАЧЕ -ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.СуммаУпр, ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.СуммаУпр, 0))
	|			КОНЕЦ
...
Функция ЗапросОчистки(ИмяДокумента) Экспорт
	...
	|УНИЧТОЖИТЬ РаздельныйУчет_НастройкиНалогообложенияПрибылиПостобработка;
	|УНИЧТОЖИТЬ КурсыВалют;
	|УНИЧТОЖИТЬ ВтСтроки;
	|УНИЧТОЖИТЬ ВтСтоимости;
	|УНИЧТОЖИТЬ Партии;
	|УНИЧТОЖИТЬ ВТВыручкаИСебестоимостьПродаж;
	|УНИЧТОЖИТЬ РасчетыСКлиентамиНоваяАрхитектураДетально;
	|УНИЧТОЖИТЬ РасчетыСПоставщикамиНоваяАрхитектураДетально;
	...
	|УНИЧТОЖИТЬ РаздельныйУчетНастройкиНалогообложенияПрибыли;";
	...
	Если ИмяДокумента <> "РегистраторРасчетов" Тогда
		
		СтруктураДанных = РасчетСебестоимостиНДС.ИнициализироватьСтруктуруДанныхДетализацииПартийНДСДляРеглУчета(ИмяДокумента);
		
		ВТЗапросаДанных = РеглУчетВыборкиСерверПовтИсп.ВТЗапроса(РасчетСебестоимостиНДС.ТекстЗапросаДетализацияПартийНДС(,,,,СтруктураДанных));
...
#Область ВтСтроки
	...
	|		КОГДА Движения.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажи)
	...
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)
	|		КОГДА Движения.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажи)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ТоварыКомитентовКОформлениюПоЗакупке)
	|		КОГДА Движения.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ТоварыНаСкладе)
	|		КОГДА Движения.КорРазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажи)
...
Функция ТекстПартии() Экспорт
	...
	ТекстЗапроса =
		...
		|	ВЫБОР 
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.ВидДеятельностиНДС ИНАЧЕ Т.КорВидДеятельностиНДС КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров)
		|				И Т.ДатаРегистратора < &ДатаНачалаДействия150ФЗ
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) 
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.ВидДеятельностиНДС ИНАЧЕ Т.КорВидДеятельностиНДС КОНЕЦ В (&ТипыНеОблагаемыеНДС)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
		|		ИНАЧЕ
		|			ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.ВидДеятельностиНДС ИНАЧЕ Т.КорВидДеятельностиНДС КОНЕЦ
		|	КОНЕЦ КАК НалогообложениеПартии,
		...
		|	ВЫБОР 
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.КорВидДеятельностиНДС ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров)
		|				И Т.ДатаРегистратора < &ДатаНачалаДействия150ФЗ
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт) 
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.КорВидДеятельностиНДС ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ В (&ТипыНеОблагаемыеНДС)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.КорВидДеятельностиНДС ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.РеализацияРаботУслугНаЭкспорт)
		|		 И Т.ДатаРегистратора >= &ДатаВозможностиВычетаПриОказанииУслугНеВРФ
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.КорВидДеятельностиНДС ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.РеализацияРаботУслугНаЭкспорт)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
		|		ИНАЧЕ
		|			ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.КорВидДеятельностиНДС ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ 
		|	КОНЕЦ КАК НалогообложениеНДС,
		|	ВЫБОР
		...
		|	ВЫБОР 
		|		КОГДА Т.ДатаРегистратора >= &ДатаНачалаУчетаНДСПоНаправлениямДеятельности
		|			ТОГДА ЕСТЬNULL(Т.КорАналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности, Т.КорНаправлениеДеятельности)
		|		КОГДА ВЫБОР КОГДА НЕ Т.ЭтоРаспределениеПрочихЗатрат ТОГДА Т.КорВидДеятельностиНДС ИНАЧЕ Т.ВидДеятельностиНДС КОНЕЦ В (&ТипыНеОблагаемыеНДС)
		|			ТОГДА ЕСТЬNULL(Т.КорАналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности, Т.КорНаправлениеДеятельности)
		|		ИНАЧЕ
		|			ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	...
	ТекстЗапроса =
		...
		|	ЕСТЬNULL(ВЫРАЗИТЬ(Т.Партия КАК Справочник.ПартииПроизводства).НаправлениеДеятельности,
...
Функция ТекстДолгосрочнаяЗадолженность(ДокументИмя)
	...
	Если ПолучитьФункциональнуюОпцию("ВыделятьДолгосрочныеАктивыОбязательства") Тогда
		...
		Если ЭтоРасчетныйДокумент(ДокументИмя) Тогда
			...
			|ИЗ
			|	РасчетыСКлиентамиНоваяАрхитектураДетально КАК Расчеты
			|	ЛЕВОЕ СОЕДИНЕНИЕ РасчетыСКлиентамиНоваяАрхитектураДетально КАК РасчетыКраткосрочные
		...
		Если ДокументИмя = "ЗаключениеДоговораАренды" ИЛИ ДокументИмя = "ИзменениеУсловийДоговораАренды"
			...
			|	УсловияДоговоровАрендыАктуальные.СуммаУслугПоАренде * 100 / (100 + УсловияДоговоровАрендыАктуальные.СтавкаНДС.Ставка) КАК
			|		СуммаУслугПоАренде
			...
			|	УсловияДоговоровАренды.СуммаУслугПоАренде * 100 / (100 + УсловияДоговоровАренды.СтавкаНДС.Ставка) КАК СуммаУслугПоАренде
			...
			|					ТОГДА 0
			|				ИНАЧЕ ВЫРАЗИТЬ((ЕСТЬNULL(УсловияДоговораАктуальные.СуммаУслугПоАренде, 0) -
			|					ЕСТЬNULL(ВтУслугиПоАренде.УслугаПоАренде, 0) - ЕСТЬNULL(ВтПроценты.Проценты, 0) -
			|					(ЕСТЬNULL(УсловияДоговораДо.СуммаУслугПоАренде, 0) - ЕСТЬNULL(ВтУслугиПоАрендеДо.УслугаПоАренде, 0) -
			|					ЕСТЬNULL(ВтПроцентыДо.Проценты, 0))) * КурсыВалют.Курс / КурсыВалютУпр.Курс КАК ЧИСЛО(31, 2))
			|			КОНЕЦ
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК СуммаУпр,
			|	ВЫБОР
			...
			|					ЕСТЬNULL(ВтУслугиПоАренде.УслугаПоАренде, 0) - ЕСТЬNULL(ВтПроценты.Проценты, 0) -
			|					(ЕСТЬNULL(УсловияДоговораДо.СуммаУслугПоАренде, 0) - ЕСТЬNULL(ВтУслугиПоАрендеДо.УслугаПоАренде, 0) -
			|					ЕСТЬNULL(ВтПроцентыДо.Проценты, 0))) * КурсыВалют.Курс КАК ЧИСЛО(31, 2))
			|			КОНЕЦ
			|		Иначе 0
			|	конец КАК СуммаРегл
			|ИЗ
...
#Область ОтклоненияЗабаланс // Отклонения в забалансе (Дт <> :: Кт 002,004 )
	...
	|		КОГДА Строки.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку)
	|			И (ЕСТЬNULL(Аналитика.МестоХранения.ЦеховаяКладовая, ЛОЖЬ) ИЛИ Аналитика.ТипМестаХранения = ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Подразделение))
