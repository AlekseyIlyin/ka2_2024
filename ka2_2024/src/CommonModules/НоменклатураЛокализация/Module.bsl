...
Процедура ПриДобавленииОбработчиковОбновленияСерий(Обработчики) Экспорт
	...
	НоваяСтрока.Процедура = "РегистрНакопления.ДвиженияСерийТоваров.ОбработатьДанныеДляПереходаНаНовуюВерсию";
...
Процедура ОбработатьСерииГИСМ(Параметры) Экспорт
	...
	|	СерииНоменклатуры.НомерКизГИСМ КАК ЗначениеШтрихкода,
	|	СозданныеШтрихкоды.Ссылка КАК ШтрихкодУпаковки,
	|	СозданныеШтрихкоды.Номенклатура КАК НоменклатураУпаковки
	|ПОМЕСТИТЬ СерииИШтрихкоды
	|ИЗ
	|	&ВременнаяТаблицаСсылок КАК СсылкиДляОбработки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК СерииНоменклатуры
	|		ПО СсылкиДляОбработки.Ссылка = СерииНоменклатуры.Ссылка
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СерииИШтрихкоды.Серия КАК Серия,
	|	МАКСИМУМ(СерииИШтрихкоды.ЗначениеШтрихкода) КАК ЗначениеШтрихкода,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СерииИШтрихкоды КАК СерииИШтрихкоды
	|		ПО СерииИШтрихкоды.Серия = ДвиженияСерийТоваров.Серия
	|ГДЕ
	|	ДвиженияСерийТоваров.Количество <> 0
	|	И ДвиженияСерийТоваров.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха)
	|СГРУППИРОВАТЬ ПО
	|	СерииИШтрихкоды.Серия
	|ИНДЕКСИРОВАТЬ ПО
	|	Серия,
	|	Период
	|;
	...
	|ПОМЕСТИТЬ ДанныеШтрихкодыУпаковок
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СерииИШтрихкоды.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
	|	СерииИШтрихкоды.Серия КАК Серия,
	|	ДанныеШтрихкодыУпаковок.Штрихкод КАК Штрихкод,
	|	ДанныеШтрихкодыУпаковок.Номенклатура КАК Номенклатура,
	|	ДанныеШтрихкодыУпаковок.Характеристика КАК Характеристика,
	|	ДанныеШтрихкодыУпаковок.Количество КАК Количество,
	|	ДанныеШтрихкодыУпаковок.ВидПродукции КАК ВидПродукции,
	|	ДанныеШтрихкодыУпаковок.ТипУпаковки КАК ТипУпаковки,
	|	ВЫБОР
	|		КОГДА СерииИШтрихкоды.ШтрихкодУпаковки ЕСТЬ NULL
	|				И ДанныеШтрихкодыУпаковок.Серия ЕСТЬ NULL
	|			ТОГДА 0 // действий не требуется
	|		КОГДА СерииИШтрихкоды.ШтрихкодУпаковки ЕСТЬ NULL
	|			ТОГДА 1 // создать штрихкод упаковки
	|		КОГДА СерииИШтрихкоды.НоменклатураУпаковки.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха)
	|			ТОГДА 0 // продано
	|		КОГДА ДанныеШтрихкодыУпаковок.Серия ЕСТЬ NULL
	|			ТОГДА 2 // очистить штрихкод упаковки
	|		ИНАЧЕ 3 // обновить штрихкод упаковки
	|	КОНЕЦ КАК Действие
	|ИЗ СерииИШтрихкоды КАК СерииИШтрихкоды
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДанныеШтрихкодыУпаковок КАК ДанныеШтрихкодыУпаковок
	|	ПО ДанныеШтрихкодыУпаковок.Серия = СерииИШтрихкоды.Серия
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ДанныеШтрихкодовУпаковок = Пакет[4].Выгрузить();
	...
	Пока Выборка.Следующий() Цикл 
		...
		Попытка
			...
			Если ДанныеШтрихкодов.Действие = 1 Тогда
				
				Справочники.ШтрихкодыУпаковокТоваров.СоздатьШтрихкодУпаковки(ДанныеШтрихкодов,,, КэшСтатусовУказанияСерий);
			ИначеЕсли ДанныеШтрихкодов.Действие = 2 Тогда
				НовыеРеквизиты = Новый Структура("Номенклатура,Характеристика,Серия");
				Справочники.ШтрихкодыУпаковокТоваров.ИзменитьШтрихкодУпаковки(ДанныеШтрихкодов.ШтрихкодУпаковки, НовыеРеквизиты);
			ИначеЕсли ДанныеШтрихкодов.Действие = 3 Тогда
				НовыеРеквизиты = Новый Структура("Номенклатура,Характеристика");
				ЗаполнитьЗначенияСвойств(НовыеРеквизиты, ДанныеШтрихкодов);
				Справочники.ШтрихкодыУпаковокТоваров.ИзменитьШтрихкодУпаковки(ДанныеШтрихкодов.ШтрихкодУпаковки, НовыеРеквизиты);
			КонецЕсли;
