...
Процедура ОтразитьДвижения(ТаблицыДляДвижений, Движения, Отказ) Экспорт
	...
	Если ТаблицыДляДвижений.Свойство("ТаблицаОперацииСПрослеживаемымиТоварамиРасширенный") Тогда
		...
		Если ПрослеживаемостьУП.ПроверитьНеобходимостьРасширенныхДвиженийПрослеживаемыхТоваров(ТаблицыДляДвижений.ТаблицаОперацииСПрослеживаемымиТоварамиРасширенный) Тогда
			ПроведениеДокументов.ОтразитьДвижения(ТаблицыДляДвижений, Движения , "ОперацииСПрослеживаемымиТоварамиРасширенный");
			Движения.ТоварыОрганизаций.ДополнительныеСвойства.Вставить("НеОтражатьОперацииСПрослеживаемымиТоварами", Истина);
		ИначеЕсли ТаблицыДляДвижений.Свойство("ТаблицаОперацииСПрослеживаемымиТоварами") И Движения.Найти("ОперацииСПрослеживаемымиТоварами") <> Неопределено
...
Функция ЗапросОтрицательныеОстаткиТоваровОрганизаций()
	...
	ТекстЗапроса =
		...
		|				ТОГДА -1
		|			ИНАЧЕ 1
		|		КОНЕЦ * ТоварыОрганизаций.КОформлениюСписания) КАК КОформлениюСписания
		...
		|	ОстаткиНаКонецПериодаВсе.КОформлениюСписанияОстаток КАК КОформлениюСписанияОстаток
		...
		|	СУММА(Данные.КОформлениюСписания)
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаможенныеДекларацииИмпорт.Организация КАК Организация,
		|		ТаможенныеДекларацииИмпорт.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		...
		|		ТаможенныеДекларацииИмпорт.КОформлениюСписания КАК КОформлениюСписания
		|	ИЗ
		|		ТаможенныеДекларацииИмпорт КАК ТаможенныеДекларацииИмпорт
		|	ГДЕ
		...
		|		-МИНИМУМ(ЕСТЬNULL(ТоварыОрганизаций.КОформлениюСписания, 0))
		|	ИЗ
		|		ДокументыПоступления КАК ДокументыПоступления
		...
		|	СУММА(ОстаткиНаКонецПериода.КОформлениюСписанияОстаток) КАК КОформлениюСписанияОстаток
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОстаткиПредыдущийПериод КАК ОстаткиПредыдущийПериод
		...
		|	СУММА(ОстаткиТекущийПериод.КОформлениюСписанияОстаток - ЕСТЬNULL(ОстаткиПредыдущийПериод.КОформлениюСписанияОстаток, 0))
		|ИЗ
		|	ВТОстаткиТекущийПериод КАК ОстаткиТекущийПериод
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОстаткиПредыдущийПериод КАК ОстаткиПредыдущийПериод
		|		ПО ОстаткиТекущийПериод.Организация = ОстаткиПредыдущийПериод.Организация
...
Функция ВыполнятьСторноТаможенныхДеклараций() Экспорт
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьТоварыВПутиОтПоставщиков")  
		 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНеотфактурованныеПоставки") Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	РегистрНакопления.ТоварыОрганизаций КАК ТоварыОрганизаций
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ТаможеннаяДекларацияИмпорт.Товары КАК ТаможеннаяДекларацияИмпортТовары
		|		ПО (ТаможеннаяДекларацияИмпортТовары.Ссылка = ТоварыОрганизаций.Регистратор)
		|			И (ТаможеннаяДекларацияИмпортТовары.Номенклатура = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура)
		|			И (ТаможеннаяДекларацияИмпортТовары.Характеристика = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Характеристика)
		|			И (ТаможеннаяДекларацияИмпортТовары.Серия = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Серия)
		|			И (ТаможеннаяДекларацияИмпортТовары.Склад = ТоварыОрганизаций.АналитикаУчетаНоменклатуры.МестоХранения)
		|ГДЕ
		|	ТоварыОрганизаций.ХозяйственнаяОперация В(&ОперацииТаможеннойДекларацииИмпорт)
		|	И ТаможеннаяДекларацияИмпортТовары.ДокументПоступления.ХозяйственнаяОперация В(&ОперацииРаздельногоОформленияЗакупок)";
...
Процедура ПроверкаОтрицательныхОстатковТоваровОрганизаций(ПараметрыПроверки) Экспорт
	...
	ДополнительныеПараметры.Вставить("ИспользуетсяРаздельноеОформлениеЗакупок", ЗапасыСервер.ВыполнятьСторноТаможенныхДеклараций());
	...
	ДополнительныеПараметры.Вставить("ОперацииТаможеннойДекларацииИмпорт",      ЗакупкиСервер.ХозяйственныеОперацииТаможеннойДекларацииИмпорт());
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	//++ НЕ УТ
	|		ВидыЗапасов.Договор.ПорядокОформленияСписанияВПроизводствоТоваровПринятыхНаХранение,
	//-- НЕ УТ
