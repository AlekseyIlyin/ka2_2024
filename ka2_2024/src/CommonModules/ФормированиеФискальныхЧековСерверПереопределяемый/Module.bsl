...
Процедура ОбработкаПолученияФормыПредпросмотрЧека(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	Если Не ВзаиморасчетыОбновлены() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для печати чека необходимо дождаться завершения обработчиков отложенного обновления регистров механизма ""Взаиморасчеты"".'");		
		Параметры.Вставить("Заголовок", НСтр("ru = 'Предупреждение'"));
		Параметры.Вставить("Таймаут", 12);
		Параметры.Вставить("ТекстСообщения", Новый ФорматированнаяСтрока(ТекстСообщения));
		Параметры.Вставить("БлокироватьВесьИнтерфейс", Истина);
		Параметры.Вставить("ПоказыватьКартинку", Ложь);
		Параметры.Вставить("Кнопки", "РежимДиалогаВопрос.ОК");
		
	ИначеЕсли Не ВзаиморасчетыРассчитаны(Параметры.ПараметрыОперации) Тогда
