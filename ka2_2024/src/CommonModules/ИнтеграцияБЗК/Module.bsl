...
Функция УдержанныйНДФЛЗаПериод(Организация, Период, ТринадцатыйПериод = Ложь, ПереходныйПериод = Ложь) Экспорт
	...
	КБКсПревышения = Перечисления.ТипыНалогов.КБКПоВидуНалоговогоОбязательства(Перечисления.ТипыНалогов.НДФЛСПревышения);
...
Функция НоваяТаблицаУдержанногоНДФЛ()
	...
	ТаблицаНДФЛ.Колонки.Добавить("Налог",                       Новый ОписаниеТипов("ПеречислениеСсылка.ТипыНалогов"));
...
Процедура РассчитатьИЗарегистрироватьПеречисленныйНДФЛ(Объект, Отказ) Экспорт
	...
	Если СуммаКРаспределению <> 0 Тогда
		...
		Если ЗначениеЗаполнено(Объект.НастройкаСчетовУчета.Субконто2) Тогда
			НоваяСтрока.РегистрацияВНалоговомОргане = Объект.НастройкаСчетовУчета.Субконто2;
