		...
		Для Каждого Стр Из Форма.ДанныеМногостраничныхРазделов[ИмяРаздела] Цикл 
			Если Стр.Значение.УИД = Форма.УИДТекущаяСтраница Тогда 
				Для Каждого КЗ Из Стр.Значение Цикл 
					Если Не ДанныеСправки.Свойство(КЗ.Ключ) Тогда 
						Продолжить;
					КонецЕсли;
					...
					Если ТипЗнч(Область) = Тип("ОбластьЯчеекТабличногоДокумента")
						...
						И ТипЗнч(Область.Значение) = Тип("Дата") Тогда 
						Стр.Значение[КЗ.Ключ] = РегламентированнаяОтчетностьЗагрузка.ДатаИзСтрокиЛюбогоФормата(ДанныеСправки[КЗ.Ключ]);
					Иначе
						Стр.Значение[КЗ.Ключ] = ДанныеСправки[КЗ.Ключ];
					КонецЕсли;
				КонецЦикла;
