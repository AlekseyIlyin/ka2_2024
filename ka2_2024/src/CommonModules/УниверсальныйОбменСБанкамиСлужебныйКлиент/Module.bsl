...
Процедура ПодписатьПослеСозданияМенеджераКриптографии(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ДобавитьМеткуДоверенногоВремени 
		И УниверсальныйОбменСБанкамиВызовСервера.ДоступнаУсовершенствованнаяПодпись() Тогда
...
// Для совместимости с версиями платформы, не поддерживающими усовершенствованную подпись.
Функция ТипПодписиКриптографии() 
	
	Попытка
		ТипыПодписей = Вычислить("ТипПодписиКриптографии");
	Исключение
		ВызватьИсключение НСтр("ru='Усовершенствованная подпись не поддерживается в этой версии платформы.'");
	КонецПопытки;
