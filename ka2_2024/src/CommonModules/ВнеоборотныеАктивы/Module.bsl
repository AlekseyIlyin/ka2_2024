...
//
// Параметры:
//  ПараметрыРасчета - см. НовыеПараметрыРасчетаАмортизации
...
Функция АмортизацияОСМожетНачислятьсяСДатыПринятияКУчету(СписокОрганизаций, Период) Экспорт
	
	ТекстЗапроса = ВнеоборотныеАктивыЛокализация.ТекстЗапросаАмортизацияОСМожетНачислятьсяСДатыПринятияКУчету();
...
// Возвращаемое значение:
//  Булево - Истина, если амортизация может начисляться с даты принятия к учету..
Функция АмортизацияНМАМожетНачислятьсяСДатыПринятияКУчету(СписокОрганизаций, Период) Экспорт
	
	ТекстЗапроса = ВнеоборотныеАктивыЛокализация.ТекстЗапросаАмортизацияНМАМожетНачислятьсяСДатыПринятияКУчету();
...
Функция РассчитатьРасходыПоДокументу(РасходыПоДокументам, ПараметрыРасчета) Экспорт
	...
	Если ПараметрыРасчета.ФактическаяСтоимость Тогда
		...
		РезультатРасчета.Колонки.Добавить("ИдентификаторСтрокиБУ", ОбщегоНазначения.ОписаниеТипаСтрока(36));
		РезультатРасчета.Колонки.Добавить("ИдентификаторСтрокиНУ", ОбщегоНазначения.ОписаниеТипаСтрока(36));
		...
		Для каждого СтрокаДокумента Из ПараметрыРасчета.ТаблицаОбъектовУчета Цикл
			...
			Для Сч = 0 По ДанныеПоАналитке.РасходыПоОбъектуУчета.Количество()-1 Цикл
				...
				НоваяСтрока.ИдентификаторСтрокиБУ = Новый УникальныйИдентификатор();
				НоваяСтрока.ИдентификаторСтрокиНУ = Новый УникальныйИдентификатор();
				...
				Если РаспределениеСуммаРегл <> Неопределено Тогда
					Если РаспределениеСуммаРегл[Сч] <> РасходыПоОбъектуУчета.СуммаРегл Тогда
						...
						НоваяСтрока.ПостояннаяРазница = 
							Окр(РасходыПоОбъектуУчета.ПостояннаяРазница * РаспределениеСуммаРегл[Сч] 
									/ РасходыПоОбъектуУчета.СуммаРегл, 2);
							
						НоваяСтрока.ВременнаяРазница = 
							Окр(РасходыПоОбъектуУчета.ВременнаяРазница  * РаспределениеСуммаРегл[Сч] 
									/ РасходыПоОбъектуУчета.СуммаРегл, 2);
...
Функция ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА(Объект) Экспорт
	...
	Возврат Объект <> Неопределено 
		И Объект.ДополнительныеСвойства.Свойство(ИмяПризнакаВыполненияРасчетаСтоимостиВнеоборотныхАктивов());
...
Процедура УстановитьПризнакВыполненияРасчетаСтоимостиВнеоборотныхАктивов(ДокументОбъект) Экспорт
	
	ДокументОбъект.ДополнительныеСвойства.Вставить(ИмяПризнакаВыполненияРасчетаСтоимостиВнеоборотныхАктивов(), Истина);
...
Функция ИмяПризнакаВыполненияРасчетаСтоимостиВнеоборотныхАктивов() Экспорт
...
Процедура СформироватьЗаданияКРасчетуСтоимостиВНА(Документ, ДанныеТаблиц) Экспорт
	...
	|			И (ОС.Ссылка.ОтражатьВРеглУчете = втОбъектыДокумента.ОтражатьВРеглУчете 
	|					И втОбъектыДокумента.ОтражатьВРеглУчете
	|				ИЛИ ОС.Ссылка.ОтражатьВУпрУчете = втОбъектыДокумента.ОтражатьВУпрУчете 
	|					И втОбъектыДокумента.ОтражатьВУпрУчете)
	...
	|			И (Компоненты.Ссылка.ОтражатьВРеглУчете = втОбъектыДокумента.ОтражатьВРеглУчете 
	|					И втОбъектыДокумента.ОтражатьВРеглУчете
	|				ИЛИ Компоненты.Ссылка.ОтражатьВУпрУчете = втОбъектыДокумента.ОтражатьВУпрУчете 
	|					И втОбъектыДокумента.ОтражатьВУпрУчете)
	...
	Запрос.УстановитьПараметр("ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА", ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА(Документ));
...
Процедура СформироватьЗаданияКФормированиюДвижений(Документ, ДанныеТаблиц) Экспорт
	...
	ВнеоборотныеАктивыЛокализация.ДополнитьСписокТаблицПоКоторымФормируютсяЗаданияКФормированиюДвижений(СписокТаблиц);
	...
	Если Документ <> Неопределено
		И НЕ ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА(Документ) Тогда
		...
		ЗарегистрироватьДокумент = 
			ТипДокумента = Тип("ДокументСсылка.КорректировкаСтоимостиИАмортизацииОС")
	...
	Иначе
		...
		Если ЗарегистрироватьДокумент Тогда
			...
			|					ИЛИ &ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА
			|						И ДокументыПоОС.Дата >= ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(ИзмененныеДанные.Период, МЕСЯЦ), ДЕНЬ, 1))
			|
			...
			|
			|	ВЫБРАТЬ
			|		УзлыКомпонентыАмортизации.Организация,
			|		УзлыКомпонентыАмортизации.Документ,
			|		УзлыКомпонентыАмортизации.ОбъектУчета,
			|		ДокументыПоОС.Дата
			|	ИЗ
			|		УзлыКомпонентыАмортизации КАК УзлыКомпонентыАмортизации
			|
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоОС КАК ДокументыПоОС
			|			ПО (ДокументыПоОС.Организация = УзлыКомпонентыАмортизации.Организация)
			|				И (ДокументыПоОС.ОсновноеСредство = УзлыКомпонентыАмортизации.ОбъектУчета)
			|
			|				И (НЕ &ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА
			|						И ДокументыПоОС.Дата >= УзлыКомпонентыАмортизации.Период
			|					ИЛИ &ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА
			|						И ДокументыПоОС.Дата >= ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(УзлыКомпонентыАмортизации.Период, МЕСЯЦ), ДЕНЬ, 1))
			|
			|				И (ДокументыПоОС.Дата >= &ДатаНачалаУчета)
			|				И (ДокументыПоОС.Проведен)
			|				И (ДокументыПоОС.ТипСсылки В(&ТипыДокументов))
			...
			|					ИЛИ &ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА
			|						И ДокументыПоНМА.Дата >= ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(ИзмененныеДанные.Период, МЕСЯЦ), ДЕНЬ, 1))
			|
...
Процедура ОбработкаПараметровВыбораСтатьиРасходов(Параметры) Экспорт
	...
	ВариантыРаспределенияРасходов = ВнеоборотныеАктивыКлиентСервер.ДоступныеВариантыРаспределенияРасходов(Параметры);
		
	Если ВариантыРаспределенияРасходов.Количество() <> 0 Тогда
		Параметры.Отбор.Вставить("ВариантРаспределенияРасходов", Новый ФиксированныйМассив(ВариантыРаспределенияРасходов));
	КонецЕсли; 
...
Процедура Использование_АктуализацияПараметровУзловКомпонентовАмортизации(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьУзлыОбъектовЭксплуатации") Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
			ПараметрыОбработчика,
			НСтр("ru = 'Учет узлов объектов эксплуатации отключен.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
// Применяется учет узлов компонентов амортизации.
// Влияет на отображение в списке операций закрытия месяца пункта Актуализация параметров узлов компонентов амортизации.
// Пункт появляется начиная с первого месяца, в котором в системе появляются узлы или компоненты.
// 
// Параметры:
//  ПараметрыОбработчика - Структура - Параметры обработчика
// 
// Возвращаемое значение:
//  Булево - Применяется учет узлов компонентов амортизации
Функция ПрименяетсяУчетУзловКомпонентовАмортизации(ПараметрыОбработчика)
	
	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(МИНИМУМ(НАЧАЛОПЕРИОДА(УзлыКомпонентыАмортизации.Период, месяц)), НЕОПРЕДЕЛЕНО) КАК НачалоРаботыСУзлами
	|ИЗ
	...
	|		ПО УзлыКомпонентыАмортизации.ОсновноеСредство = МестонахождениеОС.ОсновноеСредство
	|ГДЕ
	...
	Если Выборка.НачалоРаботыСУзлами = Неопределено 
			ИЛИ ПараметрыОбработчика.ПараметрыРасчета.НачалоПериода < Выборка.НачалоРаботыСУзлами Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
// Опредяет статус операции "ФормированиеДвиженийПоВнеоборотнымАктивам".
// 
// Параметры:
//  ПараметрыОбработчика - Структура - Параметры обработчика
...
// Опредяет статус операции "РасчетСтоимостиВнеоборотныхАктивов".
// 
// Параметры:
//  ПараметрыОбработчика - Структура - Параметры обработчика
...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	#Область МаксимальноеКоличествоЗаданийДляРасчетаАмортизации
...
Функция ИспользуетсяУчетАрендыПоФСБУ25_2018(Организация = Неопределено, Период = '000101010000', УчестьНачалоПерехода = Ложь) Экспорт
	...
	ИспользуетсяУчетАрендыПоФСБУ25_2018 = ВнеоборотныеАктивыЛокализация.ИспользуетсяУчетАрендыПоФСБУ25_2018(Организация, Период, УчестьНачалоПерехода);
...
Функция СозданДляПереходаНаФСБУ25_2018(ДатаДокумента, Организация) Экспорт
	...
	Возврат КонецДня(ДатаДокумента) + 1 = ВнеоборотныеАктивы.НачалоУчетаАрендыПоФСБУ25_2018(Организация);
...
Процедура ТекстЗапросаРасходыПоДокументам(СписокЗапросов) Экспорт
	...
	|			КОНЕЦ, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаБУ,
	|
	|	МАКСИМУМ(ЕСТЬNULL(ВЫБОР
	...
	|					ТОГДА НАЧАЛОПЕРИОДА(СтоимостьОС.Период, ДЕНЬ)
	|				ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|			КОНЕЦ, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаУУ
	|ИЗ
	|	ДанныеКРасчету КАК ДанныеКРасчету
	|
	...
	|
	|	МАКСИМУМ(ЕСТЬNULL(ВЫБОР
	|				КОГДА СтоимостьНМА.ОтражатьВРеглУчете
	|					ТОГДА НАЧАЛОПЕРИОДА(СтоимостьНМА.Период, ДЕНЬ)
	|				ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|			КОНЕЦ, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаБУ,
	|
	|	МАКСИМУМ(ЕСТЬNULL(ВЫБОР
	|				КОГДА СтоимостьНМА.ОтражатьВУпрУчете
	|					ТОГДА НАЧАЛОПЕРИОДА(СтоимостьНМА.Период, ДЕНЬ)
	|				ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|			КОНЕЦ, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаУУ
	|ИЗ
	|	ДанныеКРасчету КАК ДанныеКРасчету
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьНМА КАК СтоимостьНМА
	|		ПО (СтоимостьНМА.НематериальныйАктив = ДанныеКРасчету.АналитикаКапитализацииРасходов)
	|			И (СтоимостьНМА.Период < ДанныеКРасчету.Период)
	|			И (СтоимостьНМА.Регистратор <> ДанныеКРасчету.Регистратор)
	|			И (СтоимостьНМА.ХозяйственнаяОперация В (
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПринятиеКУчетуНМА), 
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.УлучшениеНМА)))
	|
	...
	|		ВЫБОР 
	|			КОГДА ДанныеКРасчету.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.МодернизацияОС)
	|					ИЛИ ДанныеКРасчету.ОтражатьВРеглУчете 
	...
	|				И ДанныеКРасчету.КапитализируетЗатраты
	|
	|	ГДЕ
	...
	|
	|	ГДЕ
	|		ДанныеКРасчету.ОтражатьВРеглУчете 
...
Функция ТипыДокументовПоКоторымРассчитываетсяСтоимость(ПроверитьНаличиеПрав = Ложь) Экспорт
	...
	ВнеоборотныеАктивыСлужебный.ДобавитьТипОбъектаМетаданных("УлучшениеНМА", ПроверитьНаличиеПрав, ТипыОбъектов);
...
Функция СписокДокументовВНА() Экспорт
	...
	СписокДокументовВНА.Добавить("УлучшениеНМА");
...
Процедура УстановитьПараметрыВыбораСтатьиПоПорядкуУчета(ПараметрыВыбора, ЗначенияПараметровВыбора) Экспорт
	...
	Для Каждого Элемент Из ПараметрыВыбора Цикл
		Если Элемент.Имя <> "Отбор.ВариантРаспределенияРасходов" Тогда
			НовыеПараметрыВыбора.Добавить(Элемент);
		КонецЕсли;
	КонецЦикла;
	
	ВариантыРаспределенияРасходов = ВнеоборотныеАктивыКлиентСервер.ДоступныеВариантыРаспределенияРасходов(ЗначенияПараметровВыбора);
	Если ВариантыРаспределенияРасходов.Количество() <> 0 Тогда
		Элемент = Новый ПараметрВыбора("Отбор.ВариантРаспределенияРасходов", ВариантыРаспределенияРасходов);
		НовыеПараметрыВыбора.Добавить(Элемент);
	КонецЕсли;
...
Функция СоздатьОчередьЗаданий(ПараметрыРасчета, ИзмененныеПакеты)
	...
	|	ТекущаяОрганизация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
	|	ТекущаяОрганизация.Представление КАК ОрганизацияПредставление,
	|	ТекущаяОрганизация.ГоловнаяОрганизация.Представление КАК ГоловнаяОрганизацияПредставление
	|ИЗ
	...
	|	МАКСИМУМ(ГоловнаяОрганизацияПредставление)
	|ПО
	|	ГоловнаяОрганизация";
	...
	ВыборкаГоловныеОрганизации = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаГоловныеОрганизации.Следующий() Цикл
		
		НовоеЗадание = ОчередьЗаданийКРасчету.Добавить();
		
		НовоеЗадание.ПараметрыЗадания = НовыеПараметрыРасчетаАмортизации();
		...
		Выборка = ВыборкаГоловныеОрганизации.Выбрать();
		Пока Выборка.Следующий() Цикл
			НовыйПакет = НовоеЗадание.ПараметрыЗадания.ПакетыАмортизации.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйПакет, Выборка);
		КонецЦикла;
		
		Если НовоеЗадание.ПараметрыЗадания.ПакетыАмортизации.Количество() > 1 Тогда
			НовоеЗадание.НаименованиеФоновогоЗадания = 
				СтрШаблон(НСтр("ru = 'Расчет амортизации по организации ""%1"" и ее филиалам.'"), 
						ВыборкаГоловныеОрганизации.ГоловнаяОрганизацияПредставление);
		Иначе
			НовоеЗадание.НаименованиеФоновогоЗадания = 
				СтрШаблон(НСтр("ru = 'Расчет амортизации по организации ""%1"".'"), 
						ВыборкаГоловныеОрганизации.ОрганизацияПредставление);
		КонецЕсли; 
...
Процедура ЗаблокироватьДанныеПередРасчетомСтоимости(Регистратор, ВидОбъектовУчета) Экспорт
	...
	ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.УлучшениеНМА") Тогда
		
		ЭлементБлокировки = Блокировка.Добавить("Документ.УлучшениеНМА");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.УстановитьЗначение("Ссылка", Регистратор);
...
Функция СформироватьТаблицыДвиженийРасчетаСтоимости(ВыборкаДокумент, ПараметрыОбработчика, МенеджерВременныхТаблиц, Отказ)
	...
	Запрос.УстановитьПараметр("НачалоМесяца", НачалоМесяца(ВыборкаДокумент.Период));
	Запрос.УстановитьПараметр("КонецМесяца", КонецМесяца(ВыборкаДокумент.Период));
...
Процедура ЗаписатьДвиженияРасчетаСтоимости(ТаблицыДляДвижений, ВыборкаДокумент, ПараметрыОбработчика)
	...
	Документ = ПроведениеДокументов.ЭмуляцияДокумента(ВыборкаДокумент.Регистратор, ВыборкаДокумент.Период);
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	УстановитьПризнакВыполненияРасчетаСтоимостиВнеоборотныхАктивов(Документ);
	...
	Для Каждого Таблица Из ТаблицыДляДвижений Цикл
		...
		ПроведениеДокументов.УстановитьДопСвойстваРегистра(НаборЗаписей, Документ, МенеджерВТ, Истина);
...
Процедура ЗарегистрироватьОшибкиРасчетаСтоимостиВНА(ВыборкаДокумент, ДанныеДокумента, ПараметрыОбработчика, МенеджерВременныхТаблиц, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ВыборкаДокумент.Регистратор) = Тип("ДокументСсылка.МодернизацияОС2_4")
...
Функция ТекстЗапросаРасчетСтоимости()
	...
	|	ВЫБОР 
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Регистратор) = ТИП(Документ.ПринятиеКУчетуУзловКомпонентовАмортизации)
	|				И ТИПЗНАЧЕНИЯ(ВсеДанныеКРасчету.АналитикаКапитализацииРасходов) = ТИП(Справочник.ОбъектыЭксплуатации)
	|				И ВЫРАЗИТЬ(ВсеДанныеКРасчету.АналитикаКапитализацииРасходов КАК Справочник.ОбъектыЭксплуатации).ТипОС = ЗНАЧЕНИЕ(Перечисление.ТипыОС.ОбъектЭксплуатации)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                                                    КАК КапитализируетЗатраты
	|
	|ПОМЕСТИТЬ ДанныеКРасчету_Предварительный
	|ИЗ
	|	РегистрНакопления.СтоимостьОС КАК ДанныеКРасчету
	|
	...
	|			ТОГДА ПринятиеКУчетуНМА.ОтражатьВБУ
	|		КОГДА НЕ УлучшениеНМА.Ссылка ЕСТЬ NULL
	|			ТОГДА УлучшениеНМА.ОтражатьВРеглУчете
	|		ИНАЧЕ ДанныеКРасчету.ОтражатьВРеглУчете
	|	КОНЕЦ КАК ОтражатьВБУ,
	|
	|	ВЫБОР
	|		КОГДА НЕ ПринятиеКУчетуНМА.Ссылка ЕСТЬ NULL
	|			ТОГДА ПринятиеКУчетуНМА.ОтражатьВНУ
	|		КОГДА НЕ УлучшениеНМА.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ДанныеКРасчету.ОтражатьВРеглУчете
	...
	|		ПО ПринятиеКУчетуНМА.Ссылка = ДанныеКРасчету.Регистратор
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УлучшениеНМА КАК УлучшениеНМА
	|		ПО УлучшениеНМА.Ссылка = ДанныеКРасчету.Регистратор
	|
	|ГДЕ
	|	ДанныеКРасчету.Период МЕЖДУ &НачалоМесяца И &КонецМесяца
	|	И ДанныеКРасчету.Активность
	...
	|ВЫБРАТЬ
	|	ДанныеКРасчету.АналитикаКапитализацииРасходов КАК АналитикаКапитализацииРасходов,
	|	МАКСИМУМ(ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Регистратор) = ТИП(Документ.ПринятиеКУчетуОС2_4)) КАК ЕстьРегистраторДляОСВладельца,
	|	МАКСИМУМ(ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Регистратор) = ТИП(Документ.ПринятиеКУчетуУзловКомпонентовАмортизации)) КАК ЕстьРегистраторДляКомпонента
	|ПОМЕСТИТЬ АналитикаКапитализацииКомпонентовСВладельцами
	|ИЗ
	|	ДанныеКРасчету_Предварительный КАК ДанныеКРасчету
	|СГРУППИРОВАТЬ ПО
	|	ДанныеКРасчету.АналитикаКапитализацииРасходов
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Регистратор) = ТИП(Документ.ПринятиеКУчетуОС2_4))
	|	И МАКСИМУМ(ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Регистратор) = ТИП(Документ.ПринятиеКУчетуУзловКомпонентовАмортизации))
	|
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаКапитализацииРасходов
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Регистратор) = ТИП(Документ.ПринятиеКУчетуУзловКомпонентовАмортизации)
	|				И НЕ АналитикаКапитализацииКомпонентовСВладельцами.АналитикаКапитализацииРасходов ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК КапитализируетЗатраты
	...
	|ИЗ
	|	ДанныеКРасчету_Предварительный КАК ДанныеКРасчету
	|		ЛЕВОЕ СОЕДИНЕНИЕ АналитикаКапитализацииКомпонентовСВладельцами КАК АналитикаКапитализацииКомпонентовСВладельцами
	|		ПО (ДанныеКРасчету.АналитикаКапитализацииРасходов = АналитикаКапитализацииКомпонентовСВладельцами.АналитикаКапитализацииРасходов)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	АналитикаКапитализацииРасходов
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДанныеКРасчету_Предварительный
	|
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ АналитикаКапитализацииКомпонентовСВладельцами";
...
Процедура ТекстЗапросаТаблицаДанныеДокумента(СписокЗапросов)
	...
	|ГДЕ
	|	ТаблицаОС.Ссылка = &Регистратор
	|	И ТаблицаОС.КомпонентАмортизации.ТипОС <> ЗНАЧЕНИЕ(Перечисление.ТипыОС.Узел)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|";
	...
	|			И (Операция.ОтражатьВРеглУчете = втОбъектыДокумента.ОтражатьВРеглУчете 
	|					И втОбъектыДокумента.ОтражатьВРеглУчете
	|				ИЛИ Операция.ОтражатьВУпрУчете = втОбъектыДокумента.ОтражатьВУпрУчете 
	|					И втОбъектыДокумента.ОтражатьВУпрУчете)
	|ГДЕ
	|	Операция.Проведен
	|	И Операция.Дата МЕЖДУ &НачалоМесяца И &КонецМесяца
	...
	|	Операция.Проведен
	|	И Операция.Дата МЕЖДУ &НачалоМесяца И &КонецМесяца
	|	И Компоненты.КомпонентАмортизации.ТипОС <> ЗНАЧЕНИЕ(Перечисление.ТипыОС.Узел)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ТаблицаОС.Ссылка КАК Ссылка,
	|	ТаблицаОС.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР КОГДА ТаблицаОС.КомпонентАмортизации.ТипОС = ЗНАЧЕНИЕ(Перечисление.ТипыОС.Узел)
	|		ИЛИ (ТаблицаОС.КомпонентАмортизации.ТипОС = ЗНАЧЕНИЕ(Перечисление.ТипыОС.КомпонентАмортизации)
	|		И ТаблицаОС.КомпонентАмортизации.УчитыватьСтоимостьЛиквидационногоОбязательства) ТОГДА
	...
	|		ПО ТаблицаНМА.Ссылка = ПервоначальныеСведенияНМА.Регистратор
	|			И ТаблицаНМА.НематериальныйАктив = ПервоначальныеСведенияНМА.НематериальныйАктив
	|ГДЕ
	|	ТаблицаНМА.Ссылка = &Регистратор	
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ТекстыЗапросаОтложенногоФормированияДвижений(ТекстыЗапроса)
	...
	|
	|ГДЕ
	|	УзлыКомпонентыАмортизации.ОсновноеСредство В (
	|		ВЫБРАТЬ
	|			КэшГраниц.ОбъектУчета
	|		ИЗ
	|			ВТ_КэшГраниц КАК КэшГраниц)
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТ_КэшГраниц.ОбъектУчета
	|	ИЗ
	|		ВТ_КэшГраниц КАК ВТ_КэшГраниц
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Добавление узлов в документы в которых есть владелец
	|	ВЫБРАТЬ
	|		ДокументыПоОС.Ссылка КАК Ссылка,
	|		ДокументыПоОС.Дата КАК Дата,
	|		ДокументыПоОС.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	...
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоОС КАК ДокументыПоОС
	|			ПО ДокументыПоОС.ОсновноеСредство = УзлыКомпонентыАмортизации.ОсновноеСредство
	|
	|	ГДЕ
	|		ДокументыПоОС.Дата МЕЖДУ &НачалоМесяца И &КонецМесяца
	|		И ДокументыПоОС.Проведен
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Добавление владельцев в документы в которых есть узел
	|	ВЫБРАТЬ
	|		ДокументыПоОС.Ссылка КАК Ссылка,
	|		ДокументыПоОС.Дата КАК Дата,
	|		ДокументыПоОС.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	...
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоОС КАК ДокументыПоОС
	|			ПО ДокументыПоОС.ОсновноеСредство = УзлыКомпонентыАмортизации.УзелКомпонент
	|
	|	ГДЕ
	|		ДокументыПоОС.Дата МЕЖДУ &НачалоМесяца И &КонецМесяца
	|		И ДокументыПоОС.Проведен
	...
	|	АмортизацияОС.ЭтоДоначисление ИЛИ АмортизацияОС.ЭтоДоначислениеБУ КАК ЭтоДоначисление,
	|
	|	ВЫБОР
	|		КОГДА АмортизацияОС.ПорядокНачисленияАмортизацииУУ = ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(УчетнаяПолитикаУУ.ПорядокНачисленияАмортизацииОС, НЕОПРЕДЕЛЕНО)
	|		ИНАЧЕ АмортизацияОС.ПорядокНачисленияАмортизацииУУ 
	|	КОНЕЦ КАК ПорядокНачисленияАмортизацииУУ,
	|
	|	ВЫБОР
	|		КОГДА АмортизацияОС.ПорядокНачисленияАмортизацииБУ = ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(УчетнаяПолитикаБУ.ПорядокНачисленияАмортизацииОС, НЕОПРЕДЕЛЕНО)
	|		ИНАЧЕ АмортизацияОС.ПорядокНачисленияАмортизацииБУ 
	...
	|		ПО УчетнаяПолитикаБУ.Организация = АмортизацияОС.Организация.ГоловнаяОрганизация
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ УчетнаяПолитикаУУ КАК УчетнаяПолитикаУУ
	|		ПО УчетнаяПолитикаУУ.Организация = АмортизацияОС.Организация.ГоловнаяОрганизация
	|
	|ГДЕ
	...
	|					ИЛИ (ЕСТЬNULL(АмортизацияВНА.ЭтоДоначисление, ЛОЖЬ) И ТИПЗНАЧЕНИЯ(ДанныеКРасчету.Ссылка) В (&ТипыДокументовСписания))
...
Процедура ОбновитьТаблицыОтложенногоФормированияДвижений(Регистратор, МенеджерВременныхТаблиц)
	...
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.МодернизацияОС2_4")
		ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.УлучшениеНМА") Тогда
		Возврат;
	КонецЕсли; 
	...
	|	ВТ_СтоимостьВНА.Дата КАК Дата,
	|	ВТ_СтоимостьВНА.Организация КАК Организация,
	|	ВТ_СтоимостьВНА.ОбъектУчета КАК ОбъектУчета,
	|	ВТ_СтоимостьВНА.ЗабалансовыйУчет КАК ЗабалансовыйУчет,
	|	ВТ_СтоимостьВНА.Стоимость КАК Стоимость,
	|	ВТ_СтоимостьВНА.СтоимостьРегл КАК СтоимостьРегл,
	|	ВТ_СтоимостьВНА.СтоимостьНУ КАК СтоимостьНУ,
	|	ВТ_СтоимостьВНА.СтоимостьПР КАК СтоимостьПР,
	|	ВТ_СтоимостьВНА.СтоимостьВР КАК СтоимостьВР,
	|	ВТ_СтоимостьВНА.СтоимостьЦФ КАК СтоимостьЦФ,
	|	ВТ_СтоимостьВНА.СтоимостьНУЦФ КАК СтоимостьНУЦФ,
	|	ВТ_СтоимостьВНА.СтоимостьПРЦФ КАК СтоимостьПРЦФ,
	|	ВТ_СтоимостьВНА.СтоимостьВРЦФ КАК СтоимостьВРЦФ,
	|	ВТ_СтоимостьВНА.НеУчитываемаяСтоимостьНУ КАК НеУчитываемаяСтоимостьНУ,
	|	ВТ_СтоимостьВНА.АмортизационнаяПремия КАК АмортизационнаяПремия,
	|	ВТ_СтоимостьВНА.ЗалоговаяСтоимость КАК ЗалоговаяСтоимость,
	|	ВТ_СтоимостьВНА.ПредварительнаяСтоимость КАК ПредварительнаяСтоимость,
	|	ВТ_СтоимостьВНА.ПредварительнаяСтоимостьРегл КАК ПредварительнаяСтоимостьРегл,
	|	ВТ_СтоимостьВНА.РезервПереоценкиСтоимости КАК РезервПереоценкиСтоимости,
	|	ВТ_СтоимостьВНА.РезервПереоценкиСтоимостиРегл КАК РезервПереоценкиСтоимостиРегл,
	|	ВТ_СтоимостьВНА.ПриростСтоимости КАК ПриростСтоимости,
	|	ВТ_СтоимостьВНА.ПриростСтоимостиРегл КАК ПриростСтоимостиРегл
	|
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = ВТ_СтоимостьВНА.Ссылка
	|			И ДанныеКРасчету.ОбъектУчета = ВТ_СтоимостьВНА.ОбъектУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ_АмортизацияВНА.Дата КАК Дата,
	|	ВТ_АмортизацияВНА.Организация КАК Организация,
	|	ВТ_АмортизацияВНА.ОбъектУчета КАК ОбъектУчета,
	|	ВТ_АмортизацияВНА.НачислятьИзнос КАК НачислятьИзнос,
	|	ВТ_АмортизацияВНА.Амортизация КАК Амортизация,
	|	ВТ_АмортизацияВНА.АмортизацияРегл КАК АмортизацияРегл,
	|	ВТ_АмортизацияВНА.АмортизацияНУ КАК АмортизацияНУ,
	|	ВТ_АмортизацияВНА.АмортизацияПР КАК АмортизацияПР,
	|	ВТ_АмортизацияВНА.АмортизацияВР КАК АмортизацияВР,
	|	ВТ_АмортизацияВНА.АмортизацияЦФ КАК АмортизацияЦФ,
	|	ВТ_АмортизацияВНА.АмортизацияНУЦФ КАК АмортизацияНУЦФ,
	|	ВТ_АмортизацияВНА.АмортизацияПРЦФ КАК АмортизацияПРЦФ,
	|	ВТ_АмортизацияВНА.АмортизацияВРЦФ КАК АмортизацияВРЦФ,
	|	ВТ_АмортизацияВНА.РезервПереоценкиАмортизации КАК РезервПереоценкиАмортизации,
	|	ВТ_АмортизацияВНА.РезервПереоценкиАмортизацииРегл КАК РезервПереоценкиАмортизацииРегл
	|
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = ВТ_АмортизацияВНА.Ссылка
	|			И ДанныеКРасчету.ОбъектУчета = ВТ_АмортизацияВНА.ОбъектУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДанныеКРасчету.Приоритет КАК Приоритет,
	|
	|	ВТ_ОбесценениеВНА.Ссылка КАК Ссылка,
	|	ВТ_ОбесценениеВНА.Дата КАК Дата,
	|	ВТ_ОбесценениеВНА.Организация КАК Организация,
	|	ВТ_ОбесценениеВНА.ОбъектУчета КАК ОбъектУчета,
	|	ВТ_ОбесценениеВНА.ЕГДС КАК ЕГДС,
	|	ВТ_ОбесценениеВНА.ОбесценениеУпр КАК ОбесценениеУпр,
	|	ВТ_ОбесценениеВНА.ОбесценениеРегл КАК ОбесценениеРегл
	|
	|ПОМЕСТИТЬ ВТ_ОбесценениеВНА_Исходная
	|
	|ИЗ
	|	ВТ_ОбесценениеВНА КАК ВТ_ОбесценениеВНА
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = ВТ_ОбесценениеВНА.Ссылка
	|			И ДанныеКРасчету.ОбъектУчета = ВТ_ОбесценениеВНА.ОбъектУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Расчеты.Ссылка КАК Ссылка,
	|	ВТ_Расчеты.Дата КАК Дата,
	|	ВТ_Расчеты.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	ВТ_Расчеты.Договор КАК Договор,
	|	ВТ_Расчеты.ТипСуммы КАК ТипСуммы,
	|	ВТ_Расчеты.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ВТ_Расчеты.Валюта КАК Валюта,
	|	ВТ_Расчеты.Сумма КАК Сумма,
	|	ВТ_Расчеты.СуммаУпр КАК СуммаУпр,
	|	ВТ_Расчеты.СуммаРегл КАК СуммаРегл
	|
	|ПОМЕСТИТЬ ВТ_РасчетыПоФинансовымИнструментам_Исходная
	|
	|ИЗ
	|	ВТ_РасчетыПоФинансовымИнструментам КАК ВТ_Расчеты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = СтоимостьОС.Регистратор
	|			И ДанныеКРасчету.ОбъектУчета = СтоимостьОС.ОсновноеСредство
	|
	|ГДЕ
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = СтоимостьНМА.Регистратор
	|			И ДанныеКРасчету.ОбъектУчета = СтоимостьНМА.НематериальныйАктив
	|
	|ГДЕ
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = АмортизацияОС.Регистратор
	|			И ДанныеКРасчету.ОбъектУчета = АмортизацияОС.ОсновноеСредство
	|
	|ГДЕ
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = АмортизацияНМА.Регистратор
	|			И ДанныеКРасчету.ОбъектУчета = АмортизацияНМА.НематериальныйАктив
	|
	|ГДЕ
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ОбесценениеВНА.Период КАК Дата,
	|	ОбесценениеВНА.Организация КАК Организация,
	|	ОбесценениеВНА.ВнеоборотныйАктив КАК ОбъектУчета,
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеКРасчету КАК ДанныеКРасчету
	|		ПО ДанныеКРасчету.Ссылка = ОбесценениеВНА.Регистратор
	|			И ДанныеКРасчету.ОбъектУчета = ОбесценениеВНА.ВнеоборотныйАктив
	|
	|ГДЕ
	...
	|				ИЛИ СтоимостьВНА.Дата = СтоимостьВНА_Движения.Дата
	|					И СтоимостьВНА.Приоритет >= СтоимостьВНА_Движения.Приоритет)
	...
	|					ИЛИ АмортизацияВНА_Движения.Дата = ДанныеКРасчету.Дата
	|						И АмортизацияВНА_Движения.Приоритет <= ДанныеКРасчету.Приоритет)
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ НачислениеИзноса КАК НачислениеИзноса
	|			ПО НачислениеИзноса.ОбъектУчета = ДанныеКРасчету.ОбъектУчета
	|
...
Процедура СформироватьДвиженияПоКомпонентамАмортизации(ДанныеКРасчетуЗаМесяц)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстЗапросаПорядокУчетаОСБУ(Запрос, ТекстыЗапроса, Выборка);
...
Процедура ТекстЗапросаМестонахождениеОС(Запрос, ТекстыЗапроса, Выборка)
	...
	|		И ОбъектыВНА.Организация = МестонахождениеОС.Организация
...
Процедура ТекстЗапросаПорядокУчетаОС(Запрос, ТекстыЗапроса, Выборка)
	...
	|		И ОбъектыВНА.Организация = ПорядокУчетаОС.Организация
...
Процедура ТекстЗапросаПорядокУчетаОСУУ(Запрос, ТекстыЗапроса, Выборка)
	...
	|			И ОбъектыВНА.Организация = ПорядокУчетаОСУУ.Организация
...
Процедура ТекстЗапросаПорядокУчетаОСБУ(Запрос, ТекстыЗапроса, Выборка)
...
Процедура ТекстЗапросаСоставЕГДС(Запрос, ТекстыЗапроса, Выборка)
	...
	|		И ОбъектыВНА.Организация = СоставЕГДС.Организация
	...
	|	СоставЕГДС.ЕГДС
	|ИЗ
	|	РегистрСведений.СоставЕГДС.СрезПоследних(&ГраницаДокумента, (ВнеоборотныйАктив, Организация) В
	|			(ВЫБРАТЬ
	|				Т.ОсновноеСредство,
	|				Т.Организация
	|			ИЗ
...
Процедура СформироватьЗаданияКРасчетуАмортизацииОС(Документ, ДанныеТаблиц)
	...
	Если ТекстЗапроса = Неопределено Тогда
		...
		|		МИНИМУМ(ВЫБОР 
		|					КОГДА ПорядокУчетаОСБУ.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов2_4
		|						ТОГДА ДОБАВИТЬКДАТЕ(ПорядокУчетаОСБУ.Период, МЕСЯЦ, 1)
		|					ИНАЧЕ ПорядокУчетаОСБУ.Период
		|				КОНЕЦ)
		|	ИЗ
		|		ИзмененныеДанные КАК ИзмененныеДанные
		|
	...
	Запрос.УстановитьПараметр("Ссылка", Документ.Ссылка);
	Запрос.УстановитьПараметр("ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА", ДвиженияЗаписываютсяПриРасчетеСтоимостиВНА(Документ));
	Запрос.УстановитьПараметр("ДвиженияЗаписываютсяПриОтложенномФормированииДвижений", ДвиженияЗаписываютсяПриОтложенномФормированииДвижений(Документ));
...
Процедура СформироватьЗаданияКРасчетуАмортизацииНМА(Документ, ДанныеТаблиц)
	...
	Если ТекстЗапроса = Неопределено Тогда
		...
		|
		|		МИНИМУМ(ВЫБОР 
		|					КОГДА ИзмененныеДанные.ПорядокНачисленияАмортизацииБУ = ЗНАЧЕНИЕ(Перечисление.ПорядокНачисленияАмортизации.СоСледующегоМесяца)
		|							ИЛИ ПорядокУчетаНМАБУ.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов2_4
		|						ТОГДА ДОБАВИТЬКДАТЕ(ПорядокУчетаНМАБУ.Период, МЕСЯЦ, 1)
		|					ИНАЧЕ ПорядокУчетаНМАБУ.Период
		|				КОНЕЦ)
		|	ИЗ
		|		ИзмененныеДанные КАК ИзмененныеДанные
		|
...
Процедура РассчитатьСтавкуСтоимостьПроценты(ПараметрыРасчета) Экспорт
	...
	Иначе
		...
		Если ПараметрыРасчета.ТаблицаОС <> Неопределено Тогда
			...
			РезультатРаспределения = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(
				ПараметрыРасчета.ПриведеннаяСтоимость + АвансыДоНачалаАрендыБезНДС, Коэффициенты);
	...
	Если НЕ ПараметрыРасчета.ГрафикНачисленияПроцентовВведенВручную Тогда
		...
		Если ПараметрыРасчета.Свойство("ГрафикСтарыхПроцентов") Тогда
			ГрафикСтарыхПроцентов = ПараметрыРасчета.ГрафикСтарыхПроцентов;
			
			// При изменении условий аренды в произвольную дату, добавляется дополнительная строка
			// в графике процентов на последнюю дату действия старых условий.
			// Добавим в график платежей соответствующую запись с нулевым платежом.
			
			Для каждого СтрокаСтарыхПроцентов Из ГрафикСтарыхПроцентов Цикл 
				Если ГрафикПлатежей.Найти(СтрокаСтарыхПроцентов.Дата, "Дата") = Неопределено Тогда
					ГрафикПлатежей.Добавить().Дата = СтрокаСтарыхПроцентов.Дата;
				КонецЕсли;
			КонецЦикла;
		...
		Иначе
			ГрафикСтарыхПроцентов = Новый ТаблицаЗначений();
			ГрафикСтарыхПроцентов.Колонки.Добавить("Дата");
			ГрафикСтарыхПроцентов.Колонки.Добавить("Проценты");
		КонецЕсли;
		
		ФильтрГрафикаСтарыхПроцентов = Новый Структура("Дата");
		СтрокиГрафикаСтарыхПроцентов = Новый Массив;
		...
		Для Каждого СтрокаГрафика Из ГрафикПлатежей Цикл
			...
			Если ПараметрыРасчета.Свойство("СозданДляПерехода") Тогда 
				Если НЕ ПараметрыРасчета.СозданДляПерехода Тогда
					ФильтрГрафикаСтарыхПроцентов.Дата = СтрокаГрафика.Дата;
					СтрокиГрафикаСтарыхПроцентов = ГрафикСтарыхПроцентов.НайтиСтроки(ФильтрГрафикаСтарыхПроцентов);
				КонецЕсли;
			КонецЕсли;
			
			Если СтрокиГрафикаСтарыхПроцентов.Количество() = 0 Тогда
			...
			Иначе
				// Берем сумму процентов из старого графика до изменения условий.
				СуммаПроцентов = СтрокиГрафикаСтарыхПроцентов.Получить(0).Проценты;
			КонецЕсли;
...
Процедура ЗаполнитьГрафикОплатИНачисленийПоРасчетамИзменениеУсловий(Объект, РеквизитыДоговора) Экспорт
	...
	// Если изменение условий аренды не 1-го числа месяца, считаем, что до даты изменения действовали старые условия.
	// В таком случае, часть месяца до даты изменения считаем по цене дня по старым условиям.
	
	// Считаем, что в дне, предшествующем изменению условий расчеты по старым условиям должны быть завершены.
	// Добавим в графики запись накануне изменения условий.
	
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 1
	|	ГрафикОплатУслугПоАренде.АктуальныеУсловияДоговора КАК ПредыдущиеУсловия,
	|	ГрафикОплатУслугПоАренде.ДатаИзменения             КАК ДатаИзменения
	|
	|ПОМЕСТИТЬ ФильтрУсловийАренды
	|
	|ИЗ
	|	РегистрСведений.ГрафикОплатУслугПоАренде КАК ГрафикОплатУслугПоАренде
	|ГДЕ
	|	ГрафикОплатУслугПоАренде.АктуальныеУсловияДоговора <> &Ссылка
	|	И ГрафикОплатУслугПоАренде.Договор = &Договор
	|	И ГрафикОплатУслугПоАренде.ДатаИзменения < &ДатаИзмененияУсловий
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаИзменения УБЫВ
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""Старые условия, оплата""                          КАК Заголовок,
	...
	|	ГрафикОплатУслугПоАренде.УслугаПоАрендеИтог         КАК УслугаПоАрендеИтог,
	|	ГрафикОплатУслугПоАренде.ОбеспечительныйПлатежИтог  КАК ОбеспечительныйПлатежИтог,
	|	ГрафикОплатУслугПоАренде.ВыкупнаяСтоимостьИтог      КАК ВыкупнаяСтоимостьИтог,
	|	ГрафикОплатУслугПоАренде.ВыкупнаяСтоимостьАвансИтог КАК ВыкупнаяСтоимостьАвансИтог,
	|	0                                                   КАК Длительность,
	|	ДАТАВРЕМЯ(1,1,1)                                    КАК НачалоПериода
	|
	|ИЗ
	|	РегистрСведений.ГрафикОплатУслугПоАренде КАК ГрафикОплатУслугПоАренде
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ФильтрУсловийАренды КАК ФильтрУсловийАренды
	|		ПО ГрафикОплатУслугПоАренде.АктуальныеУсловияДоговора = ФильтрУсловийАренды.ПредыдущиеУсловия
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата
	|;
	...
	|ВЫБРАТЬ
	|
	|	""Старые условия, услуги""                             КАК Заголовок,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ФильтрУсловийАренды КАК ФильтрУсловийАренды
	|		ПО ГрафикНачисленияУслугПоАренде.АктуальныеУсловияДоговора = ФильтрУсловийАренды.ПредыдущиеУсловия
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""Старые условия, проценты""                 КАК Заголовок,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ФильтрУсловийАренды КАК ФильтрУсловийАренды
	|		ПО ГрафикНачисленияПроцентовПоАренде.АктуальныеУсловияДоговора = ФильтрУсловийАренды.ПредыдущиеУсловия
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата";
	...
	СтарыеОплаты = СтарыеУсловия.Получить(1).Выгрузить();
	СтарыеДатыГрафика = СтарыеОплаты.ВыгрузитьКолонку("Дата");
	
	СтарыеУслуги = СтарыеУсловия.Получить(2).Выгрузить();
	
	// Заполним начало периодов в старых графиках.
	
	ТекНачалоПериода = Объект.ДатаНачалаАренды;
	Для Каждого СтрСтарыхОплат Из СтарыеОплаты Цикл
		СтрСтарыхОплат.НачалоПериода = ТекНачалоПериода;
		ТекНачалоПериода = КонецДня(СтрСтарыхОплат.Дата) + 1;
	КонецЦикла;
	
	ТекНачалоПериода = Объект.ДатаНачалаАренды;
	Для Каждого СтрСтарыхУслуг Из СтарыеУслуги Цикл
		СтрСтарыхУслуг.НачалоПериода = ТекНачалоПериода;
		ТекНачалоПериода = КонецДня(СтрСтарыхУслуг.Дата) + 1;
	КонецЦикла;
	
	// Первым оплачивается обеспечительный платеж.
	// Однако, он наверняка уже был в предыдущих условиях. 
	// Добавим оплату превышения нового обеспечительного платежа над старым.
	// Обеспечение по старым условиям уже было оплачено ранее.
	// Аналогично сумма выкупа.
	// Получим ранее оплаченные суммы.

	РанееОплаченноеОбеспечение = 0;
	РанееОплаченнаяСуммаВыкупа = 0;
	РанееНачисленноеОбеспечение = 0;

	Для Каждого СтрокаСтарыхОплат Из СтарыеОплаты Цикл

		Если СтрокаСтарыхОплат.Дата < ДатаИзмененияУсловий Тогда
			РанееОплаченноеОбеспечение = РанееОплаченноеОбеспечение + СтрокаСтарыхОплат.ОбеспечительныйПлатеж;

			РанееОплаченнаяСуммаВыкупа = РанееОплаченнаяСуммаВыкупа 
											+ СтрокаСтарыхОплат.ВыкупнаяСтоимость
											+ СтрокаСтарыхОплат.ВыкупнаяСтоимостьАванс;
			
		КонецЕсли;

	КонецЦикла;
	
	Для Каждого СтрокаСтарыхУслуг Из  СтарыеУслуги Цикл

		Если СтрокаСтарыхУслуг.Дата < ДатаИзмененияУсловий Тогда
			РанееНачисленноеОбеспечение = РанееНачисленноеОбеспечение + СтрокаСтарыхУслуг.ОбеспечительныйПлатеж;			
		КонецЕсли;
	...
	Если Объект.ПериодичностьОплат <> Перечисления.ПериодичностьГрафика.Произвольная
		...
		Объект.ГрафикОплатУслуг.Очистить();
		ТаблицаГрафикОплатУслуг = Объект.ГрафикОплатУслуг.Выгрузить();
		
		ОстатокОбеспечительногоПлатежа = Объект.СуммаОбеспечительногоПлатежа - РанееОплаченноеОбеспечение;
		
		
		СтрокаГрафикаОП = Неопределено;
		Если ОстатокОбеспечительногоПлатежа > 0
			И ЗначениеЗаполнено(Объект.ДатаПервогоОбеспечительногоПлатежа) Тогда
			
			// В документе должна быть указана дата первого обеспечительного платежа равная или позже даты документа.
			// В противном случае, считаем дату первого обеспечительного платежа равной дате изменения.
			
			Если Объект.ДатаПервогоОбеспечительногоПлатежа < ДатаИзмененияУсловий Тогда
				ДатаПервогоОбеспечительногоПлатежа = ДатаИзмененияУсловий;
			Иначе
				ДатаПервогоОбеспечительногоПлатежа = Объект.ДатаПервогоОбеспечительногоПлатежа;
			КонецЕсли;
				
			// Добавим в график дельту, т.к. ранее уже была оплачена часть.
			СтрокаГрафикаОП = ТаблицаГрафикОплатУслуг.Добавить();
			СтрокаГрафикаОП.Дата = ДатаПервогоОбеспечительногоПлатежа;
			СтрокаГрафикаОП.ОбеспечительныйПлатеж = ОстатокОбеспечительногоПлатежа;
		...
		ДатыГрафика = Новый Массив;
		ТекущаяДата = Объект.ДатаПервогоПлатежа;
		ПоследняяДатаСтарыхУсловий = Неопределено;
		
		Пока ТекущаяДата <= ДатаОкончания Цикл
			
			// Старый период возьмем из предыдущих условий.
			Если ТекущаяДата < ДатаИзмененияУсловий Тогда
				ТекущаяДата = ВнеоборотныеАктивыКлиентСервер.СледующаяДатаГрафика(ТекущаяДата, Объект.ПериодичностьОплат);
				ПоследняяДатаСтарыхУсловий = ТекущаяДата;
				Продолжить;
			КонецЕсли;
			
			// Проверим изменение графика сразу после очередной оплаты.
			// Изменение условий не 1-го числа месяца.
			Если ПоследняяДатаСтарыхУсловий < ДатаИзмененияУсловий
				И ПоследняяДатаСтарыхУсловий = НачалоДня(ДатаИзмененияУсловий - 1) Тогда
					ПоследняяДатаСтарыхУсловий = Неопределено;
			КонецЕсли;
			
			// Новые условия после даты изменения.
			ДатыГрафика.Добавить(ТекущаяДата);
			ТекущаяДата = ВнеоборотныеАктивыКлиентСервер.СледующаяДатаГрафика(ТекущаяДата, Объект.ПериодичностьОплат);
		КонецЦикла;
		
		// Пересчет последнего отрезка графика по старым условиям.
		
		Если ЗначениеЗаполнено(ПоследняяДатаСтарыхУсловий) Тогда
			
			// Получим строку старого графика, в который попадает дата изменения.
			СтрокаСтарогоГрафика = Неопределено;
			
			Для Каждого СтрСтарыхОплат Из СтарыеОплаты Цикл
				Если СтрСтарыхОплат.НачалоПериода <  ДатаИзмененияУсловий 
						   И ДатаИзмененияУсловий <= СтрСтарыхОплат.Дата Тогда

					НачалоПоследнегоСтарогоПериода = СтрСтарыхОплат.НачалоПериода;
					СтрокаСтарогоГрафика = СтрСтарыхОплат;
					Прервать;

				КонецЕсли;
			КонецЦикла;
			
			// Добавим дополнительную строку для последнего дня по старым условиям.
			КоэффициентПервогоНовогоПериода = 1;
			
			Если СтрокаСтарогоГрафика <> Неопределено Тогда

				КоличествоДнейПоследнегоСтарогоПериода = (ПоследняяДатаСтарыхУсловий - НачалоПоследнегоСтарогоПериода)
														/ (24 * 60 * 60) 
														+ 1;

				ПоследняяДатаСтарыхУсловий = НачалоДня(ДатаИзмененияУсловий - 1);
				КоличествоДнейДляГрафика = (ПоследняяДатаСтарыхУсловий - НачалоПоследнегоСтарогоПериода) 
														/ (24 * 60 * 60)
														+ 1;
				Коэффициент = КоличествоДнейДляГрафика / КоличествоДнейПоследнегоСтарогоПериода;
				КоэффициентПервогоНовогоПериода = КоэффициентПервогоНовогоПериода - Коэффициент;

				ДополнительнаяСтрока = ТаблицаГрафикОплатУслуг.Добавить();
				ДополнительнаяСтрока.Дата = ПоследняяДатаСтарыхУсловий;
				ДополнительнаяСтрока.УслугаПоАренде = Окр(СтрокаСтарогоГрафика.УслугаПоАренде * Коэффициент, 2);
				ДополнительнаяСтрока.ОбеспечительныйПлатеж = Окр(СтрокаСтарогоГрафика.ОбеспечительныйПлатеж
																	* Коэффициент, 2);
					
				ДополнительнаяСтрока.ВыкупнаяСтоимость = Окр((СтрокаСтарогоГрафика.ВыкупнаяСтоимость
																+ СтрокаСтарогоГрафика.ВыкупнаяСтоимостьАванс) 
															  * Коэффициент, 2);
		...
		// Изменение условий имеет смысл только до окончания действия старых условий.
		Если КоличествоДат > 0 Тогда
		
		// Сумма выкупа была оплачена ранее, вычислим положительный остаток.
			СуммаВыкупаПредметовАренды = Объект.СуммаВыкупаПредметовАренды - РанееОплаченнаяСуммаВыкупа;
			АвансированиеВыкупнойСтоимости = Макс(0, Окр(СуммаВыкупаПредметовАренды / КоличествоДат, 2));
				
		// Заполнение части графика по новым условиям.
			Для Сч = 0 По КоличествоДат - 1 Цикл

				КоэффициентСтроки = ?( Сч = 0, КоэффициентПервогоНовогоПериода, 1);

				ТекущаяДата = ДатыГрафика[Сч];

				СтрокаГрафика = ТаблицаГрафикОплатУслуг.Добавить();

				СтрокаГрафика.Дата = ТекущаяДата;
				СтрокаГрафика.УслугаПоАренде = Окр(Объект.ОплатаУслугиПоАренде * КоэффициентСтроки, 2);

				Если РеквизитыДоговора.ЕстьАвансированиеВыкупнойСтоимости Тогда

					ТекущееАвансированиеВыкупнойСтоимости = Окр(АвансированиеВыкупнойСтоимости * КоэффициентСтроки, 2);

					СтрокаГрафика.ВыкупнаяСтоимость = ?(Сч < КоличествоДат - 1, Мин(
						ТекущееАвансированиеВыкупнойСтоимости, СуммаВыкупаПредметовАренды), СуммаВыкупаПредметовАренды);
					
					// Учтем часть выкупной стоимости по старым условиям из доп. строки.
					Если Сч = 0 и СтрокаСтарогоГрафика <> Неопределено Тогда
						СтараяЧастьВыкупнойСтоимости = ДополнительнаяСтрока.ВыкупнаяСтоимость;
					Иначе
						СтараяЧастьВыкупнойСтоимости = 0;
					КонецЕсли;

					СуммаВыкупаПредметовАренды = СуммаВыкупаПредметовАренды 
													- СтрокаГрафика.ВыкупнаяСтоимость
													- СтараяЧастьВыкупнойСтоимости;

				КонецЕсли;

			КонецЦикла;
		
		// Если выкуп осуществляется одним платежом, без авансов в течение строка аренды.
		// Включим остаток аванса выкупной стоимости в первую строку по новым условиям.
			Если РеквизитыДоговора.ЕстьВыкупПредметовАренды И Не РеквизитыДоговора.ЕстьАвансированиеВыкупнойСтоимости
				И СуммаВыкупаПредметовАренды <> 0 Тогда

				СтруктураПоиска = Новый Структура("Дата", Объект.ДатаВыкупа);
				СписокСтрок = ТаблицаГрафикОплатУслуг.НайтиСтроки(СтруктураПоиска);
				Если СписокСтрок.Количество() = 0 Тогда
					СтрокаГрафика = ТаблицаГрафикОплатУслуг.Добавить();
					СтрокаГрафика.Дата = Объект.ДатаВыкупа;
				Иначе
				...
				СтрокаГрафика.ВыкупнаяСтоимость = СуммаВыкупаПредметовАренды;

			КонецЕсли;
		
		// Свернем таблицу графика оплат услуг по датам.
			ТаблицаГрафикОплатУслуг.Свернуть("Дата", "УслугаПоАренде,УслугаПоАрендеНДС,
													 |ОбеспечительныйПлатеж,ОбеспечительныйПлатежНДС,
													 |ВыкупнаяСтоимость,ВыкупнаяСтоимостьНДС");

		КонецЕсли;
												 
		// Добавим к новому графику старый.
		Для Каждого СтрокаСтарогоГрафика Из СтарыеОплаты Цикл
			
			Если СтрокаСтарогоГрафика.Дата < ДатаИзмененияУсловий Тогда
				СтрокаНовогоГрафика = ТаблицаГрафикОплатУслуг.Добавить();
				
				ЗаполнитьЗначенияСвойств(СтрокаНовогоГрафика, СтрокаСтарогоГрафика);
				
				СтрокаНовогоГрафика.ВыкупнаяСтоимость = СтрокаСтарогоГрафика.ВыкупнаяСтоимость
																+ СтрокаСтарогоГрафика.ВыкупнаяСтоимостьАванс;
			Иначе
				Прервать;
			КонецЕсли;
		...
		ТаблицаГрафикОплатУслуг.Сортировать("Дата");
		
		Объект.ГрафикОплатУслуг.Загрузить(ТаблицаГрафикОплатУслуг);
	...
	Если Объект.ПериодичностьНачислений <> Перечисления.ПериодичностьГрафика.Произвольная
		...
		Объект.ГрафикНачисленияУслуг.Очистить();
		ТаблицаУслуг = Объект.ГрафикНачисленияУслуг.Выгрузить();
		...
		Пока ТекущаяДата <= ДатаОкончания Цикл
			...
			// Старый период возьмем из предыдущих условий.
			Если ТекущаяДата < ДатаИзмененияУсловий Тогда
				ТекущаяДата = ВнеоборотныеАктивыКлиентСервер.СледующаяДатаГрафика(ТекущаяДата,
			...
			// Проверим изменение графика сразу после очередной оплаты
			Если ПоследняяДатаСтарыхУсловий < ДатаИзмененияУсловий И ПоследняяДатаСтарыхУсловий = НачалоДня(
				ДатаИзмененияУсловий - 1) Тогда
				ПоследняяДатаСтарыхУсловий = Неопределено;
			КонецЕсли;
		...
		// Пересчет последнего отрезка графика по старым условиям.
		// Добавим дополнительную строку в результирующий график.
		...
		Если ЗначениеЗаполнено(ПоследняяДатаСтарыхУсловий) Тогда
			...
			Для Каждого СтрСтарыхУслуг Из СтарыеУслуги Цикл
				Если СтрСтарыхУслуг.НачалоПериода < ДатаИзмененияУсловий 
					 И ДатаИзмененияУсловий <= СтрСтарыхУслуг.Дата Тогда

					НачалоПоследнегоСтарогоПериода = СтрСтарыхУслуг.НачалоПериода;
					СтрокаСтарогоГрафика = СтрСтарыхУслуг;
					Прервать;
			...
			Если СтрокаСтарогоГрафика <> Неопределено Тогда
				...
				ДополнительнаяСтрока = ТаблицаУслуг.Добавить();
				ДополнительнаяСтрока.Дата = ПоследняяДатаСтарыхУсловий;
				ДополнительнаяСтрока.УслугаПоАренде = Окр(СтрокаСтарогоГрафика.УслугаПоАренде * Коэффициент, 2);
				ДополнительнаяСтрока.ОбеспечительныйПлатеж = Окр(СтрокаСтарогоГрафика.ОбеспечительныйПлатеж
																	* Коэффициент, 2);
		...
		Если КоличествоДат > 0 Тогда
		// Обеспечительный платеж начислялся ранее.
			СуммаОбеспечительногоПлатежа = Макс(0, Объект.СуммаОбеспечительногоПлатежа - РанееНачисленноеОбеспечение);
			ЗачетОбеспечительногоПлатежа = Окр(СуммаОбеспечительногоПлатежа / КоличествоДат, 2);
		...
		// Заполнение графика.
			Для Сч = 0 По КоличествоДат - 1 Цикл
				...
				СтрокаГрафика = ТаблицаУслуг.Добавить();
				СтрокаГрафика.Дата = ТекущаяДата;
				СтрокаГрафика.УслугаПоАренде = Окр(Объект.НачислениеУслугиПоАренде * КоэффициентСтроки, 2);
			
			// В итоге, обеспечительный платеж не должен превысить сумму указанную в документе.
				ТекущийЗачетОбеспечительногоПлатежа = Окр(ЗачетОбеспечительногоПлатежа * КоэффициентСтроки, 2);

				СтрокаГрафика.ОбеспечительныйПлатеж = ?(Сч < КоличествоДат - 1, 
														Мин(ТекущийЗачетОбеспечительногоПлатежа, 
															СуммаОбеспечительногоПлатежа), 
														СуммаОбеспечительногоПлатежа);
														
				Если Сч = 0 И СтрокаСтарогоГрафика <> Неопределено Тогда
					СтараяЧастьОбеспечительногоПлатежа = ДополнительнаяСтрока.ОбеспечительныйПлатеж;
				Иначе
					СтараяЧастьОбеспечительногоПлатежа = 0;
				КонецЕсли;

				СуммаОбеспечительногоПлатежа = Макс(0, СуммаОбеспечительногоПлатежа
														- СтрокаГрафика.ОбеспечительныйПлатеж
														- СтараяЧастьОбеспечительногоПлатежа  );

			КонецЦикла;

		КонецЕсли;
		
		// Добавим строки по старым условиям.
		Для Каждого СтрокаСтарыхУслуг Из СтарыеУслуги Цикл

			Если СтрокаСтарыхУслуг.Дата < ДатаИзмененияУсловий Тогда
				СтрокаНовыхУслуг = ТаблицаУслуг.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаНовыхУслуг, СтрокаСтарыхУслуг);
			Иначе
				Прервать;
			КонецЕсли;
		...
		// Свернем таблицу услуг.
		ТаблицаУслуг.Свернуть("Дата","УслугаПоАренде,УслугаПоАрендеНДС,
									 |ОбеспечительныйПлатеж,ОбеспечительныйПлатежНДС");
		
		ТаблицаУслуг.Сортировать("Дата");
		
		Объект.ГрафикНачисленияУслуг.Загрузить(ТаблицаУслуг);
...
Процедура ПроверитьДокументАренды(Объект, ВспомогательныеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ) Экспорт
	...
	Если ВспомогательныеРеквизиты.ИспользуетсяУчетАрендыПоФСБУ25_2018
		...
		АвансыБезНДС        = ВнеоборотныеАктивыКлиентСервер.ВсегоАвансовБезНДС(Объект.ГрафикОплатУслуг, Объект.ДатаНачалаАренды, Объект.СтавкаНДС); 
		ПриведеннаяСтоимость = ПриведеннаяСтоимость + АвансыБезНДС;
		ВсегоПлатежейБезНДС = ВсегоПлатежейБезНДС   + АвансыБезНДС;
		...
		Если ПриведеннаяСтоимость >= ВсегоПлатежейБезНДС И ВсегоПлатежейБезНДС > 0 Тогда
			Если РеквизитыДоговора.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоровАренды.Лизинг") Тогда
				ТекстСообщения = НСтр("ru = 'Сумма вложений лизингодателя должна быть меньше суммы платежей без НДС (%1)'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Стоимость предметов аренды должна быть меньше суммы платежей без НДС (%1)'");
			КонецЕсли;
			
			ТекстСообщения = СтрШаблон(ТекстСообщения, ВсегоПлатежейБезНДС);
...
Функция ИдентификаторыДокументовПоКоторымФормируютсяОтложенныеДвижения() Экспорт
	...
	ВнеоборотныеАктивыЛокализация.ДополнитьИдентификаторыДокументовПоКоторымФормируютсяОтложенныеДвижения(СписокОбъектов);
...
Функция ТипыДокументовКоторыеМогутИзменитьСтоимость() Экспорт
	...
	ВнеоборотныеАктивыСлужебный.ДобавитьТипОбъектаМетаданных("НачислениеСписаниеРезервовПредстоящихРасходов", Ложь, ТипыОбъектов);
