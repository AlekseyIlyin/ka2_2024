...
Функция ОписаниеКолонокМакетаДляЗагрузкиНоменклатуры(ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ИспользуютсяСпецификации Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("Спецификация", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)), НСтр("ru = 'Спецификация'"), 15);
		Колонка.Позиция = 9;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ДополнительныеПараметры.ИспользуютсяДатыВыпуска Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаВыпуска", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата выпуска'"), 15);
		Колонка.Позиция = 10;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	
	Если ДополнительныеПараметры.ИспользуютсяДатыВыпускаПродукцииПолуфабриката Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаВыпускаПродукцииПолуфабриката", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата выпуска продукции полуфабриката'"), 20);
		Колонка.Позиция = 11;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	
	Если ДополнительныеПараметры.ТипПлана = "ПланПроизводства" Тогда
		Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета("ДатаЗапуска", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Дата запуска'"), 15);
		Колонка.Позиция = 12;
		КолонкиМакета.Добавить(Колонка);
	КонецЕсли;
	//-- НЕ УТ
	...
	Если ДополнительныеПараметры.ТипПлана = "ПланВнутреннихПотреблений"
		ИЛИ ДополнительныеПараметры.ТипПлана = "ПланПродаж" Тогда
