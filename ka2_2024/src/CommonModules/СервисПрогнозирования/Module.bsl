...
Функция ВыполнитьHTTPЗапрос(Метод,
	...
	HTTPСоединение = Новый HTTPСоединение(СтруктураURI.Хост, СтруктураURI.Порт, , , Прокси);
...
Процедура ВыгрузитьКоллекции(КоллекцииДляВыгрузки = Неопределено, АдресХранилища = Неопределено) Экспорт
	...
	Для Каждого КлючЗначениеКоллекции Из НастройкиСервиса.Коллекции Цикл
		...
		СводкаПоКоллекции = СервисПрогнозированияПереопределяемый.ПолучитьСводкуПоКоллекции(Коллекция);
...
Функция СоздатьКонфигурациюМодели(ВидПлана) Экспорт
	...
	ТребуемыеРеквизитыДополнительно = "ЗаполнятьПоХарактеристикамНоменклатуры, ЗаполнятьПартнера, ЗаполнятьПартнераВТЧ, ЗаполнятьСклад, ЗаполнятьСкладВТЧ, ТипПлана";
...
Функция ТаблицуЗначенийВМассивСтруктур(ТаблицаЗначений, НастройкиСервиса, ТипДанных = Неопределено, ДоступныеРеквизиты = Неопределено,
...
Функция ШаблонНастроекСервиса()
	...
	ВсеНастройкиСервиса.Вставить("Версия", 1.0);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	ИначеЕсли ТипЗнч(НастройкиПрогнозирования.Коллекции) = Тип("Структура") Тогда
		// "Коллекции" имеет тип "Структура" в старой версии, и "Соответствие" в новой.
		Ключи = КлючиКОбработке();
		
		СтарыеКоллекции = НастройкиПрогнозирования.Коллекции;
		НовыеКоллекции = ШаблонНовыхНастроек.Коллекции;
		
		ЕстьИзменениеКоллекций = Ложь;
		Для Каждого Ключ Из Ключи Цикл
			НовыйКлюч = СоответствиеКоллекцийИсточника(Ключ);
			СуществующаяЗапись = Неопределено;
			СтарыеКоллекции.Свойство(Ключ, СуществующаяЗапись);
			Если СуществующаяЗапись <> Неопределено Тогда
				СуществующаяЗапись.ИмяВИсточнике = НовыйКлюч;
				НовыеКоллекции.Вставить(НовыйКлюч, СуществующаяЗапись);
				ЕстьИзменениеКоллекций = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если ЕстьИзменениеКоллекций Тогда
			НастройкиПрогнозирования.Коллекции = НовыеКоллекции;
		КонецЕсли;
		
		ЕстьИзменениеВерсии = Ложь;
		Если НастройкиПрогнозирования.Версия < 1 Тогда
			НастройкиПрогнозирования.Версия = 1.0;
			ЕстьИзменениеВерсии = Истина;
		КонецЕсли;
		
		Если ЕстьИзменениеКоллекций
			Или ЕстьИзменениеВерсии Тогда
