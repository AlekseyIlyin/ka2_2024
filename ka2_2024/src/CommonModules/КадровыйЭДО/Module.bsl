...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет признак соответсвия электронных документов требованиям к формату.'");
...
Функция БлокировкаПечатнойФормыСУсловием(ИдентификаторПечатнойФормы)
	...
	НастрокйиПечатнойФормы = КадровыйЭДОПовтИсп.НастройкиПечатныхФорм().Получить(ИдентификаторПечатнойФормы);
	Если НастрокйиПечатнойФормы <> Неопределено Тогда
		Возврат НастрокйиПечатнойФормы.БлокировкаСУсловием;
	КонецЕсли;
	Возврат Ложь;
...
Функция РезультатЗапросаТребуетсяСогласиеНаПрисоединениеККЭДО(ТаблицаФизическихЛиц) Экспорт
	...
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВТ, Ложь, "ГоловнаяОрганизация,ДатаНачалаУчета,ДатаЗавершенияРаботы");
...
Функция НовыйСогласиеНаПрисоединениеККЭДО(ФизическоеЛицо, Организация, ДатаНачалаУчета, СведенияОПодписях) 
	...
	СогласиеНаПрисоединениеККЭДО.Дата 			= Макс(ТекущаяДатаСеанса(), ДатаНачалаУчета);
...
Процедура ПриСозданииФормыСпискаДокументовКЭДО(УправляемаяФорма) Экспорт
	...
	КадровыйЭДОВнутренний.ПриСозданииФормыСпискаДокументовКЭДО(УправляемаяФорма);
