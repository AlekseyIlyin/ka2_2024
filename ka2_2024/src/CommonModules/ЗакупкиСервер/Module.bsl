...
//		* ВсегдаИспользоватьСуммаПоРНПТДляСтоимостиКомплекта - Булево - 
//
...
Функция СоздатьСоставнойПрослеживаемыйНомерГТД(КомплектующиеДляСозданияСоставногоНомераГТД,
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		НоваяСтрока.СуммаПоРНПТ = СуммаПоВыбранномуПрослеживаемомуТовару(Строка.Номенклатура, Строка.НомерРНПТ);
		Если НоваяСтрока.СуммаПоРНПТ = 0 Тогда
...
Функция СуммаПоВыбранномуПрослеживаемомуТовару(Номенклатура, НомерРНПТ = Неопределено)
	...
	Если Цена = 0 И НомерРНПТ <> Неопределено Тогда
		Цена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НомерРНПТ, "СуммаПоРНПТ");
	КонецЕсли;
