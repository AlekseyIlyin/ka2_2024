...
Процедура Выполнить_СторноДоходовКУДиР(ПараметрыОбработчика) Экспорт
	...
	Пока НачалоРасчета <= ПараметрыОбработчика.ПараметрыРасчета.КонецПериода Цикл
		...
		Для Каждого ТекущаяОрганизация Из ПараметрыОбработчика.ДанныеЭтапа.МассивОрганизаций Цикл
...
Функция РасходыПоНалогу(Реквизиты, СправкаРасчет)
	...
	|	ЗНАЧЕНИЕ(Справочник.ПрочиеАктивыПассивы.ПустаяСсылка) КАК Аналитика,
	|	НЕОПРЕДЕЛЕНО КАК НастройкаСчетовУчета,
...
Функция СодержаниеДохода(ПараметрыСодержанияДохода)
	...
	КонецЕсли;
	Если ПараметрыСодержанияДохода.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КомиссияПоЭквайрингу Тогда
...
Процедура ЗафиксироватьРасчет(НомерЗадания, Операция, Организация, НачалоПериода, КонецПериода, ДанныеКРасчету, Отказ)
	...
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Попытка
			ЗакрытиеМесяцаСервер.ЗаблокироватьРегистрЗаданий(НомерЗадания, Организация, Операция);
			ЗакрытиеМесяцаСервер.ЗафиксироватьРасчет(КонецПериода + 1, Операция, ДанныеКРасчету);
			ЗафиксироватьТранзакцию();
		Исключение
			ОтменитьТранзакцию();
...
Процедура СкорректироватьНалоговуюСтавку(КонтекстРасчета, ТекущаяНалоговаяБаза, ДанныеПрогрессивнойШкалыУСН, Ошибки)
	...
	ПараметрыУчетнойПолитикиУСН = НастройкиНалоговУчетныхПолитикПовтИсп.ДействующиеПараметрыНалоговУчетныхПолитик(
