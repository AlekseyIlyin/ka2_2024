...
//	Строка - имя файла.
//
Функция ПолучитьИмяФайлаФинОтчетностиВБанки(ПараметрыФайлаОтчета) Экспорт
	
	Если ПараметрыФайлаОтчета.СданВНалоговую = Истина Тогда
		ПараметрыФайлаОтчета.ШаблонИмениФайла = СтрЗаменить(ПараметрыФайлаОтчета.ШаблонИмениФайла, "BANK_", "NO_");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ПараметрыФайлаОтчета.ФорматИменованияФайлов) 
		Или ПараметрыФайлаОтчета.ФорматИменованияФайлов = СтандартФНС() Тогда
	...
	ИначеЕсли ПараметрыФайлаОтчета.ФорматИменованияФайлов = СокращенныйПериодБезУИД() Тогда
...
Функция УнифицироватьИмяФайлаКВыгрузке(Знач ИмяФайла, ФорматИменованияФайлов, Префикс, УникальныеИменаФайловВПакете) Экспорт
	...
	ИмяБезРасширения = ДобавитьУникальныйНомер(
		ИмяБезРасширения, 
		РасширениеБезТочки, 
...
Функция СформироватьОтчетПоФормеБанка(ПараметрыОтчета) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Сформировать отчет по СКД'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(
			ФинОтчетностьВБанкиКлиентСервер.СобытиеЖурналаРегистрации(ИмяСобытия),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			РезультатФормированияОтчета.ПодробноеПредставлениеОшибки);
...
Функция ДобавитьПрефиксФайла(Знач ИмяФайла, Знач ПрефиксФайла) 
...
Функция ПолучитьИмяФайлаСокращенныйПериодБезУИД(ИНН, КПП, Знач ШаблонИмениФайла, Знач УточнениеНаименования, ДатаНачала, ДатаОкончания)
	...
	ПредставлениеПериода = СокращенноеПредставлениеПериодаДляИмениФайла(
		ДатаНачала,
		ДатаОкончания); 
...
#Область Идентификаторы_ФорматИменованияФайлов
