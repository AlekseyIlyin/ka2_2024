...
Процедура РедактироватьТекстЗапросаПоОстаткамЗаказов(ТекстЗапроса) Экспорт
	...
	ТекстЗапроса =
		СтрЗаменить(
			ТекстЗапроса,
			"&ЗаказТоварыНоменклатураНабора",   "ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)");
	ТекстЗапроса =
		СтрЗаменить(
			ТекстЗапроса,
			"&ЗаказТоварыХарактеристикаНабора", "ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)");
	ТекстЗапроса =
		СтрЗаменить(
			ТекстЗапроса,
			"&ЗаказТоварыВидЦены",              "ЗаказТовары.Ссылка.ВидЦены");
