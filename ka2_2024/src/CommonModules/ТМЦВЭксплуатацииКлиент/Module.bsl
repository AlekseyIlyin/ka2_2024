...
Процедура ОбработатьСтрокуПриИзмененииКатегорииЭксплуатации(ДанныеСтроки, ТабличнаяЧасть, СтруктураДействий = Неопределено) Экспорт
		...
		Для Каждого ИДСтроки Из ДанныеСтроки Цикл
			ТекущаяСтрока = ТабличнаяЧасть.НайтиПоИдентификатору(ИДСтроки);
		 	Если ЗначениеЗаполнено(ТекущаяСтрока.КатегорияЭксплуатации) 
				...
				И ТекущаяСтрока.КоличествоУпаковок > 1 Тогда
	...
	Иначе
		...
		ТребуетсяРазбитьСтроки = 
			...
			И ДанныеСтроки.КоличествоУпаковок > 1;
