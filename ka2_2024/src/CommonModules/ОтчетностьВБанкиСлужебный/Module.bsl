...
//
// Параметры:
//   Счет - ПланыСчетов.Хозрасчетный - счет, по которому формируется отчет.
...
Функция ЦифровойСлед() Экспорт
	...
	МожноПолучитьIP = (ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.17.0") > 0);
	МожноПолучитьMAC = (ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.18.0") > 0);
	
	Если МожноПолучитьIP Тогда
		Сеанс = ПолучитьТекущийСеансИнформационнойБазы();
		IPАдресКлиента = "IPАдресКлиента";
		ЦифровойСлед.IPАдрес = Сеанс[IPАдресКлиента];
	КонецЕсли;
	
	Если МожноПолучитьMAC Тогда
		ВсеАдаптеры = Новый Массив;
		
		Выполнить("ВсеАдаптеры = ПолучитьИнформациюОСетевыхАдаптерах();");
		...
		Если ВсеАдаптеры.Количество() > 0 Тогда
			...
			IPАдрес = ТекущийАдаптер.IPAddress;
			MACАдрес = ТекущийАдаптер.MACAddress;
			Если Не ЗначениеЗаполнено(ЦифровойСлед.IPАдрес) Тогда
				ЦифровойСлед.IPАдрес = IPАдрес;
			КонецЕсли;
			ЦифровойСлед.MACАдрес = MACАдрес;
...
Процедура ПодготовитьИОтправитьОтчетыВБанкПоПлану(Организация, ОтправлятьВсе = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ПараметрыОтчетов.ОСВПоСчетам.Отправлять Тогда
			Возврат;
		КонецЕсли;
...
Процедура ВывестиСтраницуВУзел_АнализСчетов_АнализСчета(Компоновщик, ПараметрыЗаполнения)
	...
	// В случае, если у счета отсутствует субконто, то вместо него передается Null.
