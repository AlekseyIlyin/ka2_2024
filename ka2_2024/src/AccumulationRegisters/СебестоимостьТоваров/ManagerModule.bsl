...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//++ НЕ УТ
	СписокДокументов.Добавить("Документ.ОтчетПереработчика2_5");
	//-- НЕ УТ
	...
	|		ПО Изделия.Ссылка = СебестоимостьТоваров.Регистратор
	|		И Изделия.ИдентификаторСтроки = СебестоимостьТоваров.ИдентификаторСтроки
	|	ГДЕ
	|		НЕ СебестоимостьТоваров.СтатьяАктивовПассивов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка)
	|		И СебестоимостьТоваров.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	|		И НЕ СебестоимостьТоваров.ИдентификаторСтроки = """"
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// Настройки счетов учета получаем из таблицы видов запасов, так как к моменту обновления движений регистра данные таблицы гарантированно обновлены.
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.ОтчетПереработчика2_5.ВидыЗапасовИзделия КАК ДанныеДокумента
	|ГДЕ
	...
	|	ВЫБОР
	//++ НЕ УТ
	|		КОГДА ДанныеДокумента.НастройкаСчетовУчета ЕСТЬ НЕ NULL
	...
	|	ПО ДанныеДокумента.ИдентификаторСтроки = СебестоимостьТоваров.ИдентификаторСтроки
	|	И НЕ СебестоимостьТоваров.СтатьяАктивовПассивов = ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ПустаяСсылка)
	|	И СебестоимостьТоваров.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	|	И НЕ СебестоимостьТоваров.ИдентификаторСтроки = """"
	|	И НЕ ДанныеДокумента.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	//-- НЕ УТ
	|ГДЕ
