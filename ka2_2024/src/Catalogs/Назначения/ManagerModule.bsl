...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Контролировать превышение обособленного обеспечения"" справочника ""Направления деятельности""
									|Переформировывает наименования для элементов с заполненным реквизитом ""Тип объекта назначения""'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	Справочник.Назначения КАК Назначения
		|ГДЕ
		|	Назначения.ТипОбъектаНазначения <> ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовНазначений.ПустаяСсылка)
		|	И НЕ Назначения.УдалитьНаименованиеОбработано
		|	И Назначения.Наименование <> """"");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Выборка.Ссылка,
				"НаправлениеДеятельности, ТипОбъектаНазначения, УдалитьНаименованиеОбработано");
		...
		Попытка
			...
			Если ЗначениеЗаполнено(Реквизиты.НаправлениеДеятельности) Тогда
				...
				ЭлементБлокировки.УстановитьЗначение("Ссылка", Реквизиты.НаправлениеДеятельности);
				
			КонецЕсли;
			Блокировка.Заблокировать();
			
			ДанныеОбъекта = Выборка.Ссылка.ПолучитьОбъект(); //СправочникОбъект
			ОбъектОбработан = Ложь;
			
			Если Реквизиты.НаправлениеДеятельности
					= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Ссылка, "НаправлениеДеятельности") Тогда
			
				ДопускаетсяОбособлениеСверхПотребности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.НаправлениеДеятельности,
					"ДопускаетсяОбособлениеСверхПотребности");
				
				Если ДопускаетсяОбособлениеСверхПотребности = Ложь
			...
			Если ЗначениеЗаполнено(Реквизиты.ТипОбъектаНазначения) И Не Реквизиты.УдалитьНаименованиеОбработано Тогда
				ТипОбъектаНазначенияНаОсновномЯзыке = Перечисления.ТипыОбъектовНазначений.ТипОбъектаНазначенияНаОсновномЯзыке(Реквизиты.ТипОбъектаНазначения);
				ДанныеОбъекта.Наименование = ДанныеОбъекта.Наименование + " (" + ТипОбъектаНазначенияНаОсновномЯзыке + ")";
				ДанныеОбъекта.УдалитьНаименованиеОбработано = Истина;
				ОбъектОбработан = Истина;
			КонецЕсли;
			
			Если ДанныеОбъекта = Неопределено Тогда
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(Выборка.Ссылка);
			ИначеЕсли ОбъектОбработан Тогда
