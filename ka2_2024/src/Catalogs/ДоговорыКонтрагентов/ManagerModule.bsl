...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Создает служебный документ ""Регистратор графика движения товаров"", если договоры используются при приемке товаров;
	                                    |Заполняет реквизит ""Курс валюты договора"" значением по умолчанию ""Переменный""'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|ГДЕ
		|	ДоговорыКонтрагентов.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
		|ГДЕ
		|	ДоговорыКонтрагентов.ПорядокОтраженияНеотфактурованныхПоставокНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияНеотфактурованныхПоставокНУ.ПустаяСсылка)
		|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого ЭлементСправочника Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(СправочникОбъект.ВариантКурсаДоговора) Тогда
				СправочникОбъект.ВариантКурсаДоговора = Перечисления.ВариантыКурсаДоговора.Переменный;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СправочникОбъект.ПорядокОтраженияНеотфактурованныхПоставокНУ) Тогда
				СправочникОбъект.ПорядокОтраженияНеотфактурованныхПоставокНУ = Перечисления.ВариантыОтраженияНеотфактурованныхПоставокНУ.ОтражатьПриПоступлении;
			КонецЕсли;
