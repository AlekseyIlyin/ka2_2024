...
Процедура ОформитьПредставлениеУплаты(Состояние)
	...
	Если КоличествоПлатежек > 0 Тогда
		...
		Для Каждого СтрокаМассива Из ВсеДокументы Цикл
			ПредставлениеДокументов = ПредставлениеДокументов + Символы.ПС 
						+ СтрШаблон(ШаблонДокумента, СтрокаМассива.Ссылка, СтрокаМассива.ПредставлениеСсылки);
		КонецЦикла;
...
Функция ПолучитьПредставлениеУплаты(СуммаУплаты, ВыбраннаяСумма = Неопределено)
	...
	Если ЗначениеЗаполнено(СуммаУплаты) Тогда
		Результат =  НСтр("ru = 'Общая сумма:'") + " <b>" + Формат(СуммаУплаты, "ЧЦ=15; ЧН=00") 
	...
	Если МассивСсылок <> Неопределено И МассивСсылок.Количество() > 0 Тогда
		...
		Для Каждого СтрокаМассива Из МассивСсылок Цикл
			...
			Если ЗначениеЗаполнено(НашлиСсылку.Ссылка) Тогда
				НоваяСтрока = Новый Структура;
				НоваяСтрока.Вставить("ПредставлениеСсылки", СокрЛП(НашлиСсылку.Ссылка)
								+ ?(ЕстьСумма, " на сумму " + Формат(НашлиСсылку.СуммаДокумента, "ЧЦ=15; ЧДЦ=2") + НСтр("ru = ' руб.'"), ""));
				НоваяСтрока.Вставить("Ссылка", ПолучитьНавигационнуюСсылку(НашлиСсылку.Ссылка));
