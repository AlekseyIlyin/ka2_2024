...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.ПартииПроизводства.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.9.57";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("e936ee2c-b634-424c-906c-f55936d5bad7");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ПартииПроизводства.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет новый реквизит ""Уникальный идентификатор"".
								|Выполняет заполнение пустого представления группы затрат в элементах справочника ""Партии производства"" с заполненными полями группы затрат.'");
	...
	Изменяемые.Добавить(Метаданные.Справочники.ПартииПроизводства.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Справочники.ПартииПроизводства.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Справочник.ПартииПроизводства";
	...
	Запрос = Новый Запрос(
		...
		|		Справочник.ПартииПроизводства КАК Таблица
		|	ГДЕ
		|		Таблица.УникальныйИдентификатор = &ПустойИдентификатор
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ГДЕ
		|		НЕ Таблица.ПометкаУдаления
		|		И Таблица.ГруппаЗатрат = &ГруппаЗатрат
		|		И НЕ (Таблица.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	...
	Запрос.УстановитьПараметр("ГруппаЗатрат", НСтр("ru = '<группа с пустыми значениями>'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Справочник.ПартииПроизводства";
	...
	Для каждого ЭлементСправочника Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			СправочникОбъект = ЭлементСправочника.Ссылка.ПолучитьОбъект(); // СправочникОбъект.ПартииПроизводства
			...
			Если Не ЗначениеЗаполнено(СправочникОбъект.УникальныйИдентификатор) Тогда
				СправочникОбъект.УникальныйИдентификатор = ЭлементСправочника.Ссылка.УникальныйИдентификатор();
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			Если Не СправочникОбъект.ПометкаУдаления
				И СправочникОбъект.ГруппаЗатрат = НСтр("ru = '<группа с пустыми значениями>'", ОбщегоНазначения.КодОсновногоЯзыка())
				И (ЗначениеЗаполнено(СправочникОбъект.Номенклатура)
					Или ЗначениеЗаполнено(СправочникОбъект.Характеристика)
					Или ЗначениеЗаполнено(СправочникОбъект.Назначение)
					Или ЗначениеЗаполнено(СправочникОбъект.Серия)
					Или ЗначениеЗаполнено(СправочникОбъект.Спецификация)) Тогда
				СправочникОбъект.ГруппаЗатрат = ГруппыЗатратСервер.ПредставлениеГруппыЗатрат(СправочникОбъект);
				СправочникОбъект.Наименование = НаименованиеПартииПроизводства(СправочникОбъект, СправочникОбъект.ГруппаЗатрат);
				ОбъектИзменен = Истина;
			КонецЕсли;
