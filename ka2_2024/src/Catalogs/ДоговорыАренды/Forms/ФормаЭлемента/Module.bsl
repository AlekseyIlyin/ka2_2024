...
Процедура КонтактноеЛицоПриИзменении(Элемент)
	...
	КонтактноеЛицоПриИзмененииСервер(Объект.Организация, Объект.Партнер, Объект.Контрагент, Объект.ОплатаВВалюте, Объект.БанковскийСчетКонтрагента, Объект.КонтактноеЛицо);
...
Процедура НастроитьЗависимыеЭлементыФормыНаКлиентеНаСервере(Знач Форма, Знач ИзмененныеРеквизиты = "")
	...
	ПараметрыРеквизитовОбъекта = ВнеоборотныеАктивыКлиентСервер.ЗначенияСвойствЗависимыхРеквизитов_ДоговорыАренды(
...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(Знач ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ДатаНачалаДействия")
		...
		Иначе
			ДатаПроверки = ?(Объект.ДатаНачалаДействия < ТекущаяДатаСеанса(), Неопределено, Объект.ДатаНачалаДействия);
		КонецЕсли;
			
		ВнеоборотныеАктивыСлужебный.УстановитьСвойствоСтруктуры(
			"ИспользуетсяУчетАрендыПоФСБУ25_2018",
			ВнеоборотныеАктивы.ИспользуетсяУчетАрендыПоФСБУ25_2018(Объект.Организация, ДатаПроверки, Ложь),
			СлужебныеПараметрыФормы);
...
Процедура ПриИзмененииРеквизитовНаКлиентеНаСервере(Форма, СтруктураИзмененныхРеквизитов)
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ТребуетсяОформлениеЗаявленийОВвозе") Тогда
...
Процедура ПартнерПриИзмененииСервер(Организация, Партнер, Контрагент, ОплатаВВалюте, БанковскийСчетКонтрагента, КонтактноеЛицо)
	
	ПартнерыИКонтрагенты.ЗаполнитьКонтрагентаПартнераПоУмолчанию(Партнер, Контрагент);
	ПартнерыИКонтрагенты.ЗаполнитьКонтактноеЛицоПартнераПоУмолчанию(Партнер, КонтактноеЛицо);
	ЗаполнитьБанковскийСчетКонтрагентаПоУмолчанию(Организация, Контрагент, ОплатаВВалюте, БанковскийСчетКонтрагента);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура КонтактноеЛицоПриИзмененииСервер(Организация, Партнер, Контрагент, ОплатаВВалюте, БанковскийСчетКонтрагента, КонтактноеЛицо)
	...
	Если ВладелецКонтактногоЛица <> Партнер Тогда
		...
		ПартнерПриИзмененииСервер(Организация, Партнер, Контрагент, ОплатаВВалюте, БанковскийСчетКонтрагента, КонтактноеЛицо);
	КонецЕсли;
...
Процедура ПрочитатьУсловияДоговора()
	...
	|	УсловияДоговоровАренды.Период КАК УсловияДоговора_ИзменениеУсловийПериод,
	...
	Если ЗначениеЗаполнено(ЗаключениеДоговораАренды) Тогда
		...
		Если ЗначениеЗаполнено(УсловияДоговора_ИзменениеУсловий) Тогда
			ЗаголовокНадписи.Добавить(Символы.ПС);
			ЗаголовокНадписи.Добавить(НСтр("ru = 'Условия договора изменены:'")); 
			ЗаголовокНадписи.Добавить(" "); 
			ЗаголовокНадписи.Добавить(Новый ФорматированнаяСтрока(Формат(Выборка.УсловияДоговора_ИзменениеУсловийПериод, "ДЛФ=D;"),,,, ПолучитьНавигационнуюСсылку(УсловияДоговора_ИзменениеУсловий))); 
		КонецЕсли;
