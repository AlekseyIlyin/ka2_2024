...
Процедура СоздатьПрограммыКриптографииНаСервере(Знач ОписанияПрограмм)
	...
	Для Каждого НастройкиПрограммыКриптографии Из ОписанияПрограмм Цикл
		Если НастройкиПрограммыКриптографии.ИмяПрограммы = "Microsoft Enhanced Cryptographic Provider v1.0"
			ИЛИ Не НастройкиПрограммыКриптографии.Установлена Тогда
			Продолжить;
		КонецЕсли;

		ОписаниеПрограммы = ЭлектроннаяПодпись.НовоеОписаниеПрограммы();
		ЗаполнитьЗначенияСвойств(ОписаниеПрограммы, НастройкиПрограммыКриптографии);
