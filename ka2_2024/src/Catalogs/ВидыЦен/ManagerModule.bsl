...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.ВидыЦен.ОбновитьФормулыДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.12.22";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("f9188072-65e1-4905-b95c-4ed006208789");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ВидыЦен.ЗарегистрироватьДанныеОбновленияФормулыДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Обновление значений формул.'");
...
//
Процедура ЗарегистрироватьДанныеОбновленияФормулыДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК СформироватьФормулу
	|ПОМЕСТИТЬ ВидыЦенКОбработке
	|ИЗ
	|	Справочник.ВидыЦен КАК ВидыЦен
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВидыЦенКОбработке.Ссылка КАК Ссылка,
	|	ВидыЦенКОбработке.СформироватьФормулу КАК СформироватьФормулу,
	|	ВидыЦен.БазовыйВидЦены.Идентификатор КАК БазовыйВидЦеныИдентификатор,
	|	ВидыЦен.Наценка КАК Наценка,
	|	ВЫРАЗИТЬ(ВидыЦен.Формула КАК СТРОКА(100)) КАК Формула
	|ИЗ
	|	ВидыЦенКОбработке КАК ВидыЦенКОбработке
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЦен КАК ВидыЦен
	|		ПО ВидыЦенКОбработке.Ссылка = ВидыЦен.Ссылка
	|		И (ВидыЦенКОбработке.СформироватьФормулу)
	|ГДЕ
	|	ВидыЦенКОбработке.СформироватьФормулу
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	ВидыЦенКОбработке.Ссылка,
	|	ВидыЦенКОбработке.СформироватьФормулу,
	|	ВидыЦен.БазовыйВидЦены.Идентификатор,
	|	ВидыЦен.Наценка,
	|	ВЫРАЗИТЬ(ВидыЦен.Формула КАК СТРОКА(100))
	|ИЗ
	|	ВидыЦенКОбработке КАК ВидыЦенКОбработке
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЦен.ЦеновыеГруппы КАК ВидыЦен
	|		ПО ВидыЦенКОбработке.Ссылка = ВидыЦен.Ссылка
	|		И (ВидыЦенКОбработке.СформироватьФормулу)
	|
	|ОБЪЕДИНИТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЦен КАК ВидыЦен
	|		ПО ВидыЦенКОбработке.Ссылка = ВидыЦен.Ссылка
	|		И (НЕ ВидыЦенКОбработке.СформироватьФормулу)
	|ГДЕ
	|	ВЫРАЗИТЬ(ВидыЦен.Формула КАК СТРОКА(100)) <> &ПустаяФормула
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ПустаяФормула","");
	
	ТаблицаДляОбработки = Запрос.Выполнить().Выгрузить();
	СсылкиДляОбработки = Новый Массив();
	ИдентификаторыБазовогоВидаЦен = Новый Структура();
	ТекущийВидЦены = Неопределено;
	НеобходимаОбработка = Ложь; 
	
	Для Каждого ВидЦены Из ТаблицаДляОбработки Цикл
		
		Если ТекущийВидЦены <> ВидЦены.Ссылка Тогда
			ТекущийВидЦены = ВидЦены.Ссылка;
			НеобходимаОбработка = Ложь; 
		ИначеЕсли НеобходимаОбработка Тогда
			Продолжить;
		КонецЕсли;
		
		Если ВидЦены.СформироватьФормулу Тогда
			Формула = "[" + ВидЦены.БазовыйВидЦеныИдентификатор + "]*" + Формат(((ВидЦены.Наценка/100)+1),"ЧРД=.; ЧГ=0");
			НеобходимаОбработка = Формула <> ВидЦены.Формула;
		Иначе
			НеобходимаОбработка = Истина;
		КонецЕсли;
		
		Если НеобходимаОбработка Тогда
			СсылкиДляОбработки.Добавить(ВидЦены.Ссылка);
		КонецЕсли;
		
	КонецЦикла;
	
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, СсылкиДляОбработки);
	
КонецПроцедуры

Процедура ОбновитьФормулыДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	ПолноеИмяОбъекта = ПустаяСсылка().Метаданные().ПолноеИмя();
	
	ОбновляемыеДанные = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, ПолноеИмяОбъекта);
	...
	Пока ОбновляемыеДанные.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ОбновляемыеДанные.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			
			Блокировка.Заблокировать();
			
			СправочникОбъект = ОбновляемыеДанные.Ссылка.ПолучитьОбъект();

			Если СправочникОбъект = Неопределено Тогда
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ОбновляемыеДанные.Ссылка);
				ЗафиксироватьТранзакцию();
				Продолжить;
			КонецЕсли;
			
			Если СправочникОбъект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаДругойВидЦен Тогда
				ИдентификаторБазовогоВидаЦен = НайтиИдентификаторВидаЦен(ИдентификаторыБазовогоВидаЦен, СправочникОбъект.БазовыйВидЦены);
				СправочникОбъект.Формула = "[" + ИдентификаторБазовогоВидаЦен + "]*" + Формат(((СправочникОбъект.Наценка/100)+1),"ЧРД=.; ЧГ=0");

				Для Каждого СтрокаТЧ Из СправочникОбъект.ЦеновыеГруппы Цикл
					ИдентификаторБазовогоВидаЦен = НайтиИдентификаторВидаЦен(ИдентификаторыБазовогоВидаЦен, СтрокаТЧ.БазовыйВидЦены);
					СтрокаТЧ.Формула = "[" + ИдентификаторБазовогоВидаЦен + "]*" + Формат(((СтрокаТЧ.Наценка/100)+1),"ЧРД=.; ЧГ=0");
				КонецЦикла;
				
			Иначе
				СправочникОбъект.Формула = "";
				Для Каждого СтрокаТЧ Из СправочникОбъект.ЦеновыеГруппы Цикл
					СтрокаТЧ.Формула = "";
				КонецЦикла;
			КонецЕсли;
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ИнформацияОбОшибке(), ОбновляемыеДанные.Ссылка);
