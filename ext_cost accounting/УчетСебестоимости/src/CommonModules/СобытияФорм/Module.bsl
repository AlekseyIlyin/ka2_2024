...
Процедура усПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	Если усЭтоФормаДляОтображенияСтоимости(Форма) Тогда
		...
		ус_ОбновитьСебестоимостьМаржу(Форма);	
	КонецЕсли;
...
Процедура ус_ОбновитьСебестоимостьМаржу(Форма)
	...
	Для Каждого СтрокаТовары Из ОбъектФормы.Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СведенияПоПартиям) Тогда
			Продолжить;
		КонецЕсли;
		ТаблицаПартий = усНоваяТаблицаПартий(СведенияПоПартиям);
		
		усЗаполнитьСтоимостьМаржу(СтрокаТовары, ТаблицаПартий);
...
Функция усНоваяТаблицаПартий(СведенияПоПартиям)
