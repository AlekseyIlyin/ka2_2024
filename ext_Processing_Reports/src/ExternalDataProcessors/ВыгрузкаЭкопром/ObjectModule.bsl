...
Процедура Заполнить_ДетализациюДвиженияТовара(ИмяТЧ, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		КонтрагентПвртнер = Выборка.Партнер;
		Если ЗначениеЗаполнено(КонтрагентПвртнер) Тогда
			КонтрагентПвртнер = Выборка.Контрагент;
		КонецЕсли;
		НоваяСтрока.clientcode = XmlСтрока(КонтрагентПвртнер.УникальныйИдентификатор());
