...
Процедура Заполнить_ДетализациюДвиженияТовара(ИмяТЧ, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипОперации = 0 Или ТипОперации = 1 Или ТипОперации = 7 Тогда
			Сумма = НоваяСтрока.amount_cp;
		Иначе
			Сумма = НоваяСтрока.amount_bp;
		КонецЕсли;
		Если Сумма < 0.20 Тогда
			Сумма = 0.20;
		КонецЕсли;
...
Функция ТекстЗапроса_ДетализациюДвиженияТовара()
	...
	|	ВТ_АналитикиУчетаПоПартнерам.ЮрАдрес КАК address_ur,
	|	ВТ_АналитикиУчетаПоПартнерам.ФактАдрес КАК address,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_АналитикиУчетаПоПартнерам КАК ВТ_АналитикиУчетаПоПартнерам
	|		ПО усОстаткиПоПартиямОбороты.Регистратор.Партнер = ВТ_АналитикиУчетаПоПартнерам.Партнер
	|СГРУППИРОВАТЬ ПО
	...
	|	усОстаткиПоПартиямОбороты.Регистратор.Контрагент,
	...
	|	ВТ_АналитикиУчетаПоПартнерам.ЮрАдрес,
	|	ВТ_АналитикиУчетаПоПартнерам.ФактАдрес";
КонецФункции
