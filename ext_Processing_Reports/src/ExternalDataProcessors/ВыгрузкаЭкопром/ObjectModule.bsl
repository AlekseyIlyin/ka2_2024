...
Процедура Отправить(АдресРезультатаВыгрузки, ВыгрузкаИзФормы = Ложь) Экспорт
	
	Если Не ВыгрузкаИзФормы Тогда
		Если Не ЗначениеЗаполнено(ПутьКлиентаCisLink) Тогда
			//@skip-check undefined-variable
	...
	Попытка
		ЗапуститьПриложение(СтрШаблон("%1 console", ПутьКлиентаCisLink));
		//@skip-check undefined-variable
		рвк_ЖурналРегистрацииВызовСервера.ЗаписатьИнформацию(ИмяСобытияЖР(), "Клиент CISLink успешно запущен", ДополнительнаяОбработкаСсылка);
	Исключение
		//@skip-check undefined-variable
		рвк_ЖурналРегистрацииВызовСервера.ЗаписатьОшибку(ИмяСобытияЖР(), 
			СтрШаблон("При запуске клиента CISLink возникла ошибка: %1", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())), ДополнительнаяОбработкаСсылка);
	КонецПопытки;
...
Функция ТекстЗапроса_ДетализациюДвиженияТовара()
	...
	|	МАКСИМУМ(ЕСТЬNULL(КИ_ФактАдрес.Представление, """")) КАК ФактАдрес,
	...
	|	усОстаткиПоПартиямОбороты.Регистратор.Контрагент.НаименованиеПолное,
