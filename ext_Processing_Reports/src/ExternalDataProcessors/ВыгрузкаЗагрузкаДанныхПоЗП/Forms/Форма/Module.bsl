...
Функция ВыгрузитьНаСервере()
	...
	Запрос.Текст =
		...
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.Активность,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.Сотрудник,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ПорядокРасчета,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ОтработаноДней,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ОтработаноЧасов,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ОтработаноДнейШестидневка,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ОтработаноДнейПятидневка,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ОтработаноЧасовПятидневка,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.ОтработаноДнейКалендарных,
		|	ДанныеОВремениДляРасчетаСреднегоОбщий.Сторно
		|ИЗ
	...
	ИмяРегистра = "ДанныеОВремениДляРасчетаСреднегоОбщий";
...
Функция ЗагрузитьНаСервере(Знач Организация, Знач ТекстJSON, СписокОшибок)
	...
	Для Каждого НаборДвижений Из РезультатДесериализации Цикл
		...
		Для Каждого Движение Из СписокДвижений Цикл
			...
			СтрокаДвижения.Вставить("Период", XMLЗначение(Тип("Дата"), ЗначениеJSON));
			...
			Для Каждого Измерение Из МетаданныеРН.Измерения Цикл
				...
				Если Не ЗначениеЗаполнено(Значение) И Измерение.ЗапрещатьНезаполненныеЗначения Тогда
					Отказ = Истина;
				КонецЕсли;
				СтрокаДвижения.Вставить(Имя, Значение);
			КонецЦикла;
			...
			Если Не Отказ Тогда
				ЗаполнитьЗначенияСвойств(ДвиженияРН.Добавить(), СтрокаДвижения);	
			КонецЕсли;
